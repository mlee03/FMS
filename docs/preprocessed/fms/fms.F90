...
function monotonic_array_r4(array, direction) result(ret)
real(r4_kind), intent(in) :: array(:) !< An array of real values. If size(array) < 2, this function
...
# 7 "fms/include/fms_r4.fh" 2
# 719 "fms/fms.F90" 2

# 1 "fms/include/fms_r8.fh" 1
...
function monotonic_array_r8(array, direction) result(ret)
real(r8_kind), intent(in) :: array(:) !< An array of real values. If size(array) < 2, this function
                                            !! assumes the array is not monotonic; no fatal error will occur
                                            !! in this case.
integer, intent(out), optional :: direction !< If the input array is:
                                            !! >> strictly monotonic (small to large), then direction = +1.
