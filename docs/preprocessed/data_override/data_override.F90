# 1 "data_override/data_override.F90"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "data_override/data_override.F90"
!***********************************************************************
...
# 1 "data_override/include/data_override_r4.fh" 1
!***********************************************************************
!*                             Apache License 2.0
!*
!* This file is part of the GFDL Flexible Modeling System (FMS).
...
!> @ingroup data_override_r4
!! @{

# 1 "data_override/include/data_override.inc" 1
!***********************************************************************
!*                             Apache License 2.0
!*
!* This file is part of the GFDL Flexible Modeling System (FMS).
...
# 56 "data_override/include/data_override.inc" 2
...
type data_type
   ...
   real(r4_kind)        :: factor          !< For unit conversion, default=1, see OVERVIEW above
   real(r4_kind)        :: lon_start, lon_end, lat_start, lat_end
   integer            :: region_type
...
type override_type
   ...
   integer                          :: numthreads
   real(r4_kind), allocatable :: lon_in(:)
   real(r4_kind), allocatable :: lat_in(:)
...
real(r4_kind), dimension(:,:), target, allocatable :: lon_local_ocn, lat_local_ocn
real(r4_kind), dimension(:,:), target, allocatable :: lon_local_atm, lat_local_atm
real(r4_kind), dimension(:,:), target, allocatable :: lon_local_ice, lat_local_ice
real(r4_kind), dimension(:,:), target, allocatable :: lon_local_lnd, lat_local_lnd
real(r4_kind) :: min_glo_lon_ocn, max_glo_lon_ocn
real(r4_kind) :: min_glo_lon_atm, max_glo_lon_atm
real(r4_kind) :: min_glo_lon_lnd, max_glo_lon_lnd
real(r4_kind) :: min_glo_lon_ice, max_glo_lon_ice
...
public :: data_override_init_r4, data_override_unset_atm_r4, data_override_unset_ocn_r4, &
        & data_override_unset_lnd_r4, data_override_unset_ice_r4, data_override_0d_r4, &
        & data_override_2d_r4, data_override_3d_r4, data_override_ug_1d_r4, &
...
!! are comma-separated.
subroutine data_override_init_r4(Atm_domain_in, Ocean_domain_in, Ice_domain_in, Land_domain_in, Land_domainUG_in)
...
# 269 "data_override/include/data_override.inc"
...
end subroutine data_override_init_r4
...
subroutine check_valid_lat_lon(lon_start, lon_end, lat_start, lat_end)
   real(r4_kind), intent(in) :: lon_start !< Starting longitude of the data_override region
   real(r4_kind), intent(in) :: lon_end   !< Ending longitude of the data_override region
   real(r4_kind), intent(in) :: lat_start !< Starting lattiude of the data_override region
   real(r4_kind), intent(in) :: lat_end   !< Ending lattiude of the data_override region
...
subroutine data_override_0d_r4(gridname,fieldname_code,data_out,time,override,data_index)
  ...
  logical, intent(out), optional :: override !< true if the field has been overridden succesfully
  type(time_type), intent(in) :: time !< (target) model time
  real(r4_kind), intent(out) :: data_out !< output data array returned by this call
...
subroutine data_override_2d_r4(gridname,fieldname,data_2D,time,override, is_in, ie_in, js_in, je_in)
  ...
  real(r4_kind), dimension(:,:), intent(inout) :: data_2D !< data returned by this call
  integer,           optional,  intent(in) :: is_in, ie_in, js_in, je_in
  real(r4_kind), dimension(:,:,:), allocatable ::  data_3D
  integer       :: index1
  integer       :: i
...
!1  Look  for the data file in data_table
  ...
  data_3D(:,:,1) = data_2D
  call data_override_3d_r4(gridname,fieldname,data_3D,time,override,data_index=index1,&
...
subroutine data_override_3d_r4(gridname,fieldname_code,return_data,time,override,data_index, is_in, ie_in, js_in, je_in)
  character(len=3),             intent(in) :: gridname !< model grid ID
  character(len=*),             intent(in) :: fieldname_code !< field name as used in the model
  logical,           optional, intent(out) :: override !< true if the field has been overridden succesfully
  type(time_type),              intent(in) :: time !< (target) model time
  integer,           optional,  intent(in) :: data_index
  real(r4_kind), dimension(:,:,:), intent(inout) :: return_data !< data returned by this call
  integer,           optional,  intent(in) :: is_in, ie_in, js_in, je_in
  ...
  real(r4_kind), dimension(:,:), pointer :: lon_local =>NULL() !< of output (target) grid cells
  real(r4_kind), dimension(:,:), pointer :: lat_local =>NULL() !< of output (target) grid cells
  real(r4_kind), dimension(:), allocatable :: lon_tmp, lat_tmp
...
subroutine data_override_ug_1d_r4(gridname,fieldname,return_data,time,override)
  character(len=3),   intent(in) :: gridname !< model grid ID
  character(len=*),   intent(in) :: fieldname !< field to override
  real(r4_kind), dimension(:), intent(inout) :: return_data !< data returned by this call
  type(time_type),    intent(in) :: time !<  model time
  logical, intent(out), optional :: override !< true if the field has been overridden succesfully
  !local vars
  real(r4_kind), dimension(:,:), allocatable ::  data_SG
  ...
  call data_override_2d_r4(gridname,fieldname,data_SG,time,override)

  call mpp_pass_SG_to_UG(UG_domain, data_SG(:,:), return_data(:))

  deallocate(data_SG)
end subroutine data_override_ug_1d_r4

!> @brief Data override for 2D unstructured grids
subroutine data_override_ug_2d_r4(gridname,fieldname,return_data,time,override)
  character(len=3),     intent(in) :: gridname !< model grid ID
  character(len=*),     intent(in) :: fieldname !< field to override
  real(r4_kind), dimension(:,:), intent(inout) :: return_data !< data returned by this call
  type(time_type),      intent(in) :: time !<  model time
  ...
  !local vars
  real(r4_kind), dimension(:,:,:), allocatable ::  data_SG
  real(r4_kind), dimension(:,:),   allocatable ::  data_UG
  type(domainUG) :: UG_domain
  integer       :: index1
...
!1  Look  for the data file in data_table
  ...
  data_SG = 0._lkind
  call data_override_3d_r4(gridname,fieldname,data_SG,time,override)

  call mpp_pass_SG_to_UG(UG_domain, data_SG(:,:,:), data_UG(:,:))
  return_data(:,1:nlevel) = data_UG(:,1:nlevel)

  deallocate(data_SG, data_UG)
end subroutine data_override_ug_2d_r4
# 55 "data_override/include/data_override_r4.fh" 2
!> @}

# 68 "data_override/include/data_override_r4.fh"

# 24 "data_override/data_override.F90" 2
...
# 1 "data_override/include/data_override_r8.fh" 1
!***********************************************************************
!*                             Apache License 2.0
!*
!* This file is part of the GFDL Flexible Modeling System (FMS).
...
type data_type
   ...
   character(len=128) :: ext_weights_file_name
   character(len=128) :: ext_weights_source
   real(r8_kind)        :: factor          !< For unit conversion, default=1, see OVERVIEW above
   real(r8_kind)        :: lon_start, lon_end, lat_start, lat_end
   integer            :: region_type
   logical            :: multifile = .false.
   character(len=FMS_PATH_LEN) :: prev_file_name   !< name of netCDF data file for previous segment
   character(len=FMS_PATH_LEN) :: next_file_name   !< name of netCDF data file for next segment
...
type override_type
   ...
   integer                          :: comp_domain(4)     !< istart,iend,jstart,jend for compute domain
   integer                          :: numthreads
   real(r8_kind), allocatable :: lon_in(:)
   real(r8_kind), allocatable :: lat_in(:)
   logical, allocatable             :: need_compute(:)
   integer                          :: numwindows
   integer                          :: window_size(2)
   integer                          :: is_src, ie_src, js_src, je_src
...
real(r8_kind), dimension(:,:), target, allocatable :: lon_local_ocn, lat_local_ocn
real(r8_kind), dimension(:,:), target, allocatable :: lon_local_atm, lat_local_atm
real(r8_kind), dimension(:,:), target, allocatable :: lon_local_ice, lat_local_ice
real(r8_kind), dimension(:,:), target, allocatable :: lon_local_lnd, lat_local_lnd
real(r8_kind) :: min_glo_lon_ocn, max_glo_lon_ocn
real(r8_kind) :: min_glo_lon_atm, max_glo_lon_atm
real(r8_kind) :: min_glo_lon_lnd, max_glo_lon_lnd
real(r8_kind) :: min_glo_lon_ice, max_glo_lon_ice
integer :: num_fields = 0 !< number of fields in override_array already processed
...
public :: data_override_init_r8, data_override_unset_atm_r8, data_override_unset_ocn_r8, &
        & data_override_unset_lnd_r8, data_override_unset_ice_r8, data_override_0d_r8, &
        & data_override_2d_r8, data_override_3d_r8, data_override_ug_1d_r8, &
...
!! or data_table.yaml. Each line of data_table contains one data_entry. Items of data_entry
!! are comma-separated.
subroutine data_override_init_r8(Atm_domain_in, Ocean_domain_in, Ice_domain_in, Land_domain_in, Land_domainUG_in)
  type (domain2d), intent(in), optional :: Atm_domain_in    !> Atmosphere domain
  type (domain2d), intent(in), optional :: Ocean_domain_in  !> Ocean domain
  type (domain2d), intent(in), optional :: Ice_domain_in    !> Ice domain
  type (domain2d), intent(in), optional :: Land_domain_in   !> Land domain
...
end subroutine data_override_init_r8
...
!! Crashes if it is not.
subroutine check_valid_lat_lon(lon_start, lon_end, lat_start, lat_end)
   real(r8_kind), intent(in) :: lon_start !< Starting longitude of the data_override region
   real(r8_kind), intent(in) :: lon_end   !< Ending longitude of the data_override region
   real(r8_kind), intent(in) :: lat_start !< Starting lattiude of the data_override region
   real(r8_kind), intent(in) :: lat_end   !< Ending lattiude of the data_override region
...
subroutine data_override_unset_atm_r8
  atm_domain = NULL_DOMAIN2D
  if (allocated(lon_local_atm)) deallocate(lon_local_atm)
  if (allocated(lat_local_atm)) deallocate(lat_local_atm)
end subroutine

subroutine data_override_unset_ocn_r8
  ocn_domain = NULL_DOMAIN2D
  if (allocated(lon_local_ocn)) deallocate(lon_local_ocn)
  if (allocated(lat_local_ocn)) deallocate(lat_local_ocn)
end subroutine

subroutine data_override_unset_lnd_r8
  lnd_domain = NULL_DOMAIN2D
  if (allocated(lon_local_lnd)) deallocate(lon_local_lnd)
  if (allocated(lat_local_lnd)) deallocate(lat_local_lnd)
end subroutine

subroutine data_override_unset_ice_r8
  ice_domain = NULL_DOMAIN2D
  if (allocated(lon_local_ice)) deallocate(lon_local_ice)
  if (allocated(lat_local_ice)) deallocate(lat_local_ice)
end subroutine
...
!> @brief Routine to perform data override for scalar fields
subroutine data_override_0d_r8(gridname,fieldname_code,data_out,time,override,data_index)
  character(len=3), intent(in) :: gridname !< model grid ID (ocn,ice,atm,lnd)
  character(len=*), intent(in) :: fieldname_code !< field name as used in the model (may be
                                                 !! different from the name in NetCDF data file)
  logical, intent(out), optional :: override !< true if the field has been overridden succesfully
  type(time_type), intent(in) :: time !< (target) model time
  real(r8_kind), intent(out) :: data_out !< output data array returned by this call
  integer, intent(in), optional :: data_index
...
subroutine data_override_2d_r8(gridname,fieldname,data_2D,time,override, is_in, ie_in, js_in, je_in)
  ...
  logical, intent(out), optional :: override !< true if the field has been overridden succesfully
  type(time_type), intent(in) :: time !<  model time
  real(r8_kind), dimension(:,:), intent(inout) :: data_2D !< data returned by this call
  integer,           optional,  intent(in) :: is_in, ie_in, js_in, je_in
  real(r8_kind), dimension(:,:,:), allocatable ::  data_3D
  integer       :: index1
  integer       :: i
...
!1  Look  for the data file in data_table
  ...
  allocate(data_3D(size(data_2D,1),size(data_2D,2),1))
  data_3D(:,:,1) = data_2D
  call data_override_3d_r8(gridname,fieldname,data_3D,time,override,data_index=index1,&
                       is_in=is_in,ie_in=ie_in,js_in=js_in,je_in=je_in)
...
!> @brief This routine performs data override for 3D fields
subroutine data_override_3d_r8(gridname,fieldname_code,return_data,time,override,data_index, is_in, ie_in, js_in, je_in)
  character(len=3),             intent(in) :: gridname !< model grid ID
  character(len=*),             intent(in) :: fieldname_code !< field name as used in the model
  logical,           optional, intent(out) :: override !< true if the field has been overridden succesfully
  type(time_type),              intent(in) :: time !< (target) model time
  integer,           optional,  intent(in) :: data_index
  real(r8_kind), dimension(:,:,:), intent(inout) :: return_data !< data returned by this call
  integer,           optional,  intent(in) :: is_in, ie_in, js_in, je_in
  logical, dimension(:,:,:),   allocatable :: mask_out
  ...
  integer            :: axis_sizes(4)
  character(len=32)  :: axis_names(4)
  real(r8_kind), dimension(:,:), pointer :: lon_local =>NULL() !< of output (target) grid cells
  real(r8_kind), dimension(:,:), pointer :: lat_local =>NULL() !< of output (target) grid cells
  real(r8_kind), dimension(:), allocatable :: lon_tmp, lat_tmp
...
!> @brief Data override for 1D unstructured grids
subroutine data_override_ug_1d_r8(gridname,fieldname,return_data,time,override)
  character(len=3),   intent(in) :: gridname !< model grid ID
  character(len=*),   intent(in) :: fieldname !< field to override
  real(r8_kind), dimension(:), intent(inout) :: return_data !< data returned by this call
  type(time_type),    intent(in) :: time !<  model time
  logical, intent(out), optional :: override !< true if the field has been overridden succesfully
  !local vars
  real(r8_kind), dimension(:,:), allocatable ::  data_SG
  type(domainUG) :: UG_domain
  integer       :: index1
  integer       :: i
  integer, dimension(4) :: comp_domain = 0  !< istart,iend,jstart,jend for compute domain
  ...
  call data_override_2d_r8(gridname,fieldname,data_SG,time,override)

  call mpp_pass_SG_to_UG(UG_domain, data_SG(:,:), return_data(:))

  deallocate(data_SG)
end subroutine data_override_ug_1d_r8

!> @brief Data override for 2D unstructured grids
subroutine data_override_ug_2d_r8(gridname,fieldname,return_data,time,override)
  character(len=3),     intent(in) :: gridname !< model grid ID
  character(len=*),     intent(in) :: fieldname !< field to override
  real(r8_kind), dimension(:,:), intent(inout) :: return_data !< data returned by this call
  type(time_type),      intent(in) :: time !<  model time
  ...
  !local vars
  real(r8_kind), dimension(:,:,:), allocatable ::  data_SG
  real(r8_kind), dimension(:,:),   allocatable ::  data_UG
  type(domainUG) :: UG_domain
  integer       :: index1
  integer       :: i, nlevel, nlevel_max
  integer, dimension(4) :: comp_domain = 0  !< istart,iend,jstart,jend for compute domain
...
!1  Look  for the data file in data_table
  ...
  allocate(data_UG(size(return_data,1), nlevel_max))
  data_SG = 0._lkind
  call data_override_3d_r8(gridname,fieldname,data_SG,time,override)

  call mpp_pass_SG_to_UG(UG_domain, data_SG(:,:,:), data_UG(:,:))
  return_data(:,1:nlevel) = data_UG(:,1:nlevel)

  deallocate(data_SG, data_UG)
end subroutine data_override_ug_2d_r8
# 55 "data_override/include/data_override_r8.fh" 2
!> @}

# 32 "data_override/data_override.F90" 2
