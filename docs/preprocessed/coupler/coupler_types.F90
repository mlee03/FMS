...
# 46 "coupler/coupler_types.F90" 2
...
# 1 "coupler/include/coupler_types_r4.fh" 1
!***********************************************************************
!*                             Apache License 2.0
!*
!* This file is part of the GFDL Flexible Modeling System (FMS).
...
# 1 "coupler/include/coupler_types.inc" 1
!***********************************************************************
!*                             Apache License 2.0
!*
!* This file is part of the GFDL Flexible Modeling System (FMS).
...
!! @author Richard Slater, John Dunne
  ...
  subroutine CT_rescale_data_2d_r4(var, scale, halo_size, bc_index, field_index,&
    ...
    real(r4_kind),                       intent(in)    :: scale   !< A scaling factor to multiply fields by
    ...
    if (present(bc_index)) then
      ...
      if (present(field_index)) then ; if (field_index > var%bc_r4(bc_index)%num_fields)&
          & call mpp_error(FATAL, "CT_rescale_data_2d: field_index is present and exceeds num_fields for" //&
          & trim(var%bc_r4(bc_index)%name) )
      endif
    elseif (present(field_index)) then
    ...
    do n = n1, n2
      ...
      if (do_bc .and. present(exclude_flux_type))&
          & do_bc = .not.(trim(var%bc_r4(n)%flux_type) == trim(exclude_flux_type))
      if (do_bc .and. present(only_flux_type))&
          & do_bc = (trim(var%bc_r4(n)%flux_type) == trim(only_flux_type))
      if (do_bc .and. present(pass_through_ice))&
          & do_bc = (pass_through_ice .eqv. var%bc_r4(n)%pass_through_ice)
      if (.not.do_bc) cycle
        ...
        if ( associated(var%bc_r4(n)%field(m)%values) ) then
          if (scale == 0.0_kindl) then
            if (present(halo_size)) then
              do j=var%jsc-halo,var%jec+halo
                do i=var%isc-halo,var%iec+halo
                  var%bc_r4(n)%field(m)%values(i,j) = 0.0_kindl
                enddo
              enddo
            else
              var%bc_r4(n)%field(m)%values(:,:) = 0.0_kindl
            endif
          else
            do j=var%jsc-halo,var%jec+halo
              do i=var%isc-halo,var%iec+halo
                var%bc_r4(n)%field(m)%values(i,j) = scale * var%bc_r4(n)%field(m)%values(i,j)
              enddo
            enddo
          endif
        endif
          ...
          if (scale == 0.0_kindl) then
            if (present(halo_size)) then
              do k=var%ks,var%ke
                do j=var%jsc-halo,var%jec+halo
                  do i=var%isc-halo,var%iec+halo
                    var%bc_r4(n)%field(m)%values(i,j,k) = 0.0_kindl
                  enddo
                enddo
              enddo
            else
            ...
            do k=var%ks,var%ke
              do j=var%jsc-halo,var%jec+halo
                do i=var%isc-halo,var%iec+halo
                  var%bc_r4(n)%field(m)%values(i,j,k) = scale * var%bc_r4(n)%field(m)%values(i,j,k)
                enddo
              enddo
            enddo
          endif
  ...
  subroutine CT_increment_data_2d_3d_r4(var_in, weights, var, halo_size, bc_index, field_index,&
      & scale_factor, scale_prev, exclude_flux_type, only_flux_type, pass_through_ice)
    type(coupler_3d_bc_type),   intent(in)    :: var_in  !< BC_type structure with the data to add to the other type
    real(r4_kind), dimension(:,:,:),     intent(in)    :: weights !< An array of normalized weights for the 3d-data
    ...
    integer,          optional, intent(in)    :: field_index !< The index of the field in the
                                                         !! boundary condition that is being copied
    real(r4_kind),             optional, intent(in)    :: scale_factor  !< A scaling factor for the data that
                                                                             !! is being added
    real(r4_kind),             optional, intent(in)    :: scale_prev    !< A scaling factor for the data that
    ...
    do n = n1, n2
      increment_bc = .true.
      if (increment_bc .and. present(exclude_flux_type))&
          & increment_bc = .not.(trim(var_in%bc_r4(n)%flux_type) == trim(exclude_flux_type))
      if (increment_bc .and. present(only_flux_type))&
          & increment_bc = (trim(var_in%bc_r4(n)%flux_type) == trim(only_flux_type))
      if (increment_bc .and. present(pass_through_ice))&
          & increment_bc = (pass_through_ice .eqv. var_in%bc_r4(n)%pass_through_ice)
      if (.not.increment_bc) cycle
            ...
            do j=var%jsc-halo,var%jec+halo
              do i=var%isc-halo,var%iec+halo
                var%bc_r4(n)%field(m)%values(i,j) = sc_prev * var%bc_r4(n)%field(m)%values(i,j) +&
                    & (scale * weights(i+iow,j+jow,k+kow)) * var_in%bc_r4(n)%field(m)%values(i+io1,j+io1,k)
              enddo
            enddo
          enddo
        endif
  ...
  subroutine CT_extract_data_2d_r4(var_in, bc_index, field_index, array_out,&
    ...
    integer,                    intent(in)    :: field_index !< The index of the field in the
                                                           !! boundary condition that is being copied
    real(r4_kind), dimension(1:,1:),     intent(out)   :: array_out !< The recipient array for the field; its size
                                                           !! must match the size of the data being copied
                                                           !! unless idim and jdim are supplied.
    real(r4_kind), optional, intent(in) :: scale_factor !< A scaling factor for the data that is being added
    integer,          optional, intent(in)    :: halo_size !< The extent of the halo to copy; 0 by default
    ...
    do j=var_in%jsc-halo,var_in%jec+halo
      do i=var_in%isc-halo,var_in%iec+halo
        array_out(i+i_off,j+j_off) = scale * var_in%bc_r4(bc_index)%field(field_index)%values(i,j)
      enddo
    enddo
  ...
  subroutine CT_extract_data_3d_2d_r4(var_in, bc_index, field_index, k_in, array_out,&
    ...
    do j=var_in%jsc-halo,var_in%jec+halo
      do i=var_in%isc-halo,var_in%iec+halo
        array_out(i+i_off,j+j_off) = scale * var_in%bc_r4(bc_index)%field(field_index)%values(i,j,k_in)
      enddo
    enddo
  ...
  subroutine CT_extract_data_3d_r4(var_in, bc_index, field_index, array_out,&
    ...
    integer,                    intent(in)    :: field_index !< The index of the field in the
                                                           !! boundary condition that is being copied
    real(r4_kind), dimension(1:,1:,1:),  intent(out)   :: array_out !< The recipient array for the field; its size
                                                           !! must match the size of the data being copied
                                                           !! unless idim and jdim are supplied.
    real(r4_kind), optional, intent(in)    :: scale_factor !< A scaling factor for the data that is being added
    integer,          optional, intent(in)    :: halo_size !< The extent of the halo to copy; 0 by default
    ...
    do k=var_in%ks,var_in%ke
      do j=var_in%jsc-halo,var_in%jec+halo
        do i=var_in%isc-halo,var_in%iec+halo
          array_out(i+i_off,j+j_off,k+k_off) = scale * var_in%bc_r4(bc_index)%field(field_index)%values(i,j,k)
        enddo
      enddo
    enddo
  ...
  subroutine CT_set_data_2d_r4(array_in, bc_index, field_index, var,&
      & scale_factor, halo_size, idim, jdim)
    real(r4_kind), dimension(1:,1:),     intent(in)   :: array_in   !< The source array for the field; its size
                                                           !! must match the size of the data being copied
                                                           !! unless idim and jdim are supplied.
    integer,                    intent(in)    :: bc_index  !< The index of the boundary condition
                                                           !! that is being copied
    ...
    if (field_index > var%bc_r4(bc_index)%num_fields)&
        & call mpp_error(FATAL, trim(error_header)//" field_index exceeds num_fields for" //&
        & trim(var%bc_r4(bc_index)%name) )
    ...
    do j=var%jsc-halo,var%jec+halo
      do i=var%isc-halo,var%iec+halo
        var%bc_r4(bc_index)%field(field_index)%values(i,j) = scale * array_in(i+i_off,j+j_off)
      enddo
    enddo
  ...
  subroutine CT_set_data_2d_3d_r4(array_in, bc_index, field_index, k_out, var,&
    ...
    do j=var%jsc-halo,var%jec+halo
      do i=var%isc-halo,var%iec+halo
        var%bc_r4(bc_index)%field(field_index)%values(i,j,k_out) = scale * array_in(i+i_off,j+j_off)
      enddo
    enddo
  ...
  subroutine CT_set_data_3d_r4(array_in, bc_index, field_index, var,&
      & scale_factor, halo_size, idim, jdim)
    real(r4_kind), dimension(1:,1:,1:),  intent(in)    :: array_in  !< The source array for the field; its size
                                                           !! must match the size of the data being copied
                                                           !! unless idim and jdim are supplied.
    integer,                    intent(in)    :: bc_index  !< The index of the boundary condition
                                                           !! that is being copied
    ...
    do k=var%ks,var%ke
      do j=var%jsc-halo,var%jec+halo
        do i=var%isc-halo,var%iec+halo
          var%bc_r4(bc_index)%field(field_index)%values(i,j,k) = scale * array_in(i+i_off,j+j_off,k+k_off)
        enddo
      enddo
    enddo
  end subroutine CT_set_data_3d_r4
# 53 "coupler/include/coupler_types_r4.fh" 2
# 3999 "coupler/coupler_types.F90" 2

# 1 "coupler/include/coupler_types_r8.fh" 1
!***********************************************************************
!*                             Apache License 2.0
!*
!* This file is part of the GFDL Flexible Modeling System (FMS).
...
!! @author Richard Slater, John Dunne
  ...
  subroutine CT_rescale_data_2d_r8(var, scale, halo_size, bc_index, field_index,&
      & exclude_flux_type, only_flux_type, pass_through_ice)
    type(coupler_2d_bc_type),   intent(inout) :: var !< The BC_type structure whose fields are being rescaled
    real(r8_kind),                       intent(in)    :: scale   !< A scaling factor to multiply fields by
    integer,          optional, intent(in)    :: halo_size !< The extent of the halo to copy; 0 by default or
                                                           !! the full arrays if scale is 0.
    integer,          optional, intent(in)    :: bc_index  !< The index of the boundary condition
                                                           !! that is being copied
        ...
        if ( associated(var%bc(n)%field(m)%values) ) then
          if (scale == 0.0_kindl) then
            if (present(halo_size)) then
              do j=var%jsc-halo,var%jec+halo
                do i=var%isc-halo,var%iec+halo
                  var%bc(n)%field(m)%values(i,j) = 0.0_kindl
                enddo
              enddo
            else
              var%bc(n)%field(m)%values(:,:) = 0.0_kindl
            endif
          else
            do j=var%jsc-halo,var%jec+halo
              do i=var%isc-halo,var%iec+halo
          ...
          if (scale == 0.0_kindl) then
            if (present(halo_size)) then
              do k=var%ks,var%ke
                do j=var%jsc-halo,var%jec+halo
                  do i=var%isc-halo,var%iec+halo
                    var%bc(n)%field(m)%values(i,j,k) = 0.0_kindl
                  enddo
                enddo
              enddo
            else
  ...
  subroutine CT_increment_data_2d_3d_r8(var_in, weights, var, halo_size, bc_index, field_index,&
      & scale_factor, scale_prev, exclude_flux_type, only_flux_type, pass_through_ice)
    type(coupler_3d_bc_type),   intent(in)    :: var_in  !< BC_type structure with the data to add to the other type
    real(r8_kind), dimension(:,:,:),     intent(in)    :: weights !< An array of normalized weights for the 3d-data
                                                        !! to increment the 2d-data.  There is no renormalization,
                                                        !! so if the weights do not sum to 1 in the 3rd dimension
                                                        !! there may be adverse consequences!
    type(coupler_2d_bc_type),   intent(inout) :: var !< The BC_type structure whose fields are being incremented
    ...
    integer,          optional, intent(in)    :: field_index !< The index of the field in the
                                                         !! boundary condition that is being copied
    real(r8_kind),             optional, intent(in)    :: scale_factor  !< A scaling factor for the data that
                                                                             !! is being added
    real(r8_kind),             optional, intent(in)    :: scale_prev    !< A scaling factor for the data that
                                                                      !! is already here and is being added
    character(len=*), optional, intent(in)    :: exclude_flux_type !< A string describing which types
                                                         !! of fluxes to exclude from this increment.
    character(len=*), optional, intent(in)    :: only_flux_type    !< A string describing which types
  ...
  subroutine CT_extract_data_2d_r8(var_in, bc_index, field_index, array_out,&
    ...
    integer,                    intent(in)    :: field_index !< The index of the field in the
                                                           !! boundary condition that is being copied
    real(r8_kind), dimension(1:,1:),     intent(out)   :: array_out !< The recipient array for the field; its size
                                                           !! must match the size of the data being copied
                                                           !! unless idim and jdim are supplied.
    real(r8_kind), optional, intent(in) :: scale_factor !< A scaling factor for the data that is being added
    integer,          optional, intent(in)    :: halo_size !< The extent of the halo to copy; 0 by default
  ...
  subroutine CT_extract_data_3d_r8(var_in, bc_index, field_index, array_out,&
    ...
    integer,                    intent(in)    :: field_index !< The index of the field in the
                                                           !! boundary condition that is being copied
    real(r8_kind), dimension(1:,1:,1:),  intent(out)   :: array_out !< The recipient array for the field; its size
                                                           !! must match the size of the data being copied
                                                           !! unless idim and jdim are supplied.
    real(r8_kind), optional, intent(in)    :: scale_factor !< A scaling factor for the data that is being added
    integer,          optional, intent(in)    :: halo_size !< The extent of the halo to copy; 0 by default
  ...
  subroutine CT_set_data_2d_r8(array_in, bc_index, field_index, var,&
      & scale_factor, halo_size, idim, jdim)
    real(r8_kind), dimension(1:,1:),     intent(in)   :: array_in   !< The source array for the field; its size
                                                           !! must match the size of the data being copied
                                                           !! unless idim and jdim are supplied.
    integer,                    intent(in)    :: bc_index  !< The index of the boundary condition
                                                           !! that is being copied
  ...
  subroutine CT_set_data_3d_r8(array_in, bc_index, field_index, var,&
      & scale_factor, halo_size, idim, jdim)
    real(r8_kind), dimension(1:,1:,1:),  intent(in)    :: array_in  !< The source array for the field; its size
                                                           !! must match the size of the data being copied
                                                           !! unless idim and jdim are supplied.
    integer,                    intent(in)    :: bc_index  !< The index of the boundary condition
                                                           !! that is being copied
...
# 53 "coupler/include/coupler_types_r8.fh" 2
# 4000 "coupler/coupler_types.F90" 2
