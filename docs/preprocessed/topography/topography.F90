...
# 256 "topography/topography.F90" 2
...
# 1 "topography/include/topography_r4.fh" 1
!***********************************************************************
!*                             Apache License 2.0
!*
!* This file is part of the GFDL Flexible Modeling System (FMS).
...
# 1 "topography/include/topography.inc" 1
!***********************************************************************
!*                             Apache License 2.0
!*
!* This file is part of the GFDL Flexible Modeling System (FMS).
...
!***********************************************************************
 ...
 function get_topog_mean_1d_r4(blon, blat, zmean)

   real(kind=r4_kind), intent(in),  dimension(:)   :: blon !< Longitude (radians) at grid box boundaries
   real(kind=r4_kind), intent(in),  dimension(:)   :: blat !< Latitude (radians) at grid box boundaries
   real(kind=r4_kind), intent(out), dimension(:,:) :: zmean !< Mean surface height(meters). Size must be
                                              !! size(blon)-1 by size(blat)-1
   logical :: get_topog_mean_1d_r4
...
!-----------------------------------------------------------------------
   ...
   get_topog_mean_1d_r4 = open_topog_file()

   if (get_topog_mean_1d_r4) call interp_topog(blon, blat, zmean)

!-----------------------------------------------------------------------

 end function get_topog_mean_1d_r4

!############################################################

 function get_topog_mean_2d_r4(blon, blat, zmean)

   real(kind=r4_kind), intent(in),  dimension(:,:) :: blon, blat
   real(kind=r4_kind), intent(out), dimension(:,:) :: zmean
   logical :: get_topog_mean_2d_r4
   integer, parameter :: lkind = r4_kind
!-----------------------------------------------------------------------
   if (.not. module_is_initialized) call topography_init()
   ...
   get_topog_mean_2d_r4 = open_topog_file()

   if (get_topog_mean_2d_r4) call interp_topog(blon, blat, zmean)
!-----------------------------------------------------------------------

 end function get_topog_mean_2d_r4
...
!#######################################################################
 ...
 function get_topog_stdev_1d_r4(blon, blat, stdev)

   real(kind=r4_kind), intent(in),  dimension(:)   :: blon !< Longitude (radians) at grid box boundaries
   real(kind=r4_kind), intent(in),  dimension(:)   :: blat !< Latitude (radians) at grid box boundaries
   real(kind=r4_kind), intent(out), dimension(:,:) :: stdev !< The standard deviation of surface height (in
                                     !! meters) within given input model grid boxes. Size must be
                                     !! size(blon)-1 by size(blat)-1
   logical :: get_topog_stdev_1d_r4
   integer, parameter :: lkind = r4_kind
!-----------------------------------------------------------------------
   if (.not. module_is_initialized) call topography_init()
   ...
   get_topog_stdev_1d_r4 = open_topog_file()

   if (get_topog_stdev_1d_r4) call interp_topog(blon, blat, &
              stdev, flag=COMPUTE_STDEV)

!-----------------------------------------------------------------------

 end function get_topog_stdev_1d_r4

!#######################################################################

 function get_topog_stdev_2d_r4(blon, blat, stdev)

   real(r4_kind), intent(in),  dimension(:,:) :: blon, blat
   real(r4_kind), intent(out), dimension(:,:) :: stdev
   logical :: get_topog_stdev_2d_r4
   integer, parameter :: lkind = r4_kind
!-----------------------------------------------------------------------
   if (.not. module_is_initialized) call topography_init()
   ...
   get_topog_stdev_2d_r4 = open_topog_file()

   if (get_topog_stdev_2d_r4) call interp_topog(blon, blat, &
              stdev, flag=COMPUTE_STDEV)
!-----------------------------------------------------------------------

 end function get_topog_stdev_2d_r4
...
!#######################################################################
 ...
 function get_ocean_frac_1d_r4(blon, blat, ocean_frac)

 real(r4_kind), intent(in),  dimension(:)   :: blon !< Longitude (radians) at grid box boundaries
 real(r4_kind), intent(in),  dimension(:)   :: blat !< Latitude (radians) at grid box boundaries
 real(r4_kind), intent(out), dimension(:,:) :: ocean_frac !< The fractional amount (0-1) of ocean in a grid
                                     !! box. The size must be size(blon)-1 by size(blat)-1
 logical :: get_ocean_frac_1d_r4
 integer, parameter :: lkind = r4_kind
!-----------------------------------------------------------------------
   if (.not. module_is_initialized) call topography_init()
   ...
   get_ocean_frac_1d_r4 = open_water_file()
   if(get_ocean_frac_1d_r4)  call interp_water( blon, blat, &
                ocean_frac, do_ocean=.true. )

!-----------------------------------------------------------------------

 end function get_ocean_frac_1d_r4

!#######################################################################

 function get_ocean_frac_2d_r4(blon, blat, ocean_frac)

 real(kind=r4_kind), intent(in),  dimension(:,:) :: blon, blat
 real(kind=r4_kind), intent(out), dimension(:,:) :: ocean_frac
 logical :: get_ocean_frac_2d_r4
 integer, parameter :: lkind = r4_kind
!-----------------------------------------------------------------------
   if (.not. module_is_initialized) call topography_init()
   ...
   get_ocean_frac_2d_r4 = open_water_file()
   if(get_ocean_frac_2d_r4)  call interp_water( blon, blat, &
                ocean_frac, do_ocean=.true. )

!-----------------------------------------------------------------------

 end function get_ocean_frac_2d_r4
...
!#######################################################################
 ...
 !> @returns A logical value of true if the output field was successfully created. A value of false
 !! may be returned if the Navy 1/6 degree percent water data set was not readable.
 function get_ocean_mask_1d_r4(blon, blat, ocean_mask)

 real(kind=r4_kind), intent(in),  dimension(:)   :: blon !< Longitude (radians) at grid box boundaries
 real(kind=r4_kind), intent(in),  dimension(:)   :: blat !< Latitude (radians) at grid box boundaries
 logical, intent(out), dimension(:,:) :: ocean_mask !< Mask for ocean in a grid box.
                                                 !! The size must be size(blon)-1 by size(blat)-1
 logical :: get_ocean_mask_1d_r4
 real(kind=r4_kind), dimension(size(ocean_mask,1),size(ocean_mask,2)) :: ocean_frac
 integer, parameter :: lkind = r4_kind
!-----------------------------------------------------------------------
   if (.not. module_is_initialized) call topography_init()
   ...
   if (get_ocean_frac(blon, blat, ocean_frac) ) then
     ...
     get_ocean_mask_1d_r4 = .true.
   else
    get_ocean_mask_1d_r4 = .false.
   endif
!-----------------------------------------------------------------------

 end function get_ocean_mask_1d_r4

!#######################################################################

 function get_ocean_mask_2d_r4(blon, blat, ocean_mask)

 real(kind=r4_kind), intent(in),  dimension(:,:) :: blon, blat
 logical, intent(out), dimension(:,:) :: ocean_mask
 logical :: get_ocean_mask_2d_r4
 real(kind=r4_kind), dimension(size(ocean_mask,1),size(ocean_mask,2)) :: ocean_frac
 integer, parameter :: lkind = r4_kind
!-----------------------------------------------------------------------
   if (.not. module_is_initialized) call topography_init()
   ...
   if (get_ocean_frac(blon, blat, ocean_frac) ) then
     ...
     get_ocean_mask_2d_r4 = .true.
   else
     get_ocean_mask_2d_r4 = .false.
   endif

!-----------------------------------------------------------------------

 end function get_ocean_mask_2d_r4
 ...
 !! Check the input grid size and output field size.
 function get_water_frac_1d_r4(blon, blat, water_frac)
 real(kind=r4_kind), intent(in),  dimension(:)   :: blon !< The longitude (in radians) at grid box boundaries.
 real(kind=r4_kind), intent(in),  dimension(:)   :: blat !< The latitude (in radians) at grid box boundaries.
 real(kind=r4_kind), intent(out), dimension(:,:) :: water_frac !< The fractional amount (0 to 1) of water in a
                          !! grid box. The size of this field must be size(blon)-1 by size(blat)-1.
 logical :: get_water_frac_1d_r4
 integer, parameter :: lkind = r4_kind
...
!-----------------------------------------------------------------------
   ...
   get_water_frac_1d_r4 = open_water_file()
   if(get_water_frac_1d_r4) call interp_water( blon, blat, water_frac )

!-----------------------------------------------------------------------

 end function get_water_frac_1d_r4

!#######################################################################

 function get_water_frac_2d_r4(blon, blat, water_frac)

 real(kind=r4_kind), intent(in),  dimension(:,:)   :: blon !< The longitude (in radians) at grid box boundaries.
 real(kind=r4_kind), intent(in),  dimension(:,:)   :: blat !< The latitude (in radians) at grid box boundaries.
 real(kind=r4_kind), intent(out), dimension(:,:) :: water_frac !< The fractional amount (0 to 1) of water in a
                          !! grid box. The size of this field must be size(blon)-1 by size(blat)-1.
 logical :: get_water_frac_2d_r4
 integer, parameter :: lkind = r4_kind
...
!-----------------------------------------------------------------------
   ...
   get_water_frac_2d_r4 = open_water_file()
   if(get_water_frac_2d_r4) call interp_water( blon, blat, water_frac )

!-----------------------------------------------------------------------

 end function get_water_frac_2d_r4
...
!#######################################################################
 ...
 !! was successfully created. A value of FALSE may be returned
 !! if the Navy 1/6 degree percent water data set was not readable.
 function get_water_mask_1d_r4(blon, blat, water_mask)

 real(kind=r4_kind), intent(in),  dimension(:)   :: blon !< The longitude (in radians) at grid box boundaries.
 real(kind=r4_kind), intent(in),  dimension(:)   :: blat !< The latitude (in radians) at grid box boundaries.
 logical, intent(out), dimension(:,:) :: water_mask !< A binary mask for water (true) or land (false).
                                     !! The size of this field must be size(blon)-1 by size(blat)-1.
 logical :: get_water_mask_1d_r4

 real(kind=r4_kind), dimension(size(water_mask,1),size(water_mask,2)) :: water_frac
 integer, parameter :: lkind = r4_kind
!-----------------------------------------------------------------------
   if (.not. module_is_initialized) call topography_init()
   ...
   if (get_water_frac(blon, blat, water_frac) ) then
     ...
     get_water_mask_1d_r4 = .true.
   else
    get_water_mask_1d_r4 = .false.
   endif
!-----------------------------------------------------------------------

 end function get_water_mask_1d_r4

!#######################################################################

 function get_water_mask_2d_r4(blon, blat, water_mask)

 real(kind=r4_kind), intent(in),  dimension(:,:)   :: blon !< The longitude (in radians) at grid box boundaries.
 real(kind=r4_kind), intent(in),  dimension(:,:)   :: blat !< The latitude (in radians) at grid box boundaries.
 logical, intent(out), dimension(:,:) :: water_mask !< A binary mask for water (true) or land (false).
                                     !! The size of this field must be size(blon)-1 by size(blat)-1.
 logical :: get_water_mask_2d_r4
 real(kind=r4_kind), dimension(size(water_mask,1),size(water_mask,2)) :: water_frac
 integer, parameter :: lkind = r4_kind
!-----------------------------------------------------------------------
   if (.not. module_is_initialized) call topography_init()
   ...
   if (get_water_frac(blon, blat, water_frac) ) then
     ...
     get_water_mask_2d_r4 = .true.
   else
     get_water_mask_2d_r4 = .false.
   endif

!-----------------------------------------------------------------------

 end function get_water_mask_2d_r4

!#######################################################################

subroutine interp_topog_1d_r4( blon, blat, zout, flag)
real(kind=r4_kind), intent(in)  :: blon(:), blat(:)
real(kind=r4_kind), intent(out) :: zout(:,:)
integer, intent(in), optional :: flag

real(kind=r4_kind) :: xdat(ipts+1), ydat(jpts+1)
real(kind=r4_kind) :: zdat(ipts,jpts)
real(kind=r4_kind) :: zout2(size(zout,1),size(zout,2))
integer, parameter       :: lkind = r4_kind
...
end subroutine interp_topog_1d_r4

!#######################################################################

subroutine interp_topog_2d_r4( blon, blat, zout, flag )
real(kind=r4_kind), intent(in)  :: blon(:,:), blat(:,:)
real(kind=r4_kind), intent(out) :: zout(:,:)
integer, intent(in), optional :: flag
...
end subroutine interp_topog_2d_r4

!#######################################################################

subroutine find_indices_r4( ybeg, yend, ydat, js, je )
real(kind=r4_kind), intent(in)  :: ybeg, yend, ydat(:)
...
end subroutine find_indices_r4

!#######################################################################
subroutine input_data_r4( indx, xdat, ydat, zdat )
integer, intent(in) :: indx
real(kind=r4_kind), intent(out) :: xdat(ipts+1), ydat(jpts+1), zdat(ipts,jpts)
...
end subroutine input_data_r4

!#######################################################################

subroutine interp_water_1d_r4( blon, blat, zout, do_ocean )
real(kind=r4_kind), intent(in)  :: blon(:), blat(:)
real(kind=r4_kind), intent(out) :: zout(:,:)
logical, intent(in), optional :: do_ocean
real(kind=r4_kind) :: xdat(ipts+1), ydat(jpts+1), zdat(ipts,jpts)
...
end subroutine interp_water_1d_r4

!#######################################################################

subroutine interp_water_2d_r4( blon, blat, zout, do_ocean )
real(kind=r4_kind), intent(in)  :: blon(:,:), blat(:,:)
real(kind=r4_kind), intent(out) :: zout(:,:)
logical, intent(in), optional :: do_ocean
real(kind=r4_kind) :: xdat(ipts+1), ydat(jpts+1), zdat(ipts,jpts)
...
end subroutine interp_water_2d_r4

!#######################################################################

subroutine determine_ocean_points_r4(pctwater)
real(kind=r4_kind), intent(inout) :: pctwater(:,:)
logical :: ocean(size(pctwater,1),size(pctwater,2))
integer :: i, j, m, n, im, ip, jm, jp, new
integer, parameter :: lkind = r4_kind
real(kind=r4_kind) :: ocean_pct_crit = 0.500_lkind
...
end subroutine determine_ocean_points_r4
# 79 "topography/include/topography_r4.fh" 2
# 355 "topography/topography.F90" 2

# 1 "topography/include/topography_r8.fh" 1
!***********************************************************************
!*                             Apache License 2.0
!*
!* This file is part of the GFDL Flexible Modeling System (FMS).
...
!***********************************************************************
 ...
 function get_topog_mean_1d_r8(blon, blat, zmean)

   real(kind=r8_kind), intent(in),  dimension(:)   :: blon !< Longitude (radians) at grid box boundaries
   real(kind=r8_kind), intent(in),  dimension(:)   :: blat !< Latitude (radians) at grid box boundaries
   real(kind=r8_kind), intent(out), dimension(:,:) :: zmean !< Mean surface height(meters). Size must be
                                              !! size(blon)-1 by size(blat)-1
   logical :: get_topog_mean_1d_r8
...
!-----------------------------------------------------------------------
   ...
   get_topog_mean_1d_r8 = open_topog_file()

   if (get_topog_mean_1d_r8) call interp_topog(blon, blat, zmean)

!-----------------------------------------------------------------------

 end function get_topog_mean_1d_r8

!############################################################

 function get_topog_mean_2d_r8(blon, blat, zmean)

   real(kind=r8_kind), intent(in),  dimension(:,:) :: blon, blat
   real(kind=r8_kind), intent(out), dimension(:,:) :: zmean
   logical :: get_topog_mean_2d_r8
   integer, parameter :: lkind = r8_kind
!-----------------------------------------------------------------------
   if (.not. module_is_initialized) call topography_init()
   ...
   get_topog_mean_2d_r8 = open_topog_file()

   if (get_topog_mean_2d_r8) call interp_topog(blon, blat, zmean)
!-----------------------------------------------------------------------

 end function get_topog_mean_2d_r8
...
!#######################################################################
 ...
 !! @returns A logical value of true if the output field was successfully created and false
 !! if the input topography data set was not readable.
 function get_topog_stdev_1d_r8(blon, blat, stdev)

   real(kind=r8_kind), intent(in),  dimension(:)   :: blon !< Longitude (radians) at grid box boundaries
   real(kind=r8_kind), intent(in),  dimension(:)   :: blat !< Latitude (radians) at grid box boundaries
   real(kind=r8_kind), intent(out), dimension(:,:) :: stdev !< The standard deviation of surface height (in
                                     !! meters) within given input model grid boxes. Size must be
                                     !! size(blon)-1 by size(blat)-1
   logical :: get_topog_stdev_1d_r8
   integer, parameter :: lkind = r8_kind
!-----------------------------------------------------------------------
   if (.not. module_is_initialized) call topography_init()
   ...
   get_topog_stdev_1d_r8 = open_topog_file()

   if (get_topog_stdev_1d_r8) call interp_topog(blon, blat, &
              stdev, flag=COMPUTE_STDEV)

!-----------------------------------------------------------------------

 end function get_topog_stdev_1d_r8

!#######################################################################

 function get_topog_stdev_2d_r8(blon, blat, stdev)

   real(r8_kind), intent(in),  dimension(:,:) :: blon, blat
   real(r8_kind), intent(out), dimension(:,:) :: stdev
   logical :: get_topog_stdev_2d_r8
   integer, parameter :: lkind = r8_kind
!-----------------------------------------------------------------------
   if (.not. module_is_initialized) call topography_init()
   ...
   get_topog_stdev_2d_r8 = open_topog_file()

   if (get_topog_stdev_2d_r8) call interp_topog(blon, blat, &
              stdev, flag=COMPUTE_STDEV)
!-----------------------------------------------------------------------

 end function get_topog_stdev_2d_r8
...
!#######################################################################
 ...
 !> @returns A logical value of true if the output field was successfully created. A value of false
 !! may be returned if the Navy 1/6 degree percent water data set was not readable.
 function get_ocean_frac_1d_r8(blon, blat, ocean_frac)

 real(r8_kind), intent(in),  dimension(:)   :: blon !< Longitude (radians) at grid box boundaries
 real(r8_kind), intent(in),  dimension(:)   :: blat !< Latitude (radians) at grid box boundaries
 real(r8_kind), intent(out), dimension(:,:) :: ocean_frac !< The fractional amount (0-1) of ocean in a grid
                                     !! box. The size must be size(blon)-1 by size(blat)-1
 logical :: get_ocean_frac_1d_r8
 integer, parameter :: lkind = r8_kind
!-----------------------------------------------------------------------
   if (.not. module_is_initialized) call topography_init()
   ...
   get_ocean_frac_1d_r8 = open_water_file()
   if(get_ocean_frac_1d_r8)  call interp_water( blon, blat, &
                ocean_frac, do_ocean=.true. )

!-----------------------------------------------------------------------

 end function get_ocean_frac_1d_r8

!#######################################################################

 function get_ocean_frac_2d_r8(blon, blat, ocean_frac)

 real(kind=r8_kind), intent(in),  dimension(:,:) :: blon, blat
 real(kind=r8_kind), intent(out), dimension(:,:) :: ocean_frac
 logical :: get_ocean_frac_2d_r8
 integer, parameter :: lkind = r8_kind
!-----------------------------------------------------------------------
   if (.not. module_is_initialized) call topography_init()
   ...
   get_ocean_frac_2d_r8 = open_water_file()
   if(get_ocean_frac_2d_r8)  call interp_water( blon, blat, &
                ocean_frac, do_ocean=.true. )

!-----------------------------------------------------------------------

 end function get_ocean_frac_2d_r8
...
!#######################################################################
 ...
 !> @returns A logical value of true if the output field was successfully created. A value of false
 !! may be returned if the Navy 1/6 degree percent water data set was not readable.
 function get_ocean_mask_1d_r8(blon, blat, ocean_mask)

 real(kind=r8_kind), intent(in),  dimension(:)   :: blon !< Longitude (radians) at grid box boundaries
 real(kind=r8_kind), intent(in),  dimension(:)   :: blat !< Latitude (radians) at grid box boundaries
 logical, intent(out), dimension(:,:) :: ocean_mask !< Mask for ocean in a grid box.
                                                 !! The size must be size(blon)-1 by size(blat)-1
 logical :: get_ocean_mask_1d_r8
 real(kind=r8_kind), dimension(size(ocean_mask,1),size(ocean_mask,2)) :: ocean_frac
 integer, parameter :: lkind = r8_kind
!-----------------------------------------------------------------------
   if (.not. module_is_initialized) call topography_init()
   ...
   if (get_ocean_frac(blon, blat, ocean_frac) ) then
     ...
     get_ocean_mask_1d_r8 = .true.
   else
    get_ocean_mask_1d_r8 = .false.
   endif
!-----------------------------------------------------------------------

 end function get_ocean_mask_1d_r8

!#######################################################################

 function get_ocean_mask_2d_r8(blon, blat, ocean_mask)

 real(kind=r8_kind), intent(in),  dimension(:,:) :: blon, blat
 logical, intent(out), dimension(:,:) :: ocean_mask
 logical :: get_ocean_mask_2d_r8
 real(kind=r8_kind), dimension(size(ocean_mask,1),size(ocean_mask,2)) :: ocean_frac
 integer, parameter :: lkind = r8_kind
!-----------------------------------------------------------------------
   if (.not. module_is_initialized) call topography_init()
   ...
   if (get_ocean_frac(blon, blat, ocean_frac) ) then
     ...
     get_ocean_mask_2d_r8 = .true.
   else
     get_ocean_mask_2d_r8 = .false.
   endif

!-----------------------------------------------------------------------

 end function get_ocean_mask_2d_r8
 ...
 !! @throws FATAL, shape(water_frac) is not equal to (/size(blon)-1,size(blat)-1/)
 !! Check the input grid size and output field size.
 function get_water_frac_1d_r8(blon, blat, water_frac)
 real(kind=r8_kind), intent(in),  dimension(:)   :: blon !< The longitude (in radians) at grid box boundaries.
 real(kind=r8_kind), intent(in),  dimension(:)   :: blat !< The latitude (in radians) at grid box boundaries.
 real(kind=r8_kind), intent(out), dimension(:,:) :: water_frac !< The fractional amount (0 to 1) of water in a
                          !! grid box. The size of this field must be size(blon)-1 by size(blat)-1.
 logical :: get_water_frac_1d_r8
 integer, parameter :: lkind = r8_kind
...
!-----------------------------------------------------------------------
   ...
   get_water_frac_1d_r8 = open_water_file()
   if(get_water_frac_1d_r8) call interp_water( blon, blat, water_frac )

!-----------------------------------------------------------------------

 end function get_water_frac_1d_r8

!#######################################################################

 function get_water_frac_2d_r8(blon, blat, water_frac)

 real(kind=r8_kind), intent(in),  dimension(:,:)   :: blon !< The longitude (in radians) at grid box boundaries.
 real(kind=r8_kind), intent(in),  dimension(:,:)   :: blat !< The latitude (in radians) at grid box boundaries.
 real(kind=r8_kind), intent(out), dimension(:,:) :: water_frac !< The fractional amount (0 to 1) of water in a
                          !! grid box. The size of this field must be size(blon)-1 by size(blat)-1.
 logical :: get_water_frac_2d_r8
 integer, parameter :: lkind = r8_kind
...
!-----------------------------------------------------------------------
   ...
   get_water_frac_2d_r8 = open_water_file()
   if(get_water_frac_2d_r8) call interp_water( blon, blat, water_frac )

!-----------------------------------------------------------------------

 end function get_water_frac_2d_r8
...
!#######################################################################
 ...
 !! was successfully created. A value of FALSE may be returned
 !! if the Navy 1/6 degree percent water data set was not readable.
 function get_water_mask_1d_r8(blon, blat, water_mask)

 real(kind=r8_kind), intent(in),  dimension(:)   :: blon !< The longitude (in radians) at grid box boundaries.
 real(kind=r8_kind), intent(in),  dimension(:)   :: blat !< The latitude (in radians) at grid box boundaries.
 logical, intent(out), dimension(:,:) :: water_mask !< A binary mask for water (true) or land (false).
                                     !! The size of this field must be size(blon)-1 by size(blat)-1.
 logical :: get_water_mask_1d_r8

 real(kind=r8_kind), dimension(size(water_mask,1),size(water_mask,2)) :: water_frac
 integer, parameter :: lkind = r8_kind
!-----------------------------------------------------------------------
   if (.not. module_is_initialized) call topography_init()
   ...
   if (get_water_frac(blon, blat, water_frac) ) then
     ...
     get_water_mask_1d_r8 = .true.
   else
    get_water_mask_1d_r8 = .false.
   endif
!-----------------------------------------------------------------------

 end function get_water_mask_1d_r8

!#######################################################################

 function get_water_mask_2d_r8(blon, blat, water_mask)

 real(kind=r8_kind), intent(in),  dimension(:,:)   :: blon !< The longitude (in radians) at grid box boundaries.
 real(kind=r8_kind), intent(in),  dimension(:,:)   :: blat !< The latitude (in radians) at grid box boundaries.
 logical, intent(out), dimension(:,:) :: water_mask !< A binary mask for water (true) or land (false).
                                     !! The size of this field must be size(blon)-1 by size(blat)-1.
 logical :: get_water_mask_2d_r8
 real(kind=r8_kind), dimension(size(water_mask,1),size(water_mask,2)) :: water_frac
 integer, parameter :: lkind = r8_kind
!-----------------------------------------------------------------------
   if (.not. module_is_initialized) call topography_init()
   ...
   if (get_water_frac(blon, blat, water_frac) ) then
     ...
     get_water_mask_2d_r8 = .true.
   else
     get_water_mask_2d_r8 = .false.
   endif

!-----------------------------------------------------------------------

 end function get_water_mask_2d_r8

!#######################################################################

subroutine interp_topog_1d_r8( blon, blat, zout, flag)
real(kind=r8_kind), intent(in)  :: blon(:), blat(:)
real(kind=r8_kind), intent(out) :: zout(:,:)
integer, intent(in), optional :: flag

real(kind=r8_kind) :: xdat(ipts+1), ydat(jpts+1)
real(kind=r8_kind) :: zdat(ipts,jpts)
real(kind=r8_kind) :: zout2(size(zout,1),size(zout,2))
integer, parameter       :: lkind = r8_kind
...
end subroutine interp_topog_1d_r8

!#######################################################################

subroutine interp_topog_2d_r8( blon, blat, zout, flag )
real(kind=r8_kind), intent(in)  :: blon(:,:), blat(:,:)
real(kind=r8_kind), intent(out) :: zout(:,:)
integer, intent(in), optional :: flag
...
end subroutine interp_topog_2d_r8

!#######################################################################

subroutine find_indices_r8( ybeg, yend, ydat, js, je )
real(kind=r8_kind), intent(in)  :: ybeg, yend, ydat(:)
integer,                  intent(out) :: js, je
integer                               :: j
integer, parameter                    :: lkind = r8_kind
...
end subroutine find_indices_r8

!#######################################################################
subroutine input_data_r8( indx, xdat, ydat, zdat )
integer, intent(in) :: indx
real(kind=r8_kind), intent(out) :: xdat(ipts+1), ydat(jpts+1), zdat(ipts,jpts)
...
end subroutine input_data_r8

!#######################################################################

subroutine interp_water_1d_r8( blon, blat, zout, do_ocean )
real(kind=r8_kind), intent(in)  :: blon(:), blat(:)
real(kind=r8_kind), intent(out) :: zout(:,:)
logical, intent(in), optional :: do_ocean
real(kind=r8_kind) :: xdat(ipts+1), ydat(jpts+1), zdat(ipts,jpts)
    call input_data ( WATER_INDEX, xdat, ydat, zdat )
...
end subroutine interp_water_1d_r8

!#######################################################################

subroutine interp_water_2d_r8( blon, blat, zout, do_ocean )
real(kind=r8_kind), intent(in)  :: blon(:,:), blat(:,:)
real(kind=r8_kind), intent(out) :: zout(:,:)
logical, intent(in), optional :: do_ocean
real(kind=r8_kind) :: xdat(ipts+1), ydat(jpts+1), zdat(ipts,jpts)
...
end subroutine interp_water_2d_r8

!#######################################################################

subroutine determine_ocean_points_r8(pctwater)
real(kind=r8_kind), intent(inout) :: pctwater(:,:)
logical :: ocean(size(pctwater,1),size(pctwater,2))
integer :: i, j, m, n, im, ip, jm, jp, new
integer, parameter :: lkind = r8_kind
real(kind=r8_kind) :: ocean_pct_crit = 0.500_lkind
...
end subroutine determine_ocean_points_r8
# 79 "topography/include/topography_r8.fh" 2
# 356 "topography/topography.F90" 2
