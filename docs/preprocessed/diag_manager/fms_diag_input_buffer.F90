# 1 "diag_manager/fms_diag_input_buffer.F90"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "diag_manager/fms_diag_input_buffer.F90"
!***********************************************************************
...
# 1 "diag_manager/include/fms_diag_input_buffer_r4.fh" 1
!***********************************************************************
!*                             Apache License 2.0
!*
!* This file is part of the GFDL Flexible Modeling System (FMS).
...
# 1 "diag_manager/include/fms_diag_input_buffer.inc" 1
!***********************************************************************
!*                             Apache License 2.0
!*
!* This file is part of the GFDL Flexible Modeling System (FMS).
...
!> @brief Appends the input_data_buffer and the mask (only when the mask is set to .True.)
subroutine append_data_buffer_r4(mask_out, mask_in, data_out, data_in)
  logical,             intent(inout) :: mask_out(:,:,:,:) !< Mask currently in the input_data_buffer
  logical,             intent(in)    :: mask_in(:,:,:,:)  !< Mask passed in to send_data
  real(r4_kind), intent(inout) :: data_out(:,:,:,:) !< Data currently in the input_data_buffer
  real(r4_kind), intent(in)    :: data_in(:,:,:,:)  !< Data passed in to send_data
...
end subroutine append_data_buffer_r4

!> @brief Sums the data in the input_data_buffer
subroutine sum_data_buffer_r4(mask, data_out, data_in, counter, var_is_masked)
  logical,             intent(in)    :: mask(:,:,:,:)     !< Mask passed into send_data
  real(r4_kind), intent(inout) :: data_out(:,:,:,:) !< Data currently saved in the input_data_buffer
  real(r4_kind), intent(in)    :: data_in(:,:,:,:)  !< Data passed into send_data
  integer,             intent(inout) :: counter(:,:,:,:)  !< Number of times data has been summed
  logical,             intent(in)    :: var_is_masked     !< .True. if the variable is masked
  ...
  counter = counter + 1
end subroutine sum_data_buffer_r4
# 29 "diag_manager/include/fms_diag_input_buffer_r4.fh" 2
# 349 "diag_manager/fms_diag_input_buffer.F90" 2

# 1 "diag_manager/include/fms_diag_input_buffer_r8.fh" 1
!***********************************************************************
!*                             Apache License 2.0
!*
!* This file is part of the GFDL Flexible Modeling System (FMS).
...
!> @brief Appends the input_data_buffer and the mask (only when the mask is set to .True.)
subroutine append_data_buffer_r8(mask_out, mask_in, data_out, data_in)
  logical,             intent(inout) :: mask_out(:,:,:,:) !< Mask currently in the input_data_buffer
  logical,             intent(in)    :: mask_in(:,:,:,:)  !< Mask passed in to send_data
  real(r8_kind), intent(inout) :: data_out(:,:,:,:) !< Data currently in the input_data_buffer
  real(r8_kind), intent(in)    :: data_in(:,:,:,:)  !< Data passed in to send_data
...
end subroutine append_data_buffer_r8

!> @brief Sums the data in the input_data_buffer
subroutine sum_data_buffer_r8(mask, data_out, data_in, counter, var_is_masked)
  logical,             intent(in)    :: mask(:,:,:,:)     !< Mask passed into send_data
  real(r8_kind), intent(inout) :: data_out(:,:,:,:) !< Data currently saved in the input_data_buffer
  real(r8_kind), intent(in)    :: data_in(:,:,:,:)  !< Data passed into send_data
  integer,             intent(inout) :: counter(:,:,:,:)  !< Number of times data has been summed
  logical,             intent(in)    :: var_is_masked     !< .True. if the variable is masked
  ...
  counter = counter + 1
end subroutine sum_data_buffer_r8
# 29 "diag_manager/include/fms_diag_input_buffer_r8.fh" 2
# 350 "diag_manager/fms_diag_input_buffer.F90" 2
