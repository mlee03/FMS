...
# 1 "mosaic2/include/mosaic2_r4.fh" 1
!***********************************************************************
!*                             Apache License 2.0
!*
!* This file is part of the GFDL Flexible Modeling System (FMS).
...
# 1 "mosaic2/include/mosaic2.inc" 1
!***********************************************************************
!*                             Apache License 2.0
!*
!* This file is part of the GFDL Flexible Modeling System (FMS).
...
!!
  subroutine get_mosaic_xgrid_r4(fileobj, i1, j1, i2, j2, area, ibegin, iend)
    ...
    real(kind=r4_kind),  intent(inout) :: area(:) !> area of the exchange grid. The area is scaled to
                                                        !! represent unit earth area
    integer, optional, intent(in) :: ibegin, iend

    integer                 :: start(4), nread(4), istart
    real(kind=r4_kind),  dimension(2, size(i1(:))) :: tile1_cell, tile2_cell
    integer                 :: nxgrid, n
    ...
    call read_data(fileobj, 'tile2_cell', tile2_cell, corner=start, edge_lengths=nread)
     ...
     do n = 1, nxgrid
       ...
       area(n) = real( real(area(n),r8_kind)/garea, r4_kind )
    end do

    return

  end subroutine get_mosaic_xgrid_r4
  !###############################################################################
  ...
  subroutine calc_mosaic_grid_area_r4(lon, lat, area)
     real(kind=r4_kind), dimension(:,:), intent(in)  :: lon
     real(kind=r4_kind), dimension(:,:), intent(in)  :: lat
     real(kind=r4_kind), dimension(:,:), intent(out) :: area
     integer :: nlon, nlat
     ...
     area=real(area_r8,r4_kind)

  end subroutine calc_mosaic_grid_area_r4
  !###############################################################################
  ...
  subroutine calc_mosaic_grid_great_circle_area_r4(lon, lat, area)
     real(kind=r4_kind), dimension(:,:), intent(in)    :: lon
     real(kind=r4_kind), dimension(:,:), intent(in)    :: lat
     real(kind=r4_kind), dimension(:,:), intent(inout) :: area
     integer :: nlon, nlat
   ...
   end subroutine calc_mosaic_grid_great_circle_area_r4
  !#####################################################################
  !> This function check if a point (lon1,lat1) is inside a polygon (lon2(:), lat2(:))
  !! lon1, lat1, lon2, lat2 are in radians.
  function is_inside_polygon_r4(lon1, lat1, lon2, lat2 )
     real(kind=r4_kind), intent(in) :: lon1, lat1
     real(kind=r4_kind), intent(in) :: lon2(:), lat2(:)
     logical :: is_inside_polygon_r4
     ...
     if(isinside == 1) then
        is_inside_polygon_r4 = .TRUE.
     else
        is_inside_polygon_r4 = .FALSE.
     endif

     return

  end function is_inside_polygon_r4
# 35 "mosaic2/include/mosaic2_r4.fh" 2

# 449 "mosaic2/mosaic2.F90" 2

# 1 "mosaic2/include/mosaic2_r8.fh" 1
!***********************************************************************
!*                             Apache License 2.0
!*
!* This file is part of the GFDL Flexible Modeling System (FMS).
...
!!
  subroutine get_mosaic_xgrid_r8(fileobj, i1, j1, i2, j2, area, ibegin, iend)
    type(FmsNetcdfFile_t), intent(in) :: fileobj !> The file that contains exchange grid information.
    integer,       intent(inout) :: i1(:), j1(:), i2(:), j2(:) !> i and j indices for grids 1 and 2
    real(kind=r8_kind),  intent(inout) :: area(:) !> area of the exchange grid. The area is scaled to
                                                        !! represent unit earth area
    integer, optional, intent(in) :: ibegin, iend

    integer                 :: start(4), nread(4), istart
    real(kind=r8_kind),  dimension(2, size(i1(:))) :: tile1_cell, tile2_cell
    integer                 :: nxgrid, n
    real(kind=r8_kind) :: garea
    real(kind=r8_kind) :: get_global_area
    ...
    call read_data(fileobj, 'tile2_cell', tile2_cell, corner=start, edge_lengths=nread)
     ...
     do n = 1, nxgrid
       ...
       i2(n) = int(tile2_cell(1,n))
       j2(n) = int(tile2_cell(2,n))
       area(n) = real( real(area(n),r8_kind)/garea, r8_kind )
    end do
  ...
  subroutine calc_mosaic_grid_area_r8(lon, lat, area)
     real(kind=r8_kind), dimension(:,:), intent(in)  :: lon
     real(kind=r8_kind), dimension(:,:), intent(in)  :: lat
     real(kind=r8_kind), dimension(:,:), intent(out) :: area
     integer :: nlon, nlat
  ...
  subroutine calc_mosaic_grid_great_circle_area_r8(lon, lat, area)
     real(kind=r8_kind), dimension(:,:), intent(in)    :: lon
     real(kind=r8_kind), dimension(:,:), intent(in)    :: lat
     real(kind=r8_kind), dimension(:,:), intent(inout) :: area
     integer :: nlon, nlat
  ...
  !> This function check if a point (lon1,lat1) is inside a polygon (lon2(:), lat2(:))
  !! lon1, lat1, lon2, lat2 are in radians.
  function is_inside_polygon_r8(lon1, lat1, lon2, lat2 )
     real(kind=r8_kind), intent(in) :: lon1, lat1
     real(kind=r8_kind), intent(in) :: lon2(:), lat2(:)
     logical :: is_inside_polygon_r8
     integer :: npts, isinside
     integer :: inside_a_polygon
     ...
     isinside = inside_a_polygon(real(lon1,r8_kind), real(lat1,r8_kind), npts, real(lon2,r8_kind), real(lat2,r8_kind))
     if(isinside == 1) then
        is_inside_polygon_r8 = .TRUE.
     else
        is_inside_polygon_r8 = .FALSE.
     endif

     return

  end function is_inside_polygon_r8
# 35 "mosaic2/include/mosaic2_r8.fh" 2

# 450 "mosaic2/mosaic2.F90" 2
