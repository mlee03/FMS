# 1 "interpolator/interpolator.F90"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "interpolator/interpolator.F90"
!***********************************************************************
...
# 241 "interpolator/interpolator.F90" 2
...
# 1 "interpolator/include/interpolator_r4.fh" 1
...
# 1 "interpolator/include/interpolator.inc" 1
!***********************************************************************
!*                             Apache License 2.0
!*
!* This file is part of the GFDL Flexible Modeling System (FMS).
...
subroutine interpolator_init_r4( clim_type, file_name, lonb_mod, latb_mod, &
                              data_names, data_out_of_bounds,           &
                              vert_interp, clim_units, single_year_file)
type(interpolate_type), intent(inout)   :: clim_type
character(len=*), intent(in)            :: file_name
real(r4_kind), intent(in)        :: lonb_mod(:,:), latb_mod(:,:)
...
real(r4_kind), allocatable, save :: agrid_mod(:,:,:)
integer :: nx, ny
type(FmsNetcdfFile_t) :: fileobj

integer, parameter :: lkind=r4_kind
real(r4_kind), parameter :: lPI=real(PI,r4_kind)
...
clim_type%r4_type%is_allocated = .true.
...
allocate(clim_type%r4_type%lat(nlat))
call get_axis_latlon_data(fileobj, 'lat', clim_type%r4_type%lat)
...
allocate(clim_type%r4_type%lon(nlon))
call get_axis_latlon_data(fileobj, 'lon', clim_type%r4_type%lon)

!--- get clim_type%latb
if(dimension_exists(fileobj, "latb")) then
   call get_dimension_size(fileobj, "latb", nlatb)
   allocate(clim_type%r4_type%latb(nlatb))
   call get_axis_latlon_data(fileobj, 'latb', clim_type%r4_type%latb)
else
   ...
   allocate(clim_type%r4_type%latb(nlat+1))
   dlat = (clim_type%r4_type%lat(2)-clim_type%r4_type%lat(1)) * 0.5_lkind
!  clim_type%latb(1) = min( 90., max(-90., clim_type%lat(1) - dlat) )
   clim_type%r4_type%latb(1) = min( lPI/2._lkind, max(-lPI/2._lkind, clim_type%r4_type%lat(1) - dlat) )
   clim_type%r4_type%latb(2:nlat) = ( clim_type%r4_type%lat(1:nlat-1) + &
                                             clim_type%r4_type%lat(2:nlat) ) * 0.5_lkind
   dlat = ( clim_type%r4_type%lat(nlat) - clim_type%r4_type%lat(nlat-1) ) * 0.5_lkind
!  clim_type%latb(nlat+1) = min( 90., max(-90., clim_type%lat(nlat) + dlat) )
   clim_type%r4_type%latb(nlat+1) = min( lPI/2._lkind, max(-lPI/2._lkind, &
                                                clim_type%r4_type%lat(nlat) + dlat) )
endif

!--- get clim_type%lonb
if(dimension_exists(fileobj, "lonb")) then
   call get_dimension_size(fileobj, "lonb", nlonb)
   allocate(clim_type%r4_type%lonb(nlonb))
   call get_axis_latlon_data(fileobj, 'lonb', clim_type%r4_type%lonb)
else
...
! of the boundaries to be half-way between the midpoints.
  if (size(clim_type%r4_type%lon(:)) /= 1) then
    allocate(clim_type%r4_type%lonb(size(clim_type%r4_type%lon(:))+1))
    dlon = ( clim_type%r4_type%lon(2)-clim_type%r4_type%lon(1) )/2.0_lkind
    clim_type%r4_type%lonb(1)  = clim_type%r4_type%lon(1) - dlon
    clim_type%r4_type%lonb(2:) = clim_type%r4_type%lon(1:) + dlon
  else
!! this is the case for zonal mean data, lon = 1, lonb not present
!! in file.
    allocate(clim_type%r4_type%lonb(2))
    clim_type%r4_type%lonb(1) = -360.0_lkind*real(dtr,r4_kind)
    clim_type%r4_type%lonb(2) =  360.0_lkind*real(dtr,r4_kind)
    clim_type%r4_type%lon(1) = 0.0_lkind
  endif
endif
...
if(dimension_exists(fileobj, "pfull")) then
   ...
   allocate(clim_type%r4_type%levs(nlev))
   call get_axis_level_data(fileobj, 'pfull', clim_type%r4_type%levs, &
   ...
   allocate(clim_type%r4_type%levs(nlev))
   call fms2_io_read_data(fileobj, "sigma_full", clim_type%r4_type%levs)
   ...
   nlev = 1
   allocate( clim_type%r4_type%levs(nlev) )
   clim_type%r4_type%levs = 0.0_lkind
endif

!--- get clim_type%halflevs
if(dimension_exists(fileobj, "phalf")) then
   call get_dimension_size(fileobj, "phalf", nlevh)
   allocate(clim_type%r4_type%halflevs(nlevh))
   call get_axis_level_data(fileobj, 'phalf', clim_type%r4_type%halflevs, &
                            clim_type%level_type, clim_type%vertical_indices)
else if(dimension_exists(fileobj, "sigma_half")) then
   call get_dimension_size(fileobj, "sigma_half", nlevh)
   allocate(clim_type%r4_type%halflevs(nlevh))
   call fms2_io_read_data(fileobj, "sigma_half", clim_type%r4_type%halflevs)
   clim_type%level_type = SIGMA
else
   allocate( clim_type%r4_type%halflevs(nlev+1) )
   clim_type%r4_type%halflevs(1) = 0.0_lkind
   if (clim_type%level_type == PRESSURE) then
      clim_type%r4_type%halflevs(nlev+1) = 1013.25_lkind* 100.0_lkind   ! MKS
   else if (clim_type%level_type == SIGMA   ) then
      clim_type%r4_type%halflevs(nlev+1) = 1.0_lkind
   endif
      do n=2,nlev
         clim_type%r4_type%halflevs(n) = 0.5_lkind*(clim_type%r4_type%levs(n) + &
                                                           clim_type%r4_type%levs(n-1))
      end do
endif
...
!Assume that the horizontal interpolation within a file is the same for each variable.
 ...
 if (conservative_interp) then
    call horiz_interp_new (clim_type%interph, &
                           clim_type%r4_type%lonb, clim_type%r4_type%latb, &
                           lonb_mod, latb_mod)
 else
...
!   allocate(clim_type%data5d(size(lonb_mod(:))-1, size(latb_mod(:))-1, nlev, 2, num_fields))
   allocate(clim_type%r4_type%pmon_pyear(size(lonb_mod,1)-1, size(latb_mod,2)-1, nlev, num_fields))
   allocate(clim_type%r4_type%pmon_nyear(size(lonb_mod,1)-1, size(latb_mod,2)-1, nlev, num_fields))
   allocate(clim_type%r4_type%nmon_nyear(size(lonb_mod,1)-1, size(latb_mod,2)-1, nlev, num_fields))
   allocate(clim_type%r4_type%nmon_pyear(size(lonb_mod,1)-1, size(latb_mod,2)-1, nlev, num_fields))
   clim_type%r4_type%pmon_pyear = 0.0_lkind
   clim_type%r4_type%pmon_nyear = 0.0_lkind
   clim_type%r4_type%nmon_nyear = 0.0_lkind
   clim_type%r4_type%nmon_pyear = 0.0_lkind
...
! We have a continuous time-line so treat as for 5-12 timelevels as below.
   ...
   allocate(clim_type%r4_type%data5d(size(lonb_mod,1)-1, size(latb_mod,2)-1, nlev, 2, num_fields))
   else
   allocate(clim_type%r4_type%data5d(size(lonb_mod,1)-1, size(latb_mod,2)-1, nlev, &
            ntime, num_fields))
   endif
   clim_type%r4_type%data5d = 0.0_lkind
...
! case (default)
 case(:0)
   ...
   allocate(clim_type%r4_type%data5d(size(lonb_mod,1)-1, size(latb_mod,2)-1, nlev, 1, num_fields))
end select
   ...
   do i=1,num_fields
      do n = 1, ntime
         call read_data( clim_type, clim_type%field_name(i), &
                         clim_type%r4_type%data5d(:,:,:,n,i), n, i, base_time )
      enddo
   enddo
endif
...
! Read all the data at this point.
   do i=1,num_fields
     call read_data_no_time_axis( clim_type, clim_type%field_name(i), &
                                  clim_type%r4_type%data5d(:,:,:,1,i), i )
   enddo
   call close_file (fileobj)
endif
...
end subroutine interpolator_init_r4

subroutine get_axis_latlon_data_r4(fileobj, name, latlon_data)
   type(FmsNetcdfFile_t), intent(in) :: fileobj
   character(len=*),      intent(in) :: name
   real(r4_kind), dimension(:),   intent(out) :: latlon_data
   ...
   case('degree')
      latlon_data = latlon_data*real(DTR,r4_kind)
   case('radian')
   case default
 ...
 end subroutine get_axis_latlon_data_r4


subroutine get_axis_level_data_r4(fileobj, name, level_data, level_type, vertical_indices)
   type(FmsNetcdfFile_t), intent(in) :: fileobj
   character(len=*),      intent(in) :: name
   real(r4_kind), dimension(:),   intent(out) :: level_data
   integer,              intent(out) :: level_type, vertical_indices
   real(r4_kind), dimension(:), allocatable   :: alpha
   integer                           :: n, nlev
...
end subroutine get_axis_level_data_r4
...
!! @param [out] <e2> No description
 subroutine cell_center2_r4(q1, q2, q3, q4, e2)
      real(r4_kind) , intent(in ) :: q1(2), q2(2), q3(2), q4(2)
      real(r4_kind) , intent(out) :: e2(2)
! Local
      real(r4_kind) :: p1(3), p2(3), p3(3), p4(3)
      real(r4_kind) :: ec(3,1)
      real(r4_kind) :: dd
      integer k
 ...
 end subroutine cell_center2_r4
!
!---------------------------------------------------------------------
...
!! @param [inout] <ys> No description
 subroutine cart_to_latlon_r4(np, q, xs, ys)
! vector version of cart_to_latlon1
  integer, intent(in):: np
  real(r4_kind), intent(inout):: q(3,np)
  real(r4_kind), intent(inout):: xs(np), ys(np)
! local
  ...
  do i=1,np
     ...
     xs(i) = real(lon,r4_kind)
     ys(i) = real(lat,r4_kind)
! q Normalized:
     do k=1,3
        q(k,i) = real(p(k),r4_kind)
     enddo
  enddo

 end  subroutine cart_to_latlon_r4
!
!---------------------------------------------------------------------
...
!! @param [out] <e> No description
 subroutine latlon2xyz_r4(p, e)
!
! Routine to map (lon, lat) to (x,y,z)
!
 real(r4_kind), intent(in) :: p(2)
 real(r4_kind), intent(out):: e(3)
...
!! @throw FATAL, "init_clim_diag : You must call interpolator_init before calling init_clim_diag"
!! @throw FATAL, "init_clim_diag : Trying to set up too many diagnostic fields for the climatology data"
subroutine init_clim_diag_r4(clim_type, mod_axes, init_time)
!
! Routine to register diagnostic fields for the climatology file.
! This routine calculates the domain decompostion of the climatology fields
! for later export through send_data.
...
if (.not. module_is_initialized .or. .not. allocated(clim_type%r4_type%lon)) &
   call mpp_error(FATAL, "init_clim_diag : You must call interpolator_init before calling init_clim_diag")


ndivs = mpp_npes()
nxd = size(clim_type%r4_type%lon(:))
nyd = size(clim_type%r4_type%lat(:))
...
axes(1) = diag_axis_init(clim_type%file_name(1:5)//'x',clim_type%r4_type%lon,units='degrees',&
                         cart_name='x',domain2=domain)
axes(2) = diag_axis_init(clim_type%file_name(1:5)//'y',clim_type%r4_type%lat,units='degrees',&
...
end subroutine init_clim_diag_r4
...
!!                    climatology time but we have the next time. How did
!!                    this happen?"
subroutine obtain_interpolator_time_slices_r4 (clim_type, Time)
...
integer, parameter :: lkind=r4_kind
    ...
    if (clim_type%climatological_year) then
       ...
       if (size(clim_type%time_slice) > 1) then
          call time_interp(Time, clim_type%time_slice, clim_type%r4_type%tweight, &
          ...
          taum = 1
          taup = 1
          clim_type%r4_type%tweight = 0._lkind
       end if
       !--lwh
    else
       call time_interp(Time, clim_type%time_slice, clim_type%r4_type%tweight, taum, taup, err_msg=err_msg )
       if(trim(err_msg) /= '') then
...
!RSHBUGFX   if ( modday <= day ) then
        ...
        call time_interp(Time, month, clim_type%r4_type%tweight3, taum, taup, err_msg=err_msg ) ! tweight3 is
                                                                                 !! the time weight between the months.
        !> protect against post-perth time_interp behavior below
        if (taum==2 .and. taup==2) clim_type%r4_type%tweight3 = 1._lkind
        if(trim(err_msg) /= '') then
        ...
        call time_interp(t_prev, month, clim_type%r4_type%tweight1, taum, taup, err_msg=err_msg ) !tweight1 is
                                                                      !! the time weight between the climatology years.
        !>  protect against post-perth time_interp behavior below
        if (taum==2 .and. taup==2) clim_type%r4_type%tweight1 = 1._lkind
        if(trim(err_msg) /= '') then
        ...
        call time_interp(t_next, month, clim_type%r4_type%tweight2, taum, taup, err_msg=err_msg ) !tweight1 is
                                                                      !! the time weight between the climatology years.
        !> protect against post-perth time_interp behavior below
        if (taum==2 .and. taup==2) clim_type%r4_type%tweight2 = 1._lkind
        if(trim(err_msg) /= '') then
...
! Read the data for the next month in the previous climatology.
          ...
          do i=1, size(clim_type%field_name(:))
             if (taum /= clim_type%time_init(i,1) .or. &
              ...
              call read_data(clim_type,clim_type%field_name(i),   &
                           clim_type%r4_type%pmon_pyear(:,:,:,i), taum,i,Time)
! Read the data for the next month in the previous climatology.
              call read_data(clim_type,clim_type%field_name(i),   &
                           clim_type%r4_type%nmon_pyear(:,:,:,i), taup,i,Time)
...
!       clim_type%tweight3 = 0.0 ! This makes [pn]mon_nyear irrelevant. Set them to 0 to test.
        clim_type%r4_type%tweight1 = 0.0_lkind
        clim_type%r4_type%tweight2 = 0.0_lkind
        clim_type%r4_type%tweight3 = clim_type%r4_type%tweight
      endif
    endif   !(BILINEAR)
...
! field(:,:,:,2) as the next time slice.
      do i=1, size(clim_type%field_name(:))
          call read_data(clim_type,clim_type%field_name(i), clim_type%r4_type%data5d(:,:,:,1,i), taum,i,Time)
               clim_type%time_init(i,1) = taum
               clim_type%itaum = 1
          call read_data(clim_type,clim_type%field_name(i), clim_type%r4_type%data5d(:,:,:,2,i), taup,i,Time)
               clim_type%time_init(i,2) = taup
               clim_type%itaup = 2
      end do
...
!We have the previous time step but not the next time step data
        ...
        do i=1, size(clim_type%field_name(:))
           call read_data(clim_type,clim_type%field_name(i), &
                          clim_type%r4_type%data5d(:,:,:,clim_type%itaup,i), taup,i, Time)
...
!-------------------------------------------------------------------
 ...
 end subroutine obtain_interpolator_time_slices_r4
...
!! @param [in] <dim_order> OPTIONAL: Order of the x (1), y (2), z (3), and field (4) dimensions
!!
...
!!                    intepolate_type: "
subroutine interpolator_4d_r4(clim_type, Time, phalf, interp_data,  &
                           field_name, is, js, clim_units, dim_order)
!
...
!   dim_order   : Order of the x (1), y (2), z (3), and field (4) dimensions
!
! INTENT OUT
...
type(time_type)       , intent(in)  :: Time
real(r4_kind), dimension(:,:,:), intent(in)  :: phalf
real(r4_kind), dimension(:,:,:,:), intent(out), target :: interp_data
integer               , intent(in) , optional :: is,js
character(len=*)      , intent(out), optional :: clim_units
integer               , intent(in), optional :: dim_order(4)
integer :: taum, taup, ilon
real(r4_kind), allocatable :: hinterp_data(:,:,:,:), p_fact(:,:), col_data(:,:,:), phalf_diff(:,:,:)
real(r4_kind), pointer :: interp_buf(:,:,:,:)
real(r4_kind) :: pclim(size(clim_type%r4_type%halflevs(:)))
...
nlevs = size(clim_type%r4_type%levs)
...
select case(clim_type%TIME_FLAG)
  case (LINEAR)
    do n=1, nfields
      hinterp_data(:,:,:,n) = (1._lkind-clim_type%r4_type%tweight)*  &
                clim_type%r4_type%data5d(istart:iend,jstart:jend,:,clim_type%itaum,n)  +  &
                                 clim_type%r4_type%tweight*   &
                clim_type%r4_type%data5d(istart:iend,jstart:jend,:,clim_type%itaup,n)
    end do
! case (SEASONAL)
! Do sine fit to data at this point
  case (BILINEAR)
    do n=1, nfields
      hinterp_data(:,:,:,n) = (1._lkind-clim_type%r4_type%tweight1)*&
                              (1._lkind-clim_type%r4_type%tweight3)*   &
                              clim_type%r4_type%pmon_pyear(istart:iend,jstart:jend,:,n) + &
                              (1._lkind-clim_type%r4_type%tweight2)*clim_type%r4_type%tweight3* &
                              clim_type%r4_type%nmon_pyear(istart:iend,jstart:jend,:,n) + &
                              clim_type%r4_type%tweight1* (1._lkind-clim_type%r4_type%tweight3)*&
                              clim_type%r4_type%pmon_nyear(istart:iend,jstart:jend,:,n) + &
                              clim_type%r4_type%tweight2* clim_type%r4_type%tweight3*   &
                              clim_type%r4_type%nmon_nyear(istart:iend,jstart:jend,:,n)
...
select case(clim_type%mr(i))
  case(NO_CONV)
   ...
   col_data(:,:,i) = col_data(:,:,i) + hinterp_data(:,:,k,i)* &
      (clim_type%r4_type%halflevs(k+1)-clim_type%r4_type%halflevs(k))/real(grav,r4_kind)
    enddo
  ...
  case(KG_M2)
    do k = 1,size(hinterp_data,3)
       ...
       hinterp_data(:,:,k,i) = hinterp_data(:,:,k,i)/ &
         ((clim_type%r4_type%halflevs(k+1)-clim_type%r4_type%halflevs(k))*p_fact)
    enddo
end select
    enddo
...
do j = 1, size(phalf,2)
   do ilon=1,size(phalf,1)
      pclim = p_fact(ilon,j)*clim_type%r4_type%halflevs
...
!              pclim( maxloc(pclim)) = 0
      ...
      select case(clim_type%vert_interp(i))
         case(INTERP_WEIGHTED_P)
            call interp_weighted_scalar(pclim, phalf(ilon,j,:),hinterp_data(ilon,j,:,:),interp_buf(ilon,j,:,:))
         case(INTERP_LINEAR_P)
          do n=1, nfields
            call interp_linear(pclim, phalf(ilon,j,:),hinterp_data(ilon,j,:,n),interp_buf(ilon,j,:,n))
          end do
!        case(INTERP_LOG)
      end select
   enddo
enddo
     ...
     phalf_diff = phalf(:,:,2:nk+1) - phalf(:,:,1:nk)

!--lwh
     do i= 1, nfields
       select case(clim_type%mr(i))
         case(KG_M2)
           interp_buf(:,:,:,i) = interp_buf(:,:,:,i)*phalf_diff
       end select
     end do
...
end subroutine interpolator_4d_r4
!
!#######################################################################
!#######################################################################
!
...
!! @param [out] <interp_data> The model fields with the interpolated climatology data
!! @param [out] <clim_units> OPTIONAL: The units of field_name
!! @param [in] <dim_order> OPTIONAL: Order of the x (1), y (2), and z (3) dimensions
!!
...
!! @throw FATAL "Interpolator: the field name is not contained in this
!!                    intepolate_type: "
subroutine interpolator_3d_r4(clim_type, Time, phalf, interp_data, field_name, is, js, clim_units, dim_order)
!
...
!   phalf       : The half level model pressure field.
!   is, js      : The indices of the physics window.
!   dim_order   : Order of the x (1), y (2), and z (3) dimensions
!
! INTENT OUT
...
!RSHBUGFX   if ( modday <= day ) then
        ...
        if (taum==2 .and. taup==2) &
             clim_type%r4_type%tweight3 = 1._lkind ! protect against post-perth time_interp behavior
        if(trim(err_msg) /= '') then
        ...
        call time_interp(t_prev, month, clim_type%r4_type%tweight1, taum, taup, err_msg=err_msg ) !tweight1 is
                                                                      !! the time weight between the climatology years.
        if (taum==2 .and. taup==2) &
             clim_type%r4_type%tweight1 = 1._lkind ! protect against post-perth time_interp behavior
        if(trim(err_msg) /= '') then
        ...
        call time_interp(t_next, month, clim_type%r4_type%tweight2, taum, taup, err_msg=err_msg ) !tweight1 is
                                                                      !! the time weight between the climatology years.
        if (taum==2 .and. taup==2) &
             clim_type%r4_type%tweight2 = 1._lkind ! protect against post-perth time_interp behavior
        if(trim(err_msg) /= '') then
...
! Read the data for the next month in the previous climatology.
        ...
        if (taum /= clim_type%time_init(i,1) .or. &
          ...
          call read_data(clim_type,clim_type%field_name(i), clim_type%r4_type%pmon_pyear(:,:,:,i), taum,i,Time)
! Read the data for the next month in the previous climatology.
          call read_data(clim_type,clim_type%field_name(i), clim_type%r4_type%nmon_pyear(:,:,:,i), taup,i,Time)
...
!       clim_type%tweight3 = 0.0 ! This makes [pn]mon_nyear irrelevant. Set them to 0 to test.
        clim_type%r4_type%tweight1 = 0.0_lkind ; clim_type%r4_type%tweight2 = 0.0_lkind
        clim_type%r4_type%tweight3 = clim_type%r4_type%tweight
      endif
...
select case(clim_type%TIME_FLAG)
  case (LINEAR)
    hinterp_data = (1._lkind-clim_type%r4_type%tweight) * &
                    clim_type%r4_type%data5d(istart:iend,jstart:jend,:,clim_type%itaum,i) + &
                    clim_type%r4_type%tweight * &
                    clim_type%r4_type%data5d(istart:iend,jstart:jend,:,clim_type%itaup,i)
! case (SEASONAL)
! Do sine fit to data at this point
  case (BILINEAR)
    hinterp_data = &
    (1._lkind-clim_type%r4_type%tweight1)*&
    (1._lkind-clim_type%r4_type%tweight3)*clim_type%r4_type%pmon_pyear(istart:iend,jstart:jend,:,i) + &
    (1._lkind-clim_type%r4_type%tweight2) * clim_type%r4_type%tweight3  &
    * clim_type%r4_type%nmon_pyear(istart:iend,jstart:jend,:,i) + &
    clim_type%r4_type%tweight1 * (1._lkind-clim_type%r4_type%tweight3) * &
    clim_type%r4_type%pmon_nyear(istart:iend,jstart:jend,:,i) + &
    clim_type%r4_type%tweight2 * clim_type%r4_type%tweight3 * &
    clim_type%r4_type%nmon_nyear(istart:iend,jstart:jend,:,i)
...
!              pclim( maxloc(pclim)) = 0
      ...
      select case(clim_type%vert_interp(i))
         case(INTERP_WEIGHTED_P)
            call interp_weighted_scalar(pclim, phalf(ilon,j,:),hinterp_data(ilon,j,:),interp_buf(ilon,j,:))
         case(INTERP_LINEAR_P)
            call interp_linear(pclim, phalf(ilon,j,:),hinterp_data(ilon,j,:),interp_buf(ilon,j,:))
!        case(INTERP_LOG)
      end select
   enddo
enddo
...
end subroutine interpolator_3d_r4
!
!#######################################################################
!
!++lwh
...
!! @param [out] <interp_data> The model fields with the interpolated climatology data
!! @param [out] <clim_units> OPTIONAL: The units of field_name
!! @param [in] <dim_order> OPTIONAL: Order of the x (1) and y (2) dimensions
!!
...
!! @throw FATAL "Interpolator: the field name is not contained in this
!!                    intepolate_type: "
subroutine interpolator_2d_r4(clim_type, Time, interp_data, field_name, is, js, clim_units, dim_order)
!
...
!   Time        : The model time that you wish to interpolate to.
!   is, js      : The indices of the physics window.
!   dim_order   : Order of the x (1) and y (2) dimensions
!
! INTENT OUT
!   interp_data : The model field with the interpolated climatology data.
!   clim_units  : The units of field_name
...
integer :: taum, taup
real(r4_kind), allocatable :: hinterp_data(:,:,:)
integer :: istart,iend,jstart,jend
logical :: result, found
logical :: found_field=.false.
integer :: modyear, modmonth, modday, modhour, modminute, modsecond
...
select case(clim_type%TIME_FLAG)
  case (LINEAR)
    hinterp_data = (1._lkind-clim_type%r4_type%tweight)*&
         clim_type%r4_type%data5d(istart:iend,jstart:jend,:,clim_type%itaum,i) &
         + clim_type%r4_type%tweight*clim_type%r4_type%data5d &
...
! Do sine fit to data at this point
  case (BILINEAR)
    ...
    (1._lkind-clim_type%r4_type%tweight1) * &
    (1._lkind-clim_type%r4_type%tweight3) * &
    clim_type%r4_type%pmon_pyear(istart:iend,jstart:jend,:,i) + &
    (1._lkind-clim_type%r4_type%tweight2) * &
    clim_type%r4_type%tweight3  * clim_type%r4_type%nmon_pyear(istart:iend,jstart:jend,:,i) + &
    clim_type%r4_type%tweight1 * (1._lkind-clim_type%r4_type%tweight3) * &
    clim_type%r4_type%pmon_nyear(istart:iend,jstart:jend,:,i) + &
    clim_type%r4_type%tweight2 * clim_type%r4_type%tweight3 * &
    clim_type%r4_type%nmon_nyear(istart:iend,jstart:jend,:,i)
...
if (found) then
  ...
  interp_data = reshape(hinterp_data(:,:,1), shape(interp_data), order=dim_map)
...
end subroutine interpolator_2d_r4
!--lwh
!
!#######################################################################
!
...
!! @throw FATAL "Interpolator: the field name is not contained in this
!!                    intepolate_type: "
subroutine interpolator_4d_no_time_axis_r4(clim_type, phalf, interp_data, field_name, is, js, clim_units, dim_order)
...
integer               , intent(in), optional :: dim_order(4)
integer :: ilon
real(r4_kind), allocatable :: hinterp_data(:,:,:,:), p_fact(:,:), phalf_diff(:,:,:)
real(r4_kind), pointer :: interp_buf(:,:,:,:)
real(r4_kind) :: pclim(size(clim_type%r4_type%halflevs(:)))
integer :: istart,iend,jstart,jend
logical :: found_field=.false.
...
do n=1, nfields
  hinterp_data(:,:,:,n) = clim_type%r4_type%data5d(istart:iend,jstart:jend,:,1,n)
end do
...
end subroutine interpolator_4d_no_time_axis_r4
...
!! @throw FATAL "Interpolator: the field name is not contained in this
!!                    intepolate_type: "
subroutine interpolator_3d_no_time_axis_r4(clim_type, phalf, interp_data, field_name, is, js, clim_units, dim_order)
...
real(r4_kind), allocatable :: hinterp_data(:,:,:), p_fact(:,:), phalf_diff(:,:,:)
real(r4_kind), pointer :: interp_buf(:,:,:)
real(r4_kind) :: pclim(size(clim_type%r4_type%halflevs(:))) !< No description
...
do i= 1,nfields
  if ( trim(adjustl(lowercase(field_name))) == trim(adjustl(lowercase(clim_type%field_name(i)))) ) then
    ...
    hinterp_data = clim_type%r4_type%data5d(istart:iend,jstart:jend,:,1,i)
...
end subroutine interpolator_3d_no_time_axis_r4
...
!! @throw FATAL "Interpolator: the field name is not contained in this
!!                    intepolate_type: "
subroutine interpolator_2d_no_time_axis_r4(clim_type, interp_data, field_name, is, js, clim_units, dim_order)
...
do i= 1,nfields
  if ( trim(adjustl(lowercase(field_name))) == trim(adjustl(lowercase(clim_type%field_name(i)))) ) then
    ...
    interp_data = reshape(clim_type%r4_type%data5d(istart:iend,jstart:jend,1,1,i), &
...
end subroutine interpolator_2d_no_time_axis_r4
...
subroutine read_data_r4(clim_type,field_name, hdata, nt,i, Time)
!
!  INTENT IN
...
real(r4_kind)     , intent(out) :: hdata(:,:,:)
integer                  , intent(in)  :: i
type(time_type), optional, intent(in)  :: Time

integer   :: k, km
! sjs
real(r4_kind), allocatable :: climdata(:,:,:), climdata2(:,:,:)

      allocate(climdata(size(clim_type%r4_type%lon(:)),size(clim_type%r4_type%lat(:)), &
                        size(clim_type%r4_type%levs(:))))
...
!  level data. the indices themselves were previously flipped.
      if (clim_type%vertical_indices == INCREASING_UPWARD) then
        allocate(climdata2(size(clim_type%r4_type%lon(:)),   &
                           size(clim_type%r4_type%lat(:)), &
                           size(clim_type%r4_type%levs(:))))
        km = size(clim_type%r4_type%levs(:))
        do k=1, km
...
!! @param [out] <hdata> The horizontally interpolated climatology field. This
!                       field will still be on the climatology vertical grid
subroutine read_data_no_time_axis_r4(clim_type,field_name, hdata, i)
...
real(r4_kind), allocatable :: climdata(:,:,:), climdata2(:,:,:)
      ...
      allocate(climdata(size(clim_type%r4_type%lon(:)),&
                        size(clim_type%r4_type%lat(:)),&
                        size(clim_type%r4_type%levs(:))))
...
end subroutine read_data_no_time_axis_r4
...
subroutine diag_read_data_r4(clim_type,model_data, i, Time)
!
...
!
type(interpolate_type), intent(in) :: clim_type
real(r4_kind)  , intent(in) :: model_data(:,:,:)
integer               , intent(in) :: i
type(time_type)       , intent(in) :: Time

integer :: j,k
real(r4_kind) :: col_data(size(model_data,1),size(model_data,2))
logical :: result, found
...
end subroutine diag_read_data_r4
!
!#################################################################
!
!
...
subroutine interp_weighted_scalar_2d_r4 (grdin, grdout, datin, datout )
real(r4_kind), intent(in),  dimension(:) :: grdin, grdout
real(r4_kind), intent(in),  dimension(:,:) :: datin
real(r4_kind), intent(out), dimension(:,:) :: datout
...
end subroutine interp_weighted_scalar_2d_r4
...
!! @param [in] <datin> No description
!! @param [out] <datout> No description
subroutine interp_weighted_scalar_1d_r4 (grdin, grdout, datin, datout )
real(r4_kind), intent(in),  dimension(:) :: grdin, grdout, datin
real(r4_kind), intent(out), dimension(:) :: datout
...
end subroutine interp_weighted_scalar_1d_r4
!
!#################################################################
!
!---------------------------------------------------------------------
...
!! @param [in] <datin> No description
!! @param [out] <datout> No description
subroutine interp_linear_r4 ( grdin, grdout, datin, datout )
real(r4_kind), intent(in),  dimension(:) :: grdin, grdout, datin
real(r4_kind), intent(out), dimension(:) :: datout
...
!
!#######################################################################
# 68 "interpolator/include/interpolator_r4.fh" 2
# 769 "interpolator/interpolator.F90" 2

# 1 "interpolator/include/interpolator_r8.fh" 1
...
!! @param [out] <clim_units> OPTIONAL: A list of the units for the components listed in data_names
!! @param [out] <single_year_file> OPTIONAL: No description
subroutine interpolator_init_r8( clim_type, file_name, lonb_mod, latb_mod, &
                              data_names, data_out_of_bounds,           &
                              vert_interp, clim_units, single_year_file)
type(interpolate_type), intent(inout)   :: clim_type
character(len=*), intent(in)            :: file_name
real(r8_kind), intent(in)        :: lonb_mod(:,:), latb_mod(:,:)
character(len=*), intent(in) , optional :: data_names(:)
!++lwh
integer         , intent(in)            :: data_out_of_bounds(:)
integer         , intent(in), optional  :: vert_interp(:)
...
type(time_type) :: Julian_time, Noleap_time
real(r8_kind), allocatable :: time_in(:)
real(r8_kind), allocatable, save :: agrid_mod(:,:,:)
integer :: nx, ny
type(FmsNetcdfFile_t) :: fileobj

integer, parameter :: lkind=r8_kind
real(r8_kind), parameter :: lPI=real(PI,r8_kind)
...
clim_type%r8_type%is_allocated = .true.
...
allocate(clim_type%r8_type%lat(nlat))
call get_axis_latlon_data(fileobj, 'lat', clim_type%r8_type%lat)
...
allocate(clim_type%r8_type%lon(nlon))
call get_axis_latlon_data(fileobj, 'lon', clim_type%r8_type%lon)

!--- get clim_type%latb
if(dimension_exists(fileobj, "latb")) then
   call get_dimension_size(fileobj, "latb", nlatb)
   allocate(clim_type%r8_type%latb(nlatb))
   call get_axis_latlon_data(fileobj, 'latb', clim_type%r8_type%latb)
else
   if(nlat == 1) call mpp_error(FATAL,'Interpolator_init : nlat is 1')
   ! In the case where only the grid midpoints of the latitudes are defined we force the
   ! definition of the boundaries to be half-way between the midpoints.
   allocate(clim_type%r8_type%latb(nlat+1))
   dlat = (clim_type%r8_type%lat(2)-clim_type%r8_type%lat(1)) * 0.5_lkind
!  clim_type%latb(1) = min( 90., max(-90., clim_type%lat(1) - dlat) )
   clim_type%r8_type%latb(1) = min( lPI/2._lkind, max(-lPI/2._lkind, clim_type%r8_type%lat(1) - dlat) )
   clim_type%r8_type%latb(2:nlat) = ( clim_type%r8_type%lat(1:nlat-1) + &
                                             clim_type%r8_type%lat(2:nlat) ) * 0.5_lkind
   dlat = ( clim_type%r8_type%lat(nlat) - clim_type%r8_type%lat(nlat-1) ) * 0.5_lkind
!  clim_type%latb(nlat+1) = min( 90., max(-90., clim_type%lat(nlat) + dlat) )
   clim_type%r8_type%latb(nlat+1) = min( lPI/2._lkind, max(-lPI/2._lkind, &
                                                clim_type%r8_type%lat(nlat) + dlat) )
endif

!--- get clim_type%lonb
if(dimension_exists(fileobj, "lonb")) then
   call get_dimension_size(fileobj, "lonb", nlonb)
   allocate(clim_type%r8_type%lonb(nlonb))
   call get_axis_latlon_data(fileobj, 'lonb', clim_type%r8_type%lonb)
else
! In the case where only the midpoints of the longitudes are defined we force the definition
! of the boundaries to be half-way between the midpoints.
  if (size(clim_type%r8_type%lon(:)) /= 1) then
    allocate(clim_type%r8_type%lonb(size(clim_type%r8_type%lon(:))+1))
    dlon = ( clim_type%r8_type%lon(2)-clim_type%r8_type%lon(1) )/2.0_lkind
    clim_type%r8_type%lonb(1)  = clim_type%r8_type%lon(1) - dlon
    clim_type%r8_type%lonb(2:) = clim_type%r8_type%lon(1:) + dlon
  else
!! this is the case for zonal mean data, lon = 1, lonb not present
!! in file.
    allocate(clim_type%r8_type%lonb(2))
    clim_type%r8_type%lonb(1) = -360.0_lkind*real(dtr,r8_kind)
    clim_type%r8_type%lonb(2) =  360.0_lkind*real(dtr,r8_kind)
    clim_type%r8_type%lon(1) = 0.0_lkind
  endif
endif
...
if(dimension_exists(fileobj, "pfull")) then
   call get_dimension_size(fileobj, "pfull", nlev)
   allocate(clim_type%r8_type%levs(nlev))
   call get_axis_level_data(fileobj, 'pfull', clim_type%r8_type%levs, &
                            clim_type%level_type, clim_type%vertical_indices)
else if(dimension_exists(fileobj, "sigma_full")) then
   call get_dimension_size(fileobj, "sigma_full", nlev)
   allocate(clim_type%r8_type%levs(nlev))
   call fms2_io_read_data(fileobj, "sigma_full", clim_type%r8_type%levs)
   clim_type%level_type = SIGMA
else
   ! -------------------------------------------------------------------
   ! For 2-D fields, allocate levs and halflevs here
   ...
   nlev = 1
   allocate( clim_type%r8_type%levs(nlev) )
   clim_type%r8_type%levs = 0.0_lkind
endif

!--- get clim_type%halflevs
if(dimension_exists(fileobj, "phalf")) then
   call get_dimension_size(fileobj, "phalf", nlevh)
   allocate(clim_type%r8_type%halflevs(nlevh))
   call get_axis_level_data(fileobj, 'phalf', clim_type%r8_type%halflevs, &
                            clim_type%level_type, clim_type%vertical_indices)
else if(dimension_exists(fileobj, "sigma_half")) then
   call get_dimension_size(fileobj, "sigma_half", nlevh)
   allocate(clim_type%r8_type%halflevs(nlevh))
   call fms2_io_read_data(fileobj, "sigma_half", clim_type%r8_type%halflevs)
   clim_type%level_type = SIGMA
else
   allocate( clim_type%r8_type%halflevs(nlev+1) )
   clim_type%r8_type%halflevs(1) = 0.0_lkind
   if (clim_type%level_type == PRESSURE) then
      clim_type%r8_type%halflevs(nlev+1) = 1013.25_lkind* 100.0_lkind   ! MKS
   else if (clim_type%level_type == SIGMA   ) then
      clim_type%r8_type%halflevs(nlev+1) = 1.0_lkind
   endif
      do n=2,nlev
         clim_type%r8_type%halflevs(n) = 0.5_lkind*(clim_type%r8_type%levs(n) + &
                                                           clim_type%r8_type%levs(n-1))
      end do
endif
...
!Assume that the horizontal interpolation within a file is the same for each variable.
 ...
 if (conservative_interp) then
    call horiz_interp_new (clim_type%interph, &
                           clim_type%r8_type%lonb, clim_type%r8_type%latb, &
                           lonb_mod, latb_mod)
 else
...
! We have a broken time-line. e.g. We have monthly data but only for years ending in 0. 1960,1970 etc.
!   allocate(clim_type%data5d(size(lonb_mod(:))-1, size(latb_mod(:))-1, nlev, 2, num_fields))
   allocate(clim_type%r8_type%pmon_pyear(size(lonb_mod,1)-1, size(latb_mod,2)-1, nlev, num_fields))
   allocate(clim_type%r8_type%pmon_nyear(size(lonb_mod,1)-1, size(latb_mod,2)-1, nlev, num_fields))
   allocate(clim_type%r8_type%nmon_nyear(size(lonb_mod,1)-1, size(latb_mod,2)-1, nlev, num_fields))
   allocate(clim_type%r8_type%nmon_pyear(size(lonb_mod,1)-1, size(latb_mod,2)-1, nlev, num_fields))
   clim_type%r8_type%pmon_pyear = 0.0_lkind
   clim_type%r8_type%pmon_nyear = 0.0_lkind
   clim_type%r8_type%nmon_nyear = 0.0_lkind
   clim_type%r8_type%nmon_pyear = 0.0_lkind
   clim_type%TIME_FLAG = BILINEAR
else
! We have a continuous time-line so treat as for 5-12 timelevels as below.
   if ( .not. read_all_on_init) then
   allocate(clim_type%r8_type%data5d(size(lonb_mod,1)-1, size(latb_mod,2)-1, nlev, 2, num_fields))
   else
   allocate(clim_type%r8_type%data5d(size(lonb_mod,1)-1, size(latb_mod,2)-1, nlev, &
            ntime, num_fields))
   endif
   clim_type%r8_type%data5d = 0.0_lkind
   clim_type%TIME_FLAG = LINEAR
endif
...
! case (default)
 case(:0)
   clim_type%TIME_FLAG = NOTIME
   allocate(clim_type%r8_type%data5d(size(lonb_mod,1)-1, size(latb_mod,2)-1, nlev, 1, num_fields))
end select
...
! Read all the data at this point.
   do i=1,num_fields
      do n = 1, ntime
         call read_data( clim_type, clim_type%field_name(i), &
                         clim_type%r8_type%data5d(:,:,:,n,i), n, i, base_time )
      enddo
   enddo
endif
...
! Read all the data at this point.
   do i=1,num_fields
     call read_data_no_time_axis( clim_type, clim_type%field_name(i), &
                                  clim_type%r8_type%data5d(:,:,:,1,i), i )
   enddo
   call close_file (fileobj)
endif
...
end subroutine interpolator_init_r8

subroutine get_axis_latlon_data_r8(fileobj, name, latlon_data)
   type(FmsNetcdfFile_t), intent(in) :: fileobj
   character(len=*),      intent(in) :: name
   real(r8_kind), dimension(:),   intent(out) :: latlon_data
   ...
   select case(units(1:6))
   case('degree')
      latlon_data = latlon_data*real(DTR,r8_kind)
   case('radian')
   case default
      call mpp_error(FATAL, "get_axis_latlon_data : Units for '// &
                     & trim(name)//' not recognised in file "//trim(fileobj%path))
 ...
 end subroutine get_axis_latlon_data_r8


subroutine get_axis_level_data_r8(fileobj, name, level_data, level_type, vertical_indices)
   type(FmsNetcdfFile_t), intent(in) :: fileobj
   character(len=*),      intent(in) :: name
   real(r8_kind), dimension(:),   intent(out) :: level_data
   integer,              intent(out) :: level_type, vertical_indices
   real(r8_kind), dimension(:), allocatable   :: alpha
   integer                           :: n, nlev
...
end subroutine get_axis_level_data_r8
...
!! @param [in] <q4> No description
!! @param [out] <e2> No description
 subroutine cell_center2_r8(q1, q2, q3, q4, e2)
      real(r8_kind) , intent(in ) :: q1(2), q2(2), q3(2), q4(2)
      real(r8_kind) , intent(out) :: e2(2)
! Local
      real(r8_kind) :: p1(3), p2(3), p3(3), p4(3)
      real(r8_kind) :: ec(3,1)
      real(r8_kind) :: dd
      integer k
 ...
 end subroutine cell_center2_r8
!
!---------------------------------------------------------------------
!> @brief car_to_latlon receives the variables np, q, xs, and ys
!!        as inputs and returns q, xs, and ys.
...
!! @param [inout] <xs> No description
!! @param [inout] <ys> No description
 subroutine cart_to_latlon_r8(np, q, xs, ys)
! vector version of cart_to_latlon1
  integer, intent(in):: np
  real(r8_kind), intent(inout):: q(3,np)
  real(r8_kind), intent(inout):: xs(np), ys(np)
! local
  real(f_p), parameter:: esl=1.e-10_f_p
  real(f_p):: p(3)
  real(f_p):: dist, lat, lon
  ...
  do i=1,np
     ...
     xs(i) = real(lon,r8_kind)
     ys(i) = real(lat,r8_kind)
! q Normalized:
     do k=1,3
        q(k,i) = real(p(k),r8_kind)
     enddo
  enddo

 end  subroutine cart_to_latlon_r8
!
!---------------------------------------------------------------------
!> @brief latlon2xyz receives the variable p as input and returns e
!!        as output in order to map (lon, lat) to (x,y,z).
...
!! @param [in] <p> No description
!! @param [out] <e> No description
 subroutine latlon2xyz_r8(p, e)
!
! Routine to map (lon, lat) to (x,y,z)
!
 real(r8_kind), intent(in) :: p(2)
 real(r8_kind), intent(out):: e(3)
...
!! @throw FATAL, "init_clim_diag : You must call interpolator_init before calling init_clim_diag"
!! @throw FATAL, "init_clim_diag : Trying to set up too many diagnostic fields for the climatology data"
subroutine init_clim_diag_r8(clim_type, mod_axes, init_time)
!
! Routine to register diagnostic fields for the climatology file.
! This routine calculates the domain decompostion of the climatology fields
! for later export through send_data.
...
if (.not. module_is_initialized .or. .not. allocated(clim_type%r8_type%lon)) &
   call mpp_error(FATAL, "init_clim_diag : You must call interpolator_init before calling init_clim_diag")


ndivs = mpp_npes()
nxd = size(clim_type%r8_type%lon(:))
nyd = size(clim_type%r8_type%lat(:))

! Define the domain decomposition of the climatology file. This may be (probably is) different from the model domain.
call mpp_define_layout ((/1,nxd,1,nyd/), ndivs, domain_layout)
call mpp_define_domains((/1,nxd,1,nyd/),domain_layout, domain,xhalo=0,yhalo=0)
call mpp_get_compute_domain(domain, iscomp, iecomp, jscomp, jecomp)
axes(1) = diag_axis_init(clim_type%file_name(1:5)//'x',clim_type%r8_type%lon,units='degrees',&
                         cart_name='x',domain2=domain)
axes(2) = diag_axis_init(clim_type%file_name(1:5)//'y',clim_type%r8_type%lat,units='degrees',&
                         cart_name='y',domain2=domain)
clim_type%is = iscomp
clim_type%ie = iecomp
clim_type%js = jscomp
...
end subroutine init_clim_diag_r8
...
!!                    climatology time but we have the next time. How did
!!                    this happen?"
subroutine obtain_interpolator_time_slices_r8 (clim_type, Time)
...
integer, parameter :: lkind=r8_kind
    ...
    if (clim_type%climatological_year) then
       !++lwh
       if (size(clim_type%time_slice) > 1) then
          call time_interp(Time, clim_type%time_slice, clim_type%r8_type%tweight, &
                           taum, taup, modtime=YEAR, err_msg=err_msg )
          if(trim(err_msg) /= '') then
             call mpp_error(FATAL,'interpolator_timeslice 1: '// &
                            & trim(err_msg)//' file='//trim(clim_type%file_name), FATAL)
          ...
          taum = 1
          taup = 1
          clim_type%r8_type%tweight = 0._lkind
       end if
       !--lwh
    else
       call time_interp(Time, clim_type%time_slice, clim_type%r8_type%tweight, taum, taup, err_msg=err_msg )
       if(trim(err_msg) /= '') then
          call mpp_error(FATAL,'interpolator_timeslice 2: '//trim(err_msg)//' file='//trim(clim_type%file_name), FATAL)
       endif
    endif
...
!RSHBUGFX   if ( modday <= day ) then
        ...
        call time_interp(Time, month, clim_type%r8_type%tweight3, taum, taup, err_msg=err_msg ) ! tweight3 is
                                                                                 !! the time weight between the months.
        !> protect against post-perth time_interp behavior below
        if (taum==2 .and. taup==2) clim_type%r8_type%tweight3 = 1._lkind
        if(trim(err_msg) /= '') then
          call mpp_error(FATAL,'interpolator_timeslice 3: '//trim(err_msg)//' file='//trim(clim_type%file_name), FATAL)
        endif
        ...
        call get_date(month(1), climyear, climmonth, climday, climhour, climminute, climsecond)
        t_prev = set_date(yearm, climmonth, climday, climhour, climminute, climsecond)
        call time_interp(t_prev, month, clim_type%r8_type%tweight1, taum, taup, err_msg=err_msg ) !tweight1 is
                                                                      !! the time weight between the climatology years.
        !>  protect against post-perth time_interp behavior below
        if (taum==2 .and. taup==2) clim_type%r8_type%tweight1 = 1._lkind
        if(trim(err_msg) /= '') then
           call mpp_error(FATAL,'interpolator_timeslice 4: '// &
                          & trim(err_msg)//' file='//trim(clim_type%file_name), FATAL)
        endif
        ...
        call get_date(month(1), climyear, climmonth, climday, climhour, climminute, climsecond)
        t_next = set_date(yearp, climmonth, climday, climhour, climminute, climsecond)
        call time_interp(t_next, month, clim_type%r8_type%tweight2, taum, taup, err_msg=err_msg ) !tweight1 is
                                                                      !! the time weight between the climatology years.
        !> protect against post-perth time_interp behavior below
        if (taum==2 .and. taup==2) clim_type%r8_type%tweight2 = 1._lkind
        if(trim(err_msg) /= '') then
           call mpp_error(FATAL,'interpolator_timeslice 5: '// &
                          & trim(err_msg)//' file='//trim(clim_type%file_name), FATAL)
        endif
        ...
        if (indexm == clim_type%indexm(1) .and.  &
          ...
          do i=1, size(clim_type%field_name(:))
               call read_data(clim_type,clim_type%field_name(i),  &
                    clim_type%r8_type%pmon_pyear(:,:,:,i),   &
                    clim_type%indexm(i)+(clim_type%climatology(i)-1)*12,i,Time)
! Read the data for the next month in the previous climatology.
               call read_data(clim_type,clim_type%field_name(i),  &
                    clim_type%r8_type%nmon_pyear(:,:,:,i),   &
                    clim_type%indexp(i)+(clim_type%climatology(i)-1)*12,i,Time)
               call read_data(clim_type,clim_type%field_name(i),  &
                    clim_type%r8_type%pmon_nyear(:,:,:,i),  &
                    clim_type%indexm(i)+clim_type%climatology(i)*12,i,Time)
               call read_data(clim_type,clim_type%field_name(i),  &
                    clim_type%r8_type%nmon_nyear(:,:,:,i),  &
                    clim_type%indexp(i)+clim_type%climatology(i)*12,i,Time)
           end do
         endif
          ...
          do i=1, size(clim_type%field_name(:))
             if (taum /= clim_type%time_init(i,1) .or. &
              ...
              call read_data(clim_type,clim_type%field_name(i),   &
                           clim_type%r8_type%pmon_pyear(:,:,:,i), taum,i,Time)
! Read the data for the next month in the previous climatology.
              call read_data(clim_type,clim_type%field_name(i),   &
                           clim_type%r8_type%nmon_pyear(:,:,:,i), taup,i,Time)
              clim_type%time_init(i,1) = taum
              clim_type%time_init(i,2) = taup
             endif
           end do
...
!       clim_type%tweight3 = 0.0 ! This makes [pn]mon_nyear irrelevant. Set them to 0 to test.
        clim_type%r8_type%tweight1 = 0.0_lkind
        clim_type%r8_type%tweight2 = 0.0_lkind
        clim_type%r8_type%tweight3 = clim_type%r8_type%tweight
      endif
    endif   !(BILINEAR)
...
! field(:,:,:,2) as the next time slice.
      do i=1, size(clim_type%field_name(:))
          call read_data(clim_type,clim_type%field_name(i), clim_type%r8_type%data5d(:,:,:,1,i), taum,i,Time)
               clim_type%time_init(i,1) = taum
               clim_type%itaum = 1
          call read_data(clim_type,clim_type%field_name(i), clim_type%r8_type%data5d(:,:,:,2,i), taup,i,Time)
               clim_type%time_init(i,2) = taup
               clim_type%itaup = 2
      end do
      endif ! clim_type%itaum.eq.clim_type%itaup.eq.0
...
!We have the previous time step but not the next time step data
        ...
        do i=1, size(clim_type%field_name(:))
           call read_data(clim_type,clim_type%field_name(i), &
                          clim_type%r8_type%data5d(:,:,:,clim_type%itaup,i), taup,i, Time)
           clim_type%time_init(i,clim_type%itaup)=taup
        end do
      endif
...
!-------------------------------------------------------------------
 ...
 end subroutine obtain_interpolator_time_slices_r8
...
!! @throw FATAL "Interpolator: the field name is not contained in this
!!                    intepolate_type: "
subroutine interpolator_4D_r8(clim_type, Time, phalf, interp_data,  &
                           field_name, is, js, clim_units, dim_order)
!
! Return 4-D field interpolated to model grid and time
!
...
character(len=*)      , intent(in)  :: field_name
type(time_type)       , intent(in)  :: Time
real(r8_kind), dimension(:,:,:), intent(in)  :: phalf
real(r8_kind), dimension(:,:,:,:), intent(out), target :: interp_data
integer               , intent(in) , optional :: is,js
character(len=*)      , intent(out), optional :: clim_units
integer               , intent(in), optional :: dim_order(4)
integer :: taum, taup, ilon
real(r8_kind), allocatable :: hinterp_data(:,:,:,:), p_fact(:,:), col_data(:,:,:), phalf_diff(:,:,:)
real(r8_kind), pointer :: interp_buf(:,:,:,:)
real(r8_kind) :: pclim(size(clim_type%r8_type%halflevs(:)))
integer :: istart,iend,jstart,jend
logical :: result, found
logical :: found_field=.false.
integer :: modyear, modmonth, modday, modhour, modminute, modsecond
...
nj = size(interp_data, dim_map(2))
nk = size(interp_data, dim_map(3))
nlevs = size(clim_type%r8_type%levs)
nfields = size(clim_type%field_name)
...
select case(clim_type%TIME_FLAG)
  case (LINEAR)
    do n=1, nfields
      hinterp_data(:,:,:,n) = (1._lkind-clim_type%r8_type%tweight)*  &
                clim_type%r8_type%data5d(istart:iend,jstart:jend,:,clim_type%itaum,n)  +  &
                                 clim_type%r8_type%tweight*   &
                clim_type%r8_type%data5d(istart:iend,jstart:jend,:,clim_type%itaup,n)
    end do
! case (SEASONAL)
! Do sine fit to data at this point
  case (BILINEAR)
    do n=1, nfields
      hinterp_data(:,:,:,n) = (1._lkind-clim_type%r8_type%tweight1)*&
                              (1._lkind-clim_type%r8_type%tweight3)*   &
                              clim_type%r8_type%pmon_pyear(istart:iend,jstart:jend,:,n) + &
                              (1._lkind-clim_type%r8_type%tweight2)*clim_type%r8_type%tweight3* &
                              clim_type%r8_type%nmon_pyear(istart:iend,jstart:jend,:,n) + &
                              clim_type%r8_type%tweight1* (1._lkind-clim_type%r8_type%tweight3)*&
                              clim_type%r8_type%pmon_nyear(istart:iend,jstart:jend,:,n) + &
                              clim_type%r8_type%tweight2* clim_type%r8_type%tweight3*   &
                              clim_type%r8_type%nmon_nyear(istart:iend,jstart:jend,:,n)
...
select case(clim_type%mr(i))
  case(NO_CONV)
    do k = 1,size(hinterp_data,3)
   col_data(:,:,i) = col_data(:,:,i) + hinterp_data(:,:,k,i)* &
      (clim_type%r8_type%halflevs(k+1)-clim_type%r8_type%halflevs(k))/real(grav,r8_kind)
    enddo
  ...
  case(KG_M2)
    do k = 1,size(hinterp_data,3)
       col_data(:,:,i) = col_data(:,:,i) + hinterp_data(:,:,k,i)
       hinterp_data(:,:,k,i) = hinterp_data(:,:,k,i)/ &
         ((clim_type%r8_type%halflevs(k+1)-clim_type%r8_type%halflevs(k))*p_fact)
    enddo
end select
    enddo
...
do j = 1, size(phalf,2)
   do ilon=1,size(phalf,1)
      pclim = p_fact(ilon,j)*clim_type%r8_type%halflevs
      if ( maxval(phalf(ilon,j,:)) > maxval(pclim) ) then
         if (verbose > 3) then
         call mpp_error(NOTE,"Interpolator: model surface pressure&
                             & is greater than climatology surface pressure for "&
...
end subroutine interpolator_4D_r8
!
!#######################################################################
!#######################################################################
!
...
!! @throw FATAL "Interpolator: the field name is not contained in this
!!                    intepolate_type: "
subroutine interpolator_3D_r8(clim_type, Time, phalf, interp_data, field_name, is, js, clim_units, dim_order)
!
! Return 3-D field interpolated to model grid and time
!
! INTENT INOUT
...
!RSHBUGFX   if ( modday <= day ) then
        ...
        call time_interp(Time, month, clim_type%r8_type%tweight3, taum, taup, err_msg=err_msg ) ! tweight3 is
                                                                                 !! the time weight between the months.
        if (taum==2 .and. taup==2) &
             clim_type%r8_type%tweight3 = 1._lkind ! protect against post-perth time_interp behavior
        if(trim(err_msg) /= '') then
           call mpp_error(FATAL,'interpolator_3D 3: '//trim(err_msg)//' file='//trim(clim_type%file_name), FATAL)
        endif
        ...
        call time_interp(t_prev, month, clim_type%r8_type%tweight1, taum, taup, err_msg=err_msg ) !tweight1 is
                                                                      !! the time weight between the climatology years.
        if (taum==2 .and. taup==2) &
             clim_type%r8_type%tweight1 = 1._lkind ! protect against post-perth time_interp behavior
        if(trim(err_msg) /= '') then
           call mpp_error(FATAL,'interpolator_3D 4: '//trim(err_msg)//' file='//trim(clim_type%file_name), FATAL)
        endif
        ...
        call time_interp(t_next, month, clim_type%r8_type%tweight2, taum, taup, err_msg=err_msg ) !tweight1 is
                                                                      !! the time weight between the climatology years.
        if (taum==2 .and. taup==2) &
             clim_type%r8_type%tweight2 = 1._lkind ! protect against post-perth time_interp behavior
        if(trim(err_msg) /= '') then
           call mpp_error(FATAL,'interpolator_3D 5: '//trim(err_msg)//' file='//trim(clim_type%file_name), FATAL)
        endif
...
! Read the data for the next month in the previous climatology.
        ...
        if (taum /= clim_type%time_init(i,1) .or. &
          ...
          call read_data(clim_type,clim_type%field_name(i), clim_type%r8_type%pmon_pyear(:,:,:,i), taum,i,Time)
! Read the data for the next month in the previous climatology.
          call read_data(clim_type,clim_type%field_name(i), clim_type%r8_type%nmon_pyear(:,:,:,i), taup,i,Time)
!RSHbug   clim_type%pmon_nyear = 0.0
!RSHbug   clim_type%nmon_nyear = 0.0
...
!       clim_type%tweight3 = 0.0 ! This makes [pn]mon_nyear irrelevant. Set them to 0 to test.
        clim_type%r8_type%tweight1 = 0.0_lkind ; clim_type%r8_type%tweight2 = 0.0_lkind
        clim_type%r8_type%tweight3 = clim_type%r8_type%tweight
      endif
...
select case(clim_type%TIME_FLAG)
  case (LINEAR)
    hinterp_data = (1._lkind-clim_type%r8_type%tweight) * &
                    clim_type%r8_type%data5d(istart:iend,jstart:jend,:,clim_type%itaum,i) + &
                    clim_type%r8_type%tweight * &
                    clim_type%r8_type%data5d(istart:iend,jstart:jend,:,clim_type%itaup,i)
! case (SEASONAL)
! Do sine fit to data at this point
  case (BILINEAR)
    hinterp_data = &
    (1._lkind-clim_type%r8_type%tweight1)*&
    (1._lkind-clim_type%r8_type%tweight3)*clim_type%r8_type%pmon_pyear(istart:iend,jstart:jend,:,i) + &
    (1._lkind-clim_type%r8_type%tweight2) * clim_type%r8_type%tweight3  &
    * clim_type%r8_type%nmon_pyear(istart:iend,jstart:jend,:,i) + &
    clim_type%r8_type%tweight1 * (1._lkind-clim_type%r8_type%tweight3) * &
    clim_type%r8_type%pmon_nyear(istart:iend,jstart:jend,:,i) + &
    clim_type%r8_type%tweight2 * clim_type%r8_type%tweight3 * &
    clim_type%r8_type%nmon_nyear(istart:iend,jstart:jend,:,i)
...
end subroutine interpolator_3D_r8
!
!#######################################################################
!
!++lwh
...
!! @throw FATAL "Interpolator: the field name is not contained in this
!!                    intepolate_type: "
subroutine interpolator_2D_r8(clim_type, Time, interp_data, field_name, is, js, clim_units, dim_order)
!
! Return 2-D field interpolated to model grid and time
!
!
...
integer               , intent(in), optional :: dim_order(2)
integer :: taum, taup
real(r8_kind), allocatable :: hinterp_data(:,:,:)
integer :: istart,iend,jstart,jend
logical :: result, found
logical :: found_field=.false.
integer :: modyear, modmonth, modday, modhour, modminute, modsecond
...
select case(clim_type%TIME_FLAG)
  case (LINEAR)
    hinterp_data = (1._lkind-clim_type%r8_type%tweight)*&
         clim_type%r8_type%data5d(istart:iend,jstart:jend,:,clim_type%itaum,i) &
         + clim_type%r8_type%tweight*clim_type%r8_type%data5d &
         (istart:iend,jstart:jend,:,clim_type%itaup,i)
! case (SEASONAL)
! Do sine fit to data at this point
  case (BILINEAR)
    ...
    (1._lkind-clim_type%r8_type%tweight1) * &
    (1._lkind-clim_type%r8_type%tweight3) * &
    clim_type%r8_type%pmon_pyear(istart:iend,jstart:jend,:,i) + &
    (1._lkind-clim_type%r8_type%tweight2) * &
    clim_type%r8_type%tweight3  * clim_type%r8_type%nmon_pyear(istart:iend,jstart:jend,:,i) + &
    clim_type%r8_type%tweight1 * (1._lkind-clim_type%r8_type%tweight3) * &
    clim_type%r8_type%pmon_nyear(istart:iend,jstart:jend,:,i) + &
    clim_type%r8_type%tweight2 * clim_type%r8_type%tweight3 * &
    clim_type%r8_type%nmon_nyear(istart:iend,jstart:jend,:,i)
...
end subroutine interpolator_2D_r8
!--lwh
!
!#######################################################################
!
...
!! @throw FATAL "Interpolator: the field name is not contained in this
!!                    intepolate_type: "
subroutine interpolator_4D_no_time_axis_r8(clim_type, phalf, interp_data, field_name, is, js, clim_units, dim_order)
...
integer               , intent(in), optional :: dim_order(4)
integer :: ilon
real(r8_kind), allocatable :: hinterp_data(:,:,:,:), p_fact(:,:), phalf_diff(:,:,:)
real(r8_kind), pointer :: interp_buf(:,:,:,:)
real(r8_kind) :: pclim(size(clim_type%r8_type%halflevs(:)))
integer :: istart,iend,jstart,jend
logical :: found_field=.false.
...
do n=1, nfields
  hinterp_data(:,:,:,n) = clim_type%r8_type%data5d(istart:iend,jstart:jend,:,1,n)
end do
...
end subroutine interpolator_4D_no_time_axis_r8
...
!! @throw FATAL "Interpolator: the field name is not contained in this
!!                    intepolate_type: "
subroutine interpolator_3D_no_time_axis_r8(clim_type, phalf, interp_data, field_name, is, js, clim_units, dim_order)
...
real(r8_kind), allocatable :: hinterp_data(:,:,:), p_fact(:,:), phalf_diff(:,:,:)
real(r8_kind), pointer :: interp_buf(:,:,:)
real(r8_kind) :: pclim(size(clim_type%r8_type%halflevs(:))) !< No description
integer :: istart,iend,jstart,jend                                        !< No description
logical :: found_field=.false.          !< No description
integer :: i, j, k                   !< No description
integer :: ni, nj, nk
...
do i= 1,nfields
  if ( trim(adjustl(lowercase(field_name))) == trim(adjustl(lowercase(clim_type%field_name(i)))) ) then
    ...
    hinterp_data = clim_type%r8_type%data5d(istart:iend,jstart:jend,:,1,i)
...
end subroutine interpolator_3D_no_time_axis_r8
...
!! @throw FATAL "Interpolator: the field name is not contained in this
!!                    intepolate_type: "
subroutine interpolator_2D_no_time_axis_r8(clim_type, interp_data, field_name, is, js, clim_units, dim_order)
...
do i= 1,nfields
  if ( trim(adjustl(lowercase(field_name))) == trim(adjustl(lowercase(clim_type%field_name(i)))) ) then
    ...
    interp_data = reshape(clim_type%r8_type%data5d(istart:iend,jstart:jend,1,1,i), &
                         shape(interp_data), order=dim_map)
...
end subroutine interpolator_2D_no_time_axis_r8
...
!! @param [out] <hdata> The horizontally interpolated climatology field. This
!                       field will still be on the climatology vertical grid
subroutine read_data_r8(clim_type,field_name, hdata, nt,i, Time)
!
!  INTENT IN
!    clim_type : The interpolate type which contains the data
!    src_field : The field type
...
character(len=*)         , intent(in)  :: field_name
integer                  , intent(in)  :: nt
real(r8_kind)     , intent(out) :: hdata(:,:,:)
integer                  , intent(in)  :: i
type(time_type), optional, intent(in)  :: Time

integer   :: k, km
! sjs
real(r8_kind), allocatable :: climdata(:,:,:), climdata2(:,:,:)

      allocate(climdata(size(clim_type%r8_type%lon(:)),size(clim_type%r8_type%lat(:)), &
                        size(clim_type%r8_type%levs(:))))
      if(clim_type%has_level(i)) then ! has vertical level
          call fms2_io_read_data(clim_type%fileobj,field_name, climdata,nt)
      else  ! no vertical level
          call fms2_io_read_data(clim_type%fileobj,field_name, climdata(:,:,1),nt)
...
!  level data. the indices themselves were previously flipped.
      if (clim_type%vertical_indices == INCREASING_UPWARD) then
        allocate(climdata2(size(clim_type%r8_type%lon(:)),   &
                           size(clim_type%r8_type%lat(:)), &
                           size(clim_type%r8_type%levs(:))))
        km = size(clim_type%r8_type%levs(:))
        do k=1, km
          climdata2(:,:,k) = climdata(:,:,km+1-k)
        end do
        climdata = climdata2
...
!! @param [out] <hdata> The horizontally interpolated climatology field. This
!                       field will still be on the climatology vertical grid
subroutine read_data_no_time_axis_r8(clim_type,field_name, hdata, i)
...
real(r8_kind), allocatable :: climdata(:,:,:), climdata2(:,:,:)
      ...
      allocate(climdata(size(clim_type%r8_type%lon(:)),&
                        size(clim_type%r8_type%lat(:)),&
                        size(clim_type%r8_type%levs(:))))
...
end subroutine read_data_no_time_axis_r8
...
!! @param [in] <i> The index of the field name that you are diagnosing
!! @param [in] <Time> The model time.
subroutine diag_read_data_r8(clim_type,model_data, i, Time)
!
! A routine to diagnose the data read in by read_data
!
!  INTENT IN
...
!
type(interpolate_type), intent(in) :: clim_type
real(r8_kind)  , intent(in) :: model_data(:,:,:)
integer               , intent(in) :: i
type(time_type)       , intent(in) :: Time

integer :: j,k
real(r8_kind) :: col_data(size(model_data,1),size(model_data,2))
logical :: result, found
...
end subroutine diag_read_data_r8
!
!#################################################################
!
!
...
!! @param [in] <datin> No description
!! @param [out] <datout> No description
subroutine interp_weighted_scalar_2d_r8 (grdin, grdout, datin, datout )
real(r8_kind), intent(in),  dimension(:) :: grdin, grdout
real(r8_kind), intent(in),  dimension(:,:) :: datin
real(r8_kind), intent(out), dimension(:,:) :: datout
...
end subroutine interp_weighted_scalar_2d_r8
...
!! @param [in] <datin> No description
!! @param [out] <datout> No description
subroutine interp_weighted_scalar_1d_r8 (grdin, grdout, datin, datout )
real(r8_kind), intent(in),  dimension(:) :: grdin, grdout, datin
real(r8_kind), intent(out), dimension(:) :: datout
...
end subroutine interp_weighted_scalar_1d_r8
!
!#################################################################
!
!---------------------------------------------------------------------
...
!! @param [in] <datin> No description
!! @param [out] <datout> No description
subroutine interp_linear_r8 ( grdin, grdout, datin, datout )
real(r8_kind), intent(in),  dimension(:) :: grdin, grdout, datin
real(r8_kind), intent(out), dimension(:) :: datout
...
!
!#######################################################################
# 68 "interpolator/include/interpolator_r8.fh" 2
# 770 "interpolator/interpolator.F90" 2
