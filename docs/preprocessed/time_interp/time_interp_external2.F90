# 1 "time_interp/time_interp_external2.F90"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "time_interp/time_interp_external2.F90"
!***********************************************************************
...
# 70 "time_interp/time_interp_external2.F90" 2
...
# 1 "time_interp/include/time_interp_external2_r4.fh" 1
!***********************************************************************
!*                             Apache License 2.0
!*
!* This file is part of the GFDL Flexible Modeling System (FMS).
...
# 1 "time_interp/include/time_interp_external2.inc" 1
!***********************************************************************
!*                             Apache License 2.0
!*
!* This file is part of the GFDL Flexible Modeling System (FMS).
...
!***********************************************************************
    ...
    !> @brief 1D interpolation for @ref time_interp_external
    subroutine time_interp_external_1d_r4(index, time, data_in, interp, verbose,horz_interp, mask_out, &
               is_in, ie_in, window_id)

      integer, intent(in) :: index
      type(time_type), intent(in) :: time
      real(r4_kind), dimension(:), intent(inout) :: data_in
      ...
      real(r4_kind), dimension(size(data_in,1), 1, 1) :: data_out
      logical, dimension(size(data_in), 1, 1) :: mask3d

      data_out(:,1,1) = data_in(:) ! fill initial values for the portions of array that are not touched by 3d routine
      call time_interp_external(index, time, data_out, interp, verbose, horz_interp, mask3d, &
                                is_in=is_in,ie_in=ie_in,js_in=1,je_in=1,window_id=window_id)
      data_in(:) = data_out(:,1,1)
      if (PRESENT(mask_out)) mask_out(:) = mask3d(:,1,1)

    end subroutine time_interp_external_1d_r4

    !> @brief 2D interpolation for @ref time_interp_external
    subroutine time_interp_external_2d_r4(index, time, data_in, interp, verbose,horz_interp, mask_out, &
      ...
      real(r4_kind), dimension(size(data_in,1), size(data_in,2), 1) :: data_out
      ...
      return
    end subroutine time_interp_external_2d_r4
    ...
    subroutine time_interp_external_3d_r4(index, time, time_data, interp,verbose,horz_interp, mask_out, is_in, ie_in, &
        ...
        real(r4_kind), dimension(:,:,:),  intent(inout)           :: time_data !< global or local data array
        integer,                    intent(in), optional :: interp
    ...
    !               time_data(isw:iew,jsw:jew,:) = time_interp_missing !field(index)%missing? Balaji
                    time_data(isw:iew,jsw:jew,:) = real(loaded_fields(index)%missing, r4_kind)
                end where
            else
                where(loaded_fields(index)%mask(isc:iec,jsc:jec,:,i1))
                time_data(isw:iew,jsw:jew,:) = real(loaded_fields(index)%domain_data(isc:iec,jsc:jec,:,i1),&
    ...
    end subroutine time_interp_external_3d_r4
!</SUBROUTINE> NAME="time_interp_external"

    !> @brief Scalar interpolation for @ref time_interp_external
    subroutine time_interp_external_0d_r4(index, time, time_data, verbose)

      integer, intent(in) :: index
      type(time_type), intent(in) :: time
      real(r4_kind), intent(inout) :: time_data
      logical, intent(in), optional :: verbose
      ...
      if (loaded_fields(index)%siz(4) == 1) then
         ...
         i1 = find_buf_index(1,loaded_fields(index)%ibuf)
         time_data = real(loaded_fields(index)%domain_data(1,1,1,i1), r4_kind)
      else
        if(loaded_fields(index)%have_modulo_times) then
          call time_interp(time,loaded_fields(index)%modulo_time_beg, loaded_fields(index)%modulo_time_end, &
                           loaded_fields(index)%time(:), w2, t1, t2, &
         ...
         if(i1<0.or.i2<0) &
              call mpp_error(FATAL,'time_interp_external : records were not loaded correctly in memory')
         time_data = real(loaded_fields(index)%domain_data(1,1,1,i1), r4_kind)*w1 &
              & + real(loaded_fields(index)%domain_data(1,1,1,i2), r4_kind)*w2
         if (verb) then
            write(outunit,*) 'ibuf= ',loaded_fields(index)%ibuf
            write(outunit,*) 'i1,i2= ',i1, i2
         endif
    ...
    end subroutine time_interp_external_0d_r4

! ============================================================================
# 34 "time_interp/include/time_interp_external2_r4.fh" 2
# 1144 "time_interp/time_interp_external2.F90" 2

# 1 "time_interp/include/time_interp_external2_r8.fh" 1
!***********************************************************************
!*                             Apache License 2.0
!*
!* This file is part of the GFDL Flexible Modeling System (FMS).
...
!***********************************************************************
    ...
    !> @brief 1D interpolation for @ref time_interp_external
    subroutine time_interp_external_1d_r8(index, time, data_in, interp, verbose,horz_interp, mask_out, &
               is_in, ie_in, window_id)

      integer, intent(in) :: index
      type(time_type), intent(in) :: time
      real(r8_kind), dimension(:), intent(inout) :: data_in
      integer, intent(in), optional :: interp
      logical, intent(in), optional :: verbose
      type(horiz_interp_type),intent(in), optional :: horz_interp
      logical, dimension(:), intent(out), optional :: mask_out !< set to true where output data is valid
      ...
      real(r8_kind), dimension(size(data_in,1), 1, 1) :: data_out
      logical, dimension(size(data_in), 1, 1) :: mask3d
    ...
    end subroutine time_interp_external_1d_r8

    !> @brief 2D interpolation for @ref time_interp_external
    subroutine time_interp_external_2d_r8(index, time, data_in, interp, verbose,horz_interp, mask_out, &
               is_in, ie_in, js_in, je_in, window_id)
      ...
      real(r8_kind), dimension(size(data_in,1), size(data_in,2), 1) :: data_out
      logical, dimension(size(data_in,1), size(data_in,2), 1) :: mask3d
      ...
      return
    end subroutine time_interp_external_2d_r8
    ...
    !! Data may be local to current processor or global, depending on
    !! "init_external_field" flags.
    subroutine time_interp_external_3d_r8(index, time, time_data, interp,verbose,horz_interp, mask_out, is_in, ie_in, &
                                      &  js_in, je_in, window_id)

        integer,                    intent(in)           :: index !< index of external field from previous call
                                                                  !! to init_external_field
        type(time_type),            intent(in)           :: time !< target time for data
        real(r8_kind), dimension(:,:,:),  intent(inout)           :: time_data !< global or local data array
        integer,                    intent(in), optional :: interp
        logical,                    intent(in), optional :: verbose !< flag for debugging
        type(horiz_interp_type),    intent(in), optional :: horz_interp
        logical, dimension(:,:,:), intent(out), optional :: mask_out !< set to true where output data is valid
        ...
        if (loaded_fields(index)%siz(4) == 1) then
            ...
            if( loaded_fields(index)%region_type == NO_REGION ) then
                ...
                elsewhere
    !               time_data(isw:iew,jsw:jew,:) = time_interp_missing !field(index)%missing? Balaji
                    time_data(isw:iew,jsw:jew,:) = real(loaded_fields(index)%missing, r8_kind)
                end where
            else
                where(loaded_fields(index)%mask(isc:iec,jsc:jec,:,i1))
                time_data(isw:iew,jsw:jew,:) = real(loaded_fields(index)%domain_data(isc:iec,jsc:jec,:,i1),&
    ...
    end subroutine time_interp_external_3d_r8
!</SUBROUTINE> NAME="time_interp_external"

    !> @brief Scalar interpolation for @ref time_interp_external
    subroutine time_interp_external_0d_r8(index, time, time_data, verbose)

      integer, intent(in) :: index
      type(time_type), intent(in) :: time
      real(r8_kind), intent(inout) :: time_data
      logical, intent(in), optional :: verbose
      ...
      if (loaded_fields(index)%siz(4) == 1) then
         ...
         call load_record_0d(loaded_fields(index),1)
         i1 = find_buf_index(1,loaded_fields(index)%ibuf)
         time_data = real(loaded_fields(index)%domain_data(1,1,1,i1), r8_kind)
      else
        if(loaded_fields(index)%have_modulo_times) then
          call time_interp(time,loaded_fields(index)%modulo_time_beg, loaded_fields(index)%modulo_time_end, &
                           loaded_fields(index)%time(:), w2, t1, t2, &
         ...
         if(i1<0.or.i2<0) &
              call mpp_error(FATAL,'time_interp_external : records were not loaded correctly in memory')
         time_data = real(loaded_fields(index)%domain_data(1,1,1,i1), r8_kind)*w1 &
              & + real(loaded_fields(index)%domain_data(1,1,1,i2), r8_kind)*w2
         if (verb) then
            write(outunit,*) 'ibuf= ',loaded_fields(index)%ibuf
            write(outunit,*) 'i1,i2= ',i1, i2
         endif
    ...
    end subroutine time_interp_external_0d_r8

! ============================================================================
# 34 "time_interp/include/time_interp_external2_r8.fh" 2
# 1145 "time_interp/time_interp_external2.F90" 2


# 1 "time_interp/include/time_interp_external2_bridge_r4.fh" 1
!***********************************************************************
!*                             Apache License 2.0
!*
!* This file is part of the GFDL Flexible Modeling System (FMS).
...
# 1 "time_interp/include/time_interp_external2_bridge.inc" 1
!***********************************************************************
!*                             Apache License 2.0
!*
!* This file is part of the GFDL Flexible Modeling System (FMS).
...
!***********************************************************************
    ...
    !> @brief 1D interpolation for @ref time_interp_external_bridge
    subroutine time_interp_external_bridge_1d_r4(index1, index2, time, data_in, interp, verbose,horz_interp, mask_out, &
               is_in, ie_in, window_id)

      integer, intent(in) :: index1 !< index of first external field
      integer, intent(in) :: index2 !< index of second external field
      type(time_type), intent(in) :: time !< target time for data
      real(r4_kind), dimension(:), intent(inout) :: data_in !< global or local data array
      ...
      logical, dimension(:), intent(out), optional :: mask_out !< set to true where output data is valid
      integer,                  intent(in), optional :: is_in, ie_in !< horizontal indices for load_record
      integer,                  intent(in), optional :: window_id !< harcoded to 1 in load_record

      real(r4_kind), dimension(size(data_in,1), 1, 1) :: data_out !< 3d version of data_in
      logical, dimension(size(data_in,1), 1, 1) :: mask3d !< 3d version of mask_out
    ...
    end subroutine time_interp_external_bridge_1d_r4


    !> @brief 2D interpolation for @ref time_interp_external_bridge
    subroutine time_interp_external_bridge_2d_r4(index1, index2, time, data_in, interp, verbose,horz_interp, mask_out, &
               is_in, ie_in, js_in, je_in, window_id)
      ...
      real(r4_kind), dimension(size(data_in,1), size(data_in,2), 1) :: data_out !< 3d version of data_in
      ...
      return
    end subroutine time_interp_external_bridge_2d_r4

    !> 3D interpolation for @ref time_interp_external
    !! Provide data from external file interpolated to current model time.
    !! Data may be local to current processor or global, depending on
    !! "init_external_field" flags.
    subroutine time_interp_external_bridge_3d_r4(index1, index2, time, time_data, interp,verbose,horz_interp, mask_out, &
                                             &  is_in, ie_in, js_in, je_in, window_id)
      ...
      real(r4_kind) :: w1 !< interp weight for index1
      real(r4_kind) :: w2 !< interp weight for index2
    ...
    end subroutine time_interp_external_bridge_3d_r4

    subroutine time_interp_external_bridge_0d_r4(index1, index2, time, time_data, verbose)

      integer, intent(in) :: index1 !< index of first external field
      integer, intent(in) :: index2 !< index of second external field
      type(time_type), intent(in) :: time !< target time for data
      real(r4_kind), intent(inout) :: time_data !< global or local data array
      logical, intent(in), optional :: verbose !< flag for debugging
      ...
      if(i1<0.or.i2<0) &
           call mpp_error(FATAL,'time_interp_external : records were not loaded correctly in memory')
      time_data = real(loaded_fields(index2)%domain_data(1,1,1,i1), r4_kind)*w1 &
                + real(loaded_fields(index2)%domain_data(1,1,1,i2), r4_kind)*w2
      if (verb) then
         write(outunit,*) 'ibuf= ',loaded_fields(index2)%ibuf
         write(outunit,*) 'i1,i2= ',i1, i2
      endif

    end subroutine time_interp_external_bridge_0d_r4


! ============================================================================
# 34 "time_interp/include/time_interp_external2_bridge_r4.fh" 2
# 1147 "time_interp/time_interp_external2.F90" 2

# 1 "time_interp/include/time_interp_external2_bridge_r8.fh" 1
!***********************************************************************
!*                             Apache License 2.0
!*
!* This file is part of the GFDL Flexible Modeling System (FMS).
...
!***********************************************************************
    ...
    !> @brief 1D interpolation for @ref time_interp_external_bridge
    subroutine time_interp_external_bridge_1d_r8(index1, index2, time, data_in, interp, verbose,horz_interp, mask_out, &
               is_in, ie_in, window_id)

      integer, intent(in) :: index1 !< index of first external field
      integer, intent(in) :: index2 !< index of second external field
      type(time_type), intent(in) :: time !< target time for data
      real(r8_kind), dimension(:), intent(inout) :: data_in !< global or local data array
      integer, intent(in), optional :: interp !< hardcoded to linear
      logical, intent(in), optional :: verbose !< flag for debugging
      type(horiz_interp_type),intent(in), optional :: horz_interp !< horizontal interpolation type
      logical, dimension(:), intent(out), optional :: mask_out !< set to true where output data is valid
      ...
      real(r8_kind), dimension(size(data_in,1), 1, 1) :: data_out !< 3d version of data_in
      logical, dimension(size(data_in,1), 1, 1) :: mask3d !< 3d version of mask_out
    ...
    end subroutine time_interp_external_bridge_1d_r8


    !> @brief 2D interpolation for @ref time_interp_external_bridge
    subroutine time_interp_external_bridge_2d_r8(index1, index2, time, data_in, interp, verbose,horz_interp, mask_out, &
               is_in, ie_in, js_in, je_in, window_id)
      ...
      real(r8_kind), dimension(size(data_in,1), size(data_in,2), 1) :: data_out !< 3d version of data_in
      logical, dimension(size(data_in,1), size(data_in,2), 1) :: mask3d !< 3d version of mask_out
      ...
      return
    end subroutine time_interp_external_bridge_2d_r8

    !> 3D interpolation for @ref time_interp_external
    !! Provide data from external file interpolated to current model time.
    !! Data may be local to current processor or global, depending on
    !! "init_external_field" flags.
    subroutine time_interp_external_bridge_3d_r8(index1, index2, time, time_data, interp,verbose,horz_interp, mask_out, &
                                             &  is_in, ie_in, js_in, je_in, window_id)
      ...
      real(r8_kind) :: w1 !< interp weight for index1
      real(r8_kind) :: w2 !< interp weight for index2
      logical :: verb !< verbose
      character(len=16) :: message1 !< temp string
      character(len=16) :: message2 !< temp string
      integer, parameter :: kindl = r8_kind
    ...
    end subroutine time_interp_external_bridge_3d_r8

    subroutine time_interp_external_bridge_0d_r8(index1, index2, time, time_data, verbose)

      integer, intent(in) :: index1 !< index of first external field
      integer, intent(in) :: index2 !< index of second external field
      type(time_type), intent(in) :: time !< target time for data
      real(r8_kind), intent(inout) :: time_data !< global or local data array
      logical, intent(in), optional :: verbose !< flag for debugging
      ...
      if(i1<0.or.i2<0) &
           call mpp_error(FATAL,'time_interp_external : records were not loaded correctly in memory')
      time_data = real(loaded_fields(index2)%domain_data(1,1,1,i1), r8_kind)*w1 &
                + real(loaded_fields(index2)%domain_data(1,1,1,i2), r8_kind)*w2
      if (verb) then
         write(outunit,*) 'ibuf= ',loaded_fields(index2)%ibuf
         write(outunit,*) 'i1,i2= ',i1, i2
      endif

    end subroutine time_interp_external_bridge_0d_r8


! ============================================================================
# 34 "time_interp/include/time_interp_external2_bridge_r8.fh" 2
# 1148 "time_interp/time_interp_external2.F90" 2
