# 1 "field_manager/field_manager.F90"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "field_manager/field_manager.F90"
!***********************************************************************
...
# 201 "field_manager/field_manager.F90" 2
...
!TODO these variable can be removed when the legacy table is no longer used
integer,           parameter :: MAX_FIELDS        = 250
integer,           parameter :: MAX_FIELD_METHODS = 250
!
...
# 1 "field_manager/include/field_manager_r4.fh" 1
!***********************************************************************
!*                             Apache License 2.0
!*
!* This file is part of the GFDL Flexible Modeling System (FMS).
...
# 1 "field_manager/include/field_manager.inc" 1
!***********************************************************************
!*                             Apache License 2.0
!*
!* This file is part of the GFDL Flexible Modeling System (FMS).
...
!! a user to define a large array and fill it partially with
!! values from a list. This should be the size of the value array.
function parse_reals_r4 ( text, label, values ) result (parse)
character(len=*), intent(in)  :: text !< The text string from which the values will be parsed.
character(len=*), intent(in)  :: label !< A label which describes the values being decoded.
real(r4_kind), intent(out) :: values(:) !< The value or values that have been decoded.

include 'parse.inc'
end function parse_reals_r4

function parse_real_r4 ( text, label, parse_rval ) result (parse)
character(len=*), intent(in)  :: text !< The text string from which the values will be parsed.
character(len=*), intent(in)  :: label !< A label which describes the values being decoded.
real(r4_kind), intent(out) :: parse_rval !< The value or values that have been decoded.
integer :: parse

real(r4_kind) :: values(1)

   parse = parse_reals_r4( text, label, values )
   if (parse > 0) parse_rval = values(1)
 end function parse_real_r4

!> @returns A flag to indicate whether the function operated with (false) or without
!! (true) errors.
function  fm_get_value_real_r4(name, get_rval, index)                 &
          result (success)
logical                                :: success
character(len=*), intent(in)           :: name  !< The name of a field that the user wishes to get a value for.
real(r4_kind), intent(out)        :: get_rval !< The value associated with the named field
integer,          intent(in), optional :: index !< An optional index to retrieve a single value from an array.
...
end function  fm_get_value_real_r4

!> @brief Assigns a given value to a given field
!> @returns An index for the named field
function  fm_new_value_real_r4(name, new_rval, create, index, append) &
          result (field_index)
integer                                :: field_index
character(len=*), intent(in)           :: name !< The name of a field that the user wishes to create
                                               !! a value for.
real(r4_kind),    intent(in)      :: new_rval !< The value that the user wishes to apply to the
                                                !! named field.
logical,          intent(in), optional :: create !< If present and .true., then a value for this
                                                 !! field will be created.
integer,          intent(in), optional :: index !< The index to an array of values that the user
...
end function  fm_new_value_real_r4
# 38 "field_manager/include/field_manager_r4.fh" 2
# 3646 "field_manager/field_manager.F90" 2

# 1 "field_manager/include/field_manager_r8.fh" 1
!***********************************************************************
!*                             Apache License 2.0
!*
!* This file is part of the GFDL Flexible Modeling System (FMS).
...
character(len=*), intent(in)  :: text !< The text string from which the values will be parsed.
character(len=*), intent(in)  :: label !< A label which describes the values being decoded.
real(r8_kind), intent(out) :: values(:) !< The value or values that have been decoded.

include 'parse.inc'
end function parse_reals_r8

function parse_real_r8 ( text, label, parse_rval ) result (parse)
character(len=*), intent(in)  :: text !< The text string from which the values will be parsed.
character(len=*), intent(in)  :: label !< A label which describes the values being decoded.
real(r8_kind), intent(out) :: parse_rval !< The value or values that have been decoded.
integer :: parse

real(r8_kind) :: values(1)
...
!> @returns A flag to indicate whether the function operated with (false) or without
!! (true) errors.
function  fm_get_value_real_r8(name, get_rval, index)                 &
          result (success)
logical                                :: success
character(len=*), intent(in)           :: name  !< The name of a field that the user wishes to get a value for.
real(r8_kind), intent(out)        :: get_rval !< The value associated with the named field
integer,          intent(in), optional :: index !< An optional index to retrieve a single value from an array.
...
end function  fm_get_value_real_r8

!> @brief Assigns a given value to a given field
!> @returns An index for the named field
function  fm_new_value_real_r8(name, new_rval, create, index, append) &
          result (field_index)
integer                                :: field_index
character(len=*), intent(in)           :: name !< The name of a field that the user wishes to create
                                               !! a value for.
real(r8_kind),    intent(in)      :: new_rval !< The value that the user wishes to apply to the
                                                !! named field.
logical,          intent(in), optional :: create !< If present and .true., then a value for this
                                                 !! field will be created.
integer,          intent(in), optional :: index !< The index to an array of values that the user
...
end function  fm_new_value_real_r8
# 38 "field_manager/include/field_manager_r8.fh" 2
# 3647 "field_manager/field_manager.F90" 2
