...
# 82 "horiz_interp/horiz_interp_bilinear.F90" 2
...
# 1 "horiz_interp/include/horiz_interp_bilinear_r4.fh" 1
!***********************************************************************
!*                             Apache License 2.0
!*
!* This file is part of the GFDL Flexible Modeling System (FMS).
...
# 1 "horiz_interp/include/horiz_interp_bilinear.inc" 1
!***********************************************************************
!*                             Apache License 2.0
!*
!* This file is part of the GFDL Flexible Modeling System (FMS).
...
!* governing permissions and limitations under the License.
!***********************************************************************
  subroutine horiz_interp_bilinear_new_1d_r4 ( Interp, lon_in, lat_in, lon_out, lat_out, &
       verbose, src_modulo )
    ...
    real(r4_kind)    :: wtw, wte, wts, wtn, lon, lat, tpi, hpi
    real(r4_kind)    :: glt_min, glt_max, gln_min, gln_max, min_lon, max_lon
    ...
    allocate ( Interp % horizInterpReals4_type % wti (size(lon_out,1),size(lon_out,2),2),   &
               Interp % horizInterpReals4_type % wtj (size(lon_out,1),size(lon_out,2),2),   &
    ...
    if(src_is_modulo) then
       if(lon_in(nlon_in) - lon_in(1) .gt. tpi + real(epsln, r4_kind)) &
       ...
       do m = 1, nlon_out
          ...
          if(lon_in(is) .ne. lon_in(ie) .and. (decreasing_lon .or. lon_in(is) .le. lon)) then
             ...
             if (lon_in(ie) .ge. lon ) then
                wtw = (lon_in(ie) -lon)/(lon_in(ie)-lon_in(is)+tpi+real(epsln,r4_kind))
             else
                wtw = (lon_in(ie)-lon+tpi+real(epsln,r4_kind))/(lon_in(ie)-lon_in(is)+tpi+real(epsln,r4_kind))
             endif
          endif
          wte = 1.0_kindl - wtw
          ...
          Interp % horizInterpReals4_type % wti   (m,n,1) = wtw
          Interp % horizInterpReals4_type % wti   (m,n,2) = wte
          Interp % horizInterpReals4_type % wtj   (m,n,1) = wts
          Interp % horizInterpReals4_type % wtj   (m,n,2) = wtn
  ...
  end subroutine horiz_interp_bilinear_new_1d_r4
  ...
  subroutine horiz_interp_bilinear_new_2d_r4 ( Interp, lon_in, lat_in, lon_out, lat_out, &
    ...
    real(r4_kind)                                   :: lon, lat, quadra, x, y, y1, y2
    real(r4_kind)                                   :: a1, b1, c1, d1, a2, b2, c2, d2, a, b, c
    real(r4_kind)                                   :: lon1, lat1, lon2, lat2, lon3, lat3, lon4, lat4
    real(r4_kind)                                   :: tpi, lon_min, lon_max
    real(r4_kind)                                   :: epsln2
    ...
    if(use_new_search) then
       epsln2 = real(epsln,r4_kind)* 1.0e5_kindl
       call find_neighbor_new_r4(Interp, lon_in, lat_in, lon_out, lat_out, src_is_modulo, no_crash)
    else
       ...
       call find_neighbor_r4(Interp, lon_in, lat_in, lon_out, lat_out, src_is_modulo)
    endif
    ...
    do n = 1, nlat_out
       do m = 1, nlon_out
          ...
          if(abs(quadra) < real(epsln, r4_kind)) quadra = 0.0_kindl
          ...
          if ( abs(a) .lt. epsln2) then  ! a = 0 is a linear equation
             if( abs(b) .lt. real(epsln,r4_kind)) call mpp_error(FATAL, &
           ...
           if(abs(a1+c1*y) < real(epsln,r4_kind)) call mpp_error(FATAL, &
           ...
           Interp % horizInterpReals4_type % wti(m,n,1)=1.0_kindl-x
           Interp % horizInterpReals4_type % wti(m,n,2)=x
           Interp % horizInterpReals4_type % wtj(m,n,1)=1.0_kindl-y
           Interp % horizInterpReals4_type % wtj(m,n,2)=y
       enddo
    enddo
  ...
  subroutine find_neighbor_r4 ( Interp, lon_in, lat_in, lon_out, lat_out, src_modulo )
    type(horiz_interp_type), intent(inout) :: Interp
    real(r4_kind), intent(in),       dimension(:,:) :: lon_in , lat_in
    real(r4_kind), intent(in),       dimension(:,:) :: lon_out, lat_out
    ...
    real(r4_kind)                     :: lon_min, lon_max, lon, lat, tpi
    logical                                :: found
    real(r4_kind)                     :: lon1, lat1, lon2, lat2, lon3, lat3, lon4, lat4
    ...
    do n = 1, nlat_out
       do m = 1, nlon_out
          ...
          if(.not.found) then
              print *,'lon,lat=',lon*180.0_kindl/real(PI,r4_kind),lat*180.0_kindl/real(PI,r4_kind)
              print *,'npts=',npts
              print *,'is,ie= ',istart,iend
              print *,'js,je= ',jstart,jend
              print *,'lon_in(is,js)=',lon_in(istart,jstart)*180.0_kindl/real(PI,r4_kind)
              print *,'lon_in(ie,js)=',lon_in(iend,jstart)*180.0_kindl/real(PI,r4_kind)
              print *,'lat_in(is,js)=',lat_in(istart,jstart)*180.0_kindl/real(PI,r4_kind)
              print *,'lat_in(ie,js)=',lat_in(iend,jstart)*180.0_kindl/real(PI,r4_kind)
              print *,'lon_in(is,je)=',lon_in(istart,jend)*180.0_kindl/real(PI,r4_kind)
              print *,'lon_in(ie,je)=',lon_in(iend,jend)*180.0_kindl/real(PI,r4_kind)
              print *,'lat_in(is,je)=',lat_in(istart,jend)*180.0_kindl/real(PI,r4_kind)
              print *,'lat_in(ie,je)=',lat_in(iend,jend)*180.0_kindl/real(PI,r4_kind)
  ...
  function inside_polygon_r4(polyx, polyy, x, y)
     real(r4_kind), dimension(:), intent(in) :: polyx !< longitude coordinates of corners
     real(r4_kind), dimension(:), intent(in) :: polyy !< latitude coordinates of corners
     real(r4_kind),               intent(in) :: x !< x coordinate of point to be tested
     real(r4_kind),               intent(in) :: y !< y coordinate of point to be tested
     logical                        :: inside_polygon_r4
     integer                        :: i, j, nedges
     real(r4_kind)                           :: xx

     inside_polygon_r4 = .false.
     nedges = size(polyx(:))
     j = nedges
     do i = 1, nedges
        if( (polyy(i) < y .AND. polyy(j) >= y) .OR. (polyy(j) < y .AND. polyy(i) >= y) ) then
           ...
           if( xx == x ) then
             inside_polygon_r4 = .true.
             return
           else if( xx < x ) then
             inside_polygon_r4 = .not. inside_polygon_r4
           endif
        endif
        j = i
     enddo
  ...
  !> this routine will search the source grid to fine the grid box that encloses
  !! each destination grid.
  subroutine find_neighbor_new_r4( Interp, lon_in, lat_in, lon_out, lat_out, src_modulo, no_crash )
    type(horiz_interp_type), intent(inout) :: Interp
    real(r4_kind), intent(in),       dimension(:,:) :: lon_in , lat_in
    real(r4_kind), intent(in),       dimension(:,:) :: lon_out, lat_out
    ...
    real(r4_kind)                                   :: lon_min, lon_max, lon, lat, tpi
    logical                                :: found
    real(r4_kind)                                   :: polyx(4), polyy(4)
    real(r4_kind)                                   :: min_lon, min_lat, max_lon, max_lat

    integer, parameter :: step_div=8, kindl = r4_kind
...
!                   endif
                   ...
                   if(inside_polygon_r4(polyx, polyy, lon, lat)) then
                      found = .true.
!                      print*, " found ", i, j
                      Interp % i_lon (m,n,1) = i; Interp % i_lon (m,n,2) = ip1
                      Interp % j_lat (m,n,1) = j; Interp % j_lat (m,n,2) = jp1
  ...
  !#######################################################################
  function intersect_r4(x1, y1, x2, y2, x)
     real(r4_kind), intent(in) :: x1, y1, x2, y2, x
     real(r4_kind)             :: intersect_r4

     intersect_r4 = (y2-y1)*(x-x1)/(x2-x1) + y1

  return

  end function intersect_r4
  ...
  subroutine horiz_interp_bilinear_r4 ( Interp, data_in, data_out, verbose, mask_in,mask_out, &
    ...
    real(r4_kind), intent(out), dimension(:,:)           :: data_out !< output data on source grid
    ...
    real(r4_kind), intent(in), dimension(:,:),  optional :: mask_in !< Input mask, must be the same size as
                                    !! the input data. The real(r4_kind) value of mask_in must be in the
                                    !! range (0.,1.). Set mask_in=0.0 for data points
                                    !! that should not be used or have missing data
    real(r4_kind), intent(out), dimension(:,:), optional :: mask_out !< output mask that specifies whether
                                                            !! data was computed
    ...
    real(r4_kind)    :: dwtsum, wtsum, min_in, max_in, avg_in, &
         min_out, max_out, avg_out, wtw, wte, wts, wtn
    real(r4_kind)    :: mask(size(data_in,1), size(data_in,2) )
    logical :: set_to_missing, is_missing(4), new_handler
    real(r4_kind)    :: f1, f2, f3, f4, middle, w, s
    integer, parameter    :: kindl = r4_kind
    ...
    if(new_handler) then
       ...
       do n = 1, nlat_out
          do m = 1, nlon_out
             is = Interp % i_lon (m,n,1); ie = Interp % i_lon (m,n,2)
             js = Interp % j_lat (m,n,1); je = Interp % j_lat (m,n,2)
             wtw = Interp % horizInterpReals4_type % wti   (m,n,1)
             wte = Interp % horizInterpReals4_type % wti   (m,n,2)
             wts = Interp % horizInterpReals4_type % wtj   (m,n,1)
             wtn = Interp % horizInterpReals4_type % wtj   (m,n,2)
       ...
       do n = 1, nlat_out
          do m = 1, nlon_out
             ...
             if(num_missing .gt. max_missing ) then
                data_out(m,n) = missing_value
                if(present(mask_out)) mask_out(m,n) = 0.0_kindl
             else if(wtsum .lt. real(epsln, r4_kind)) then
                if(present(missing_value)) then
                   data_out(m,n) = missing_value
                else
...
903    format ('          number of missing points = ',i6)
  ...
  subroutine horiz_interp_read_weights_bilinear_r4(Interp, weight_filename, lon_out, lat_out, lon_in, lat_in, &
    ...
    real(r4_kind), target,     intent(in)    :: lat_out(:,:)       !< Output (model) latitude
    real(r4_kind), target,      intent(in)    :: lon_out(:,:)       !< Output (model) longitude
    ...
    real(r4_kind), allocatable :: var(:,:,:)     !< Dummy variable to read the indices and weight into
    ...
    allocate ( Interp % horizInterpReals4_type % wti (Interp%nlon_dst,Interp%nlat_dst,2),   &
              Interp % horizInterpReals4_type % wtj (Interp%nlon_dst,Interp%nlat_dst,2),   &
    ...
    !! var(:,:,3) -> (i+1,j)
    !! var(:,:,4) -> (i+1,j+1)
    Interp % horizInterpReals4_type % wti = var(:,:,1:2)
    Interp % horizInterpReals4_type % wtj = var(:,:,3:4)
    deallocate(var)
  ...
  end subroutine horiz_interp_read_weights_bilinear_r4
# 49 "horiz_interp/include/horiz_interp_bilinear_r4.fh" 2
# 121 "horiz_interp/horiz_interp_bilinear.F90" 2

# 1 "horiz_interp/include/horiz_interp_bilinear_r8.fh" 1
!***********************************************************************
!*                             Apache License 2.0
!*
!* This file is part of the GFDL Flexible Modeling System (FMS).
...
!* governing permissions and limitations under the License.
!***********************************************************************
  subroutine horiz_interp_bilinear_new_1d_r8 ( Interp, lon_in, lat_in, lon_out, lat_out, &
       verbose, src_modulo )
    ...
    integer :: nlon_in, nlat_in, nlon_out, nlat_out, n, m
    integer :: ie, is, je, js, ln_err, lt_err, warns, iunit
    real(r8_kind)    :: wtw, wte, wts, wtn, lon, lat, tpi, hpi
    real(r8_kind)    :: glt_min, glt_max, gln_min, gln_max, min_lon, max_lon
    ...
    allocate ( Interp % horizInterpReals8_type % wti (size(lon_out,1),size(lon_out,2),2),   &
               Interp % horizInterpReals8_type % wtj (size(lon_out,1),size(lon_out,2),2),   &
               Interp % i_lon (size(lon_out,1),size(lon_out,2),2), &
               Interp % j_lat (size(lon_out,1),size(lon_out,2),2))
    !-----------------------------------------------------------------------
    ...
    if(src_is_modulo) then
       if(lon_in(nlon_in) - lon_in(1) .gt. tpi + real(epsln, r8_kind)) &
            call mpp_error(FATAL,'horiz_interp_bilinear_mod: '// &
            'The range of source grid longitude should be no larger than tpi')
    ...
    do n = 1, nlat_out
       do m = 1, nlon_out
          ...
          if(lon_in(is) .ne. lon_in(ie) .and. (decreasing_lon .or. lon_in(is) .le. lon)) then
             ...
             is = nlon_in
             if (lon_in(ie) .ge. lon ) then
                wtw = (lon_in(ie) -lon)/(lon_in(ie)-lon_in(is)+tpi+real(epsln,r8_kind))
             else
                wtw = (lon_in(ie)-lon+tpi+real(epsln,r8_kind))/(lon_in(ie)-lon_in(is)+tpi+real(epsln,r8_kind))
             endif
          endif
          wte = 1.0_kindl - wtw
          ...
          Interp % i_lon (m,n,1) = is; Interp % i_lon (m,n,2) = ie
          Interp % j_lat (m,n,1) = js; Interp % j_lat (m,n,2) = je
          Interp % horizInterpReals8_type % wti   (m,n,1) = wtw
          Interp % horizInterpReals8_type % wti   (m,n,2) = wte
          Interp % horizInterpReals8_type % wtj   (m,n,1) = wts
          Interp % horizInterpReals8_type % wtj   (m,n,2) = wtn
  ...
  end subroutine horiz_interp_bilinear_new_1d_r8
  ...
  !> Allocates space and initializes a derived-type variable
  !! that contains pre-computed interpolation indices and weights.
  subroutine horiz_interp_bilinear_new_2d_r8 ( Interp, lon_in, lat_in, lon_out, lat_out, &
       verbose, src_modulo, new_search, no_crash_when_not_found )
    ...
    integer                                :: nlon_in, nlat_in, nlon_out, nlat_out
    integer                                :: m, n, is, ie, js, je, num_solution
    real(r8_kind)                                   :: lon, lat, quadra, x, y, y1, y2
    real(r8_kind)                                   :: a1, b1, c1, d1, a2, b2, c2, d2, a, b, c
    real(r8_kind)                                   :: lon1, lat1, lon2, lat2, lon3, lat3, lon4, lat4
    real(r8_kind)                                   :: tpi, lon_min, lon_max
    real(r8_kind)                                   :: epsln2
    logical                                :: use_new_search, no_crash
    ...
    !--- first fine the neighbor points for the destination points.
    if(use_new_search) then
       epsln2 = real(epsln,r8_kind)* 1.0e5_kindl
       call find_neighbor_new_r8(Interp, lon_in, lat_in, lon_out, lat_out, src_is_modulo, no_crash)
    else
       if(kindl == r8_kind) then
          epsln2 = epsln
       else
       ...
       call find_neighbor_r8(Interp, lon_in, lat_in, lon_out, lat_out, src_is_modulo)
    endif
    ...
    do n = 1, nlat_out
       do m = 1, nlon_out
          ...
          c  = a2*lon-a1*lat+a1*d2-a2*d1
          quadra = b*b-4._kindl*a*c
          if(abs(quadra) < real(epsln, r8_kind)) quadra = 0.0_kindl
          if(quadra < 0.0_kindl) call mpp_error(FATAL, &
               "horiz_interp_bilinear_mod: No solution existed for this quadratic equation")
          if ( abs(a) .lt. epsln2) then  ! a = 0 is a linear equation
             if( abs(b) .lt. real(epsln,r8_kind)) call mpp_error(FATAL, &
                  "horiz_interp_bilinear_mod: no unique solution existed for this linear equation")
             y = -c/b
          else
             y1 = 0.5_kindl*(-b+sqrt(quadra))/a
           ...
           if(abs(a1+c1*y) < real(epsln,r8_kind)) call mpp_error(FATAL, &
               "horiz_interp_bilinear_mod: the denomenator is 0")
           if(abs(y) < epsln2) y = 0.0_kindl
           if(abs(1.0_kindl-y) < epsln2) y = 1.0_kindl
           x = (lon-b1*y-d1)/(a1+c1*y)
           ...
           Interp % horizInterpReals8_type % wti(m,n,1)=1.0_kindl-x
           Interp % horizInterpReals8_type % wti(m,n,2)=x
           Interp % horizInterpReals8_type % wtj(m,n,1)=1.0_kindl-y
           Interp % horizInterpReals8_type % wtj(m,n,2)=y
       enddo
    enddo
  ...
  !> this routine will search the source grid to fine the grid box that encloses
  !! each destination grid.
  subroutine find_neighbor_r8 ( Interp, lon_in, lat_in, lon_out, lat_out, src_modulo )
    type(horiz_interp_type), intent(inout) :: Interp
    real(r8_kind), intent(in),       dimension(:,:) :: lon_in , lat_in
    real(r8_kind), intent(in),       dimension(:,:) :: lon_out, lat_out
    logical,                 intent(in)    :: src_modulo
    ...
    integer                                :: is, js, jstart, jend, istart, iend, npts
    integer, allocatable, dimension(:)     :: ilon, jlat
    real(r8_kind)                     :: lon_min, lon_max, lon, lat, tpi
    logical                                :: found
    real(r8_kind)                     :: lon1, lat1, lon2, lat2, lon3, lat3, lon4, lat4
    ...
    do n = 1, nlat_out
       do m = 1, nlon_out
          ...
          if(.not.found) then
              print *,'lon,lat=',lon*180.0_kindl/real(PI,r8_kind),lat*180.0_kindl/real(PI,r8_kind)
              print *,'npts=',npts
              print *,'is,ie= ',istart,iend
              print *,'js,je= ',jstart,jend
              print *,'lon_in(is,js)=',lon_in(istart,jstart)*180.0_kindl/real(PI,r8_kind)
              print *,'lon_in(ie,js)=',lon_in(iend,jstart)*180.0_kindl/real(PI,r8_kind)
              print *,'lat_in(is,js)=',lat_in(istart,jstart)*180.0_kindl/real(PI,r8_kind)
              print *,'lat_in(ie,js)=',lat_in(iend,jstart)*180.0_kindl/real(PI,r8_kind)
              print *,'lon_in(is,je)=',lon_in(istart,jend)*180.0_kindl/real(PI,r8_kind)
              print *,'lon_in(ie,je)=',lon_in(iend,jend)*180.0_kindl/real(PI,r8_kind)
              print *,'lat_in(is,je)=',lat_in(istart,jend)*180.0_kindl/real(PI,r8_kind)
              print *,'lat_in(ie,je)=',lat_in(iend,jend)*180.0_kindl/real(PI,r8_kind)
  ...
  !! false if it is not.  If the point is exactly on the edge of a polygon,
  !! the function will return .true.
  function inside_polygon_r8(polyx, polyy, x, y)
     real(r8_kind), dimension(:), intent(in) :: polyx !< longitude coordinates of corners
     real(r8_kind), dimension(:), intent(in) :: polyy !< latitude coordinates of corners
     real(r8_kind),               intent(in) :: x !< x coordinate of point to be tested
     real(r8_kind),               intent(in) :: y !< y coordinate of point to be tested
     logical                        :: inside_polygon_r8
     integer                        :: i, j, nedges
     real(r8_kind)                           :: xx

     inside_polygon_r8 = .false.
     nedges = size(polyx(:))
     j = nedges
     do i = 1, nedges
        if( (polyy(i) < y .AND. polyy(j) >= y) .OR. (polyy(j) < y .AND. polyy(i) >= y) ) then
           xx = polyx(i)+(y-polyy(i))/(polyy(j)-polyy(i))*(polyx(j)-polyx(i))
           if( xx == x ) then
             inside_polygon_r8 = .true.
             return
           else if( xx < x ) then
             inside_polygon_r8 = .not. inside_polygon_r8
           endif
        endif
        j = i
     enddo
  ...
  !> this routine will search the source grid to fine the grid box that encloses
  !! each destination grid.
  subroutine find_neighbor_new_r8( Interp, lon_in, lat_in, lon_out, lat_out, src_modulo, no_crash )
    type(horiz_interp_type), intent(inout) :: Interp
    real(r8_kind), intent(in),       dimension(:,:) :: lon_in , lat_in
    real(r8_kind), intent(in),       dimension(:,:) :: lon_out, lat_out
    logical,                 intent(in)    :: src_modulo, no_crash
    ...
    real(r8_kind)                                   :: lon_min, lon_max, lon, lat, tpi
    logical                                :: found
    real(r8_kind)                                   :: polyx(4), polyy(4)
    real(r8_kind)                                   :: min_lon, min_lat, max_lon, max_lat

    integer, parameter :: step_div=8, kindl = r8_kind
...
!                   endif
                   ...
                   if(inside_polygon_r8(polyx, polyy, lon, lat)) then
                      found = .true.
!                      print*, " found ", i, j
                      Interp % i_lon (m,n,1) = i; Interp % i_lon (m,n,2) = ip1
                      Interp % j_lat (m,n,1) = j; Interp % j_lat (m,n,2) = jp1
  ...
  !#######################################################################
  function intersect_r8(x1, y1, x2, y2, x)
     real(r8_kind), intent(in) :: x1, y1, x2, y2, x
     real(r8_kind)             :: intersect_r8

     intersect_r8 = (y2-y1)*(x-x1)/(x2-x1) + y1

  return

  end function intersect_r8
  ...
  !!
  !! @ref horiz_interp_bilinear_new must be called before calling this routine.
  subroutine horiz_interp_bilinear_r8 ( Interp, data_in, data_out, verbose, mask_in,mask_out, &
       missing_value, missing_permit, new_handle_missing )
    !-----------------------------------------------------------------------
    type (horiz_interp_type), intent(in)        :: Interp !< Derived type variable containing
                                               !! interpolation indices and weights. Returned by a
    ...
    real(r8_kind), intent(out), dimension(:,:)           :: data_out !< output data on source grid
    integer, intent(in),               optional :: verbose !< 0 = no output; 1 = min,max,means; 2 =
                                                           !! all output
    real(r8_kind), intent(in), dimension(:,:),  optional :: mask_in !< Input mask, must be the same size as
                                    !! the input data. The real(r8_kind) value of mask_in must be in the
                                    !! range (0.,1.). Set mask_in=0.0 for data points
                                    !! that should not be used or have missing data
    real(r8_kind), intent(out), dimension(:,:), optional :: mask_out !< output mask that specifies whether
                                                            !! data was computed
    ...
    integer :: nlon_in, nlat_in, nlon_out, nlat_out, n, m,         &
         is, ie, js, je, iverbose, max_missing, num_missing, &
         miss_in, miss_out, iunit
    real(r8_kind)    :: dwtsum, wtsum, min_in, max_in, avg_in, &
         min_out, max_out, avg_out, wtw, wte, wts, wtn
    real(r8_kind)    :: mask(size(data_in,1), size(data_in,2) )
    logical :: set_to_missing, is_missing(4), new_handler
    real(r8_kind)    :: f1, f2, f3, f4, middle, w, s
    integer, parameter    :: kindl = r8_kind
    ...
    if(new_handler) then
       ...
       do n = 1, nlat_out
          do m = 1, nlon_out
             is = Interp % i_lon (m,n,1); ie = Interp % i_lon (m,n,2)
             js = Interp % j_lat (m,n,1); je = Interp % j_lat (m,n,2)
             wtw = Interp % horizInterpReals8_type % wti   (m,n,1)
             wte = Interp % horizInterpReals8_type % wti   (m,n,2)
             wts = Interp % horizInterpReals8_type % wtj   (m,n,1)
             wtn = Interp % horizInterpReals8_type % wtj   (m,n,2)
       ...
       do n = 1, nlat_out
          do m = 1, nlon_out
             ...
             if(num_missing .gt. max_missing ) then
                data_out(m,n) = missing_value
                if(present(mask_out)) mask_out(m,n) = 0.0_kindl
             else if(wtsum .lt. real(epsln, r8_kind)) then
                if(present(missing_value)) then
                   data_out(m,n) = missing_value
                else
                   data_out(m,n) = 0.0_kindl
...
903    format ('          number of missing points = ',i6)
  ...
  !> Subroutine for reading a weight file and use it to fill in the horiz interp type
  !! for the bilinear interpolation method.
  subroutine horiz_interp_read_weights_bilinear_r8(Interp, weight_filename, lon_out, lat_out, lon_in, lat_in, &
                                                 weight_file_source, interp_method, isw, iew, jsw, jew, nglon, nglat)
    type(horiz_interp_type), intent(inout) :: Interp             !< Horiz interp time to fill
    character(len=*),        intent(in)    :: weight_filename    !< Name of the weight file
    real(r8_kind), target,     intent(in)    :: lat_out(:,:)       !< Output (model) latitude
    real(r8_kind), target,      intent(in)    :: lon_out(:,:)       !< Output (model) longitude
    ...
    real(r8_kind), allocatable :: var(:,:,:)     !< Dummy variable to read the indices and weight into
    type(FmsNetcdfFile_t)           :: weight_fileobj !< FMS2io fileob for the weight file
    integer                         :: nlon           !< Number of longitudes in the model grid as read
                                                      !! from the weight file
    integer                         :: nlat           !< Number of latitude in the model grid as read
    ...
    allocate ( Interp % horizInterpReals8_type % wti (Interp%nlon_dst,Interp%nlat_dst,2),   &
              Interp % horizInterpReals8_type % wtj (Interp%nlon_dst,Interp%nlat_dst,2),   &
              Interp % i_lon (Interp%nlon_dst,Interp%nlat_dst,2), &
              Interp % j_lat (Interp%nlon_dst,Interp%nlat_dst,2))
    ...
    !! var(:,:,3) -> (i+1,j)
    !! var(:,:,4) -> (i+1,j+1)
    Interp % horizInterpReals8_type % wti = var(:,:,1:2)
    Interp % horizInterpReals8_type % wtj = var(:,:,3:4)
    deallocate(var)
    ...
    Interp% I_am_initialized = .True.
    call close_file(weight_fileobj)
  end subroutine horiz_interp_read_weights_bilinear_r8
# 49 "horiz_interp/include/horiz_interp_bilinear_r8.fh" 2
# 122 "horiz_interp/horiz_interp_bilinear.F90" 2
