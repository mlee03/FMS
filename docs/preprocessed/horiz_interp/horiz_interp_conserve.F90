...
# 137 "horiz_interp/horiz_interp_conserve.F90" 2
...
# 1 "horiz_interp/include/horiz_interp_conserve_r4.fh" 1
!***********************************************************************
!*                             Apache License 2.0
!*
!* This file is part of the GFDL Flexible Modeling System (FMS).
...
# 1 "horiz_interp/include/horiz_interp_conserve.inc" 1
!***********************************************************************
!*                             Apache License 2.0
!*
!* This file is part of the GFDL Flexible Modeling System (FMS).
...
!* governing permissions and limitations under the License.
!***********************************************************************
subroutine horiz_interp_conserve_new_1dx1d_r4 ( Interp, lon_in, lat_in, lon_out, lat_out, verbose)
    ...
    !-----------------------------------------------------------------------
    real(r4_kind), dimension(size(lat_out(:))-1,2) :: sph
    real(r4_kind), dimension(size(lon_out(:))-1,2) :: theta
    real(r4_kind), dimension(size(lat_in(:)))      :: slat_in
    real(r4_kind), dimension(size(lon_in(:))-1)    :: dlon_in
    real(r4_kind), dimension(size(lat_in(:))-1)    :: dsph_in
    real(r4_kind), dimension(size(lon_out(:))-1)   :: dlon_out
    real(r4_kind), dimension(size(lat_out(:))-1)   :: dsph_out
    real(r4_kind)                                  :: blon, fac, hpi, tpi, eps
    ...
    tpi = 4.0_kindl * real(hpi, r4_kind)
    Interp%version = 1
    nlon_in = size(lon_in(:))-1;  nlat_in = size(lat_in(:))-1
    nlon_out = size(lon_out(:))-1;  nlat_out = size(lat_out(:))-1

    allocate ( Interp % horizInterpReals4_type % facj (nlat_out,2), Interp % jlat (nlat_out,2),      &
               Interp % horizInterpReals4_type % faci (nlon_out,2), Interp % ilon (nlon_out,2),      &
               Interp % horizInterpReals4_type % area_src (nlon_in, nlat_in),   &
               Interp % horizInterpReals4_type % area_dst (nlon_out, nlat_out) )
          ...
          do iter=1,num_iters
             ...
             do j = 1, nlat_in
                if ( (s2n .and. (slat_in(j)-sph(n,n2)) <= eps .and.   &
                   ...
                   if (s2n) then
                      if (n2 == 1) Interp%horizInterpReals4_type%facj(n,n2) = 1.0_kindl - fac
                      if (n2 == 2) Interp%horizInterpReals4_type%facj(n,n2) = fac
                   else
                      if (n2 == 1) Interp%horizInterpReals4_type%facj(n,n2) = fac
                      if (n2 == 2) Interp%horizInterpReals4_type%facj(n,n2) = 1.0_kindl - fac
                   endif
                   exit
                endif
             enddo
...
710          format (': n,sph=',i3,f14.7,40x)
          ...
          do iter=1,num_iters
             ...
             do i = 1, nlon_in
                if ( (lon_in(i)-blon) <= eps .and. &
                   ...
                   if (m2 == 1) Interp%horizInterpReals4_type%faci(m,m2) = 1.0_kindl - fac
                   if (m2 == 2) Interp%horizInterpReals4_type%faci(m,m2) = fac
                   exit
                endif
             enddo
    ...
    do j = 1,nlat_in
       do i = 1,nlon_in
          Interp%horizInterpReals4_type%area_src(i,j) = dlon_in(i) * dsph_in(j)
       enddo
    enddo
    ...
    do n = 1, nlat_out
       do m = 1, nlon_out
          Interp%horizInterpReals4_type%area_dst(m,n) = dlon_out(m) * dsph_out(n)
       enddo
    enddo
    ...
    if (iverbose > 2) then
       write (*,801) (i,Interp%ilon(i,1),Interp%ilon(i,2),  &
            Interp%horizInterpReals4_type%faci(i,1),Interp%horizInterpReals4_type%faci(i,2),i=1,nlon_out)
       write (*,802) (j,Interp%jlat(j,1),Interp%jlat(j,2),  &
            Interp%horizInterpReals4_type%facj(j,1),Interp%horizInterpReals4_type%facj(j,2),j=1,nlat_out)
...
802    format (/,2x,'j',4x,'js',5x,'je',4x,'facjs',4x,'facje',  &
  ...
  end subroutine horiz_interp_conserve_new_1dx1d_r4

  !#######################################################################

  subroutine horiz_interp_conserve_new_1dx2d_r4 ( Interp, lon_in, lat_in, lon_out, lat_out, &
                                               mask_in, mask_out, verbose)
    type(horiz_interp_type),        intent(inout) :: Interp
    real(r4_kind), intent(in),              dimension(:)   :: lon_in , lat_in
    real(r4_kind), intent(in),              dimension(:,:) :: lon_out, lat_out
    real(r4_kind), intent(in),    optional, dimension(:,:) :: mask_in
    real(r4_kind), intent(inout), optional, dimension(:,:) :: mask_out
    integer, intent(in), optional                 :: verbose
    ...
    allocate(Interp%horizInterpReals4_type%area_frac_dst(nxgrid) )
    Interp%version = 2
    ...
    do i = 1, nxgrid
       Interp%horizInterpReals4_type%area_frac_dst(i) = real(xgrid_area(i)/dst_area(Interp%i_dst(i), Interp%j_dst(i) ), &
                                                    r4_kind)
    end do
    Interp%nlon_src = nlon_in;  Interp%nlat_src = nlat_in
    Interp%nlon_dst = nlon_out; Interp%nlat_dst = nlat_out
    if(present(mask_out)) then
       ...
       do i = 1, nxgrid
          mask_out(Interp%i_dst(i),Interp%j_dst(i)) = mask_out(Interp%i_dst(i), &
                  & Interp%j_dst(i)) + Interp%horizInterpReals4_type%area_frac_dst(i)
       end do
    end if
  ...
  end subroutine horiz_interp_conserve_new_1dx2d_r4

  !#######################################################################

  subroutine horiz_interp_conserve_new_2dx1d_r4 ( Interp, lon_in, lat_in, lon_out, lat_out, &
                                               mask_in, mask_out, verbose)
    type(horiz_interp_type),        intent(inout) :: Interp
    real(r4_kind), intent(in),              dimension(:,:) :: lon_in , lat_in
    real(r4_kind), intent(in),              dimension(:)   :: lon_out, lat_out
  ...
  end subroutine horiz_interp_conserve_new_2dx1d_r4

  !#######################################################################

  subroutine horiz_interp_conserve_new_2dx2d_r4 ( Interp, lon_in, lat_in, lon_out, lat_out, &
                                               mask_in, mask_out, verbose)
    type(horiz_interp_type),        intent(inout) :: Interp
    real(r4_kind), intent(in),              dimension(:,:) :: lon_in , lat_in
    real(r4_kind), intent(in),              dimension(:,:) :: lon_out, lat_out
  ...
  end subroutine horiz_interp_conserve_new_2dx2d_r4
  ...
  subroutine horiz_interp_conserve_version1_r4 ( Interp, data_in, data_out, verbose, &
    ...
    do n = 1, nlat_out
       ...
       if (Interp%jlat(n,1) <= Interp%jlat(n,2)) then
          ...
          fjs = Interp%horizInterpReals4_type%facj(n,1); fje = Interp%horizInterpReals4_type%facj(n,2)
       else
          js = Interp%jlat(n,2); je = Interp%jlat(n,1)
          fjs = Interp%horizInterpReals4_type%facj(n,2); fje = Interp%horizInterpReals4_type%facj(n,1)
       endif

       do m = 1, nlon_out
          ! longitude window
          is = Interp%ilon(m,1); ie = Interp%ilon(m,2)
          fis = Interp%horizInterpReals4_type%faci(m,1); fie = Interp%horizInterpReals4_type%faci(m,2)
          npass = 1
          dwtsum = 0.0_kindl
          wtsum =  0.0_kindl
          arsum =  0.0_kindl
          ...
          do np = 1, npass
             ...
             if ( np == 2 ) then
                ...
                fis = 1.0_kindl
                ie = Interp%ilon(m,2)
                fie = Interp%horizInterpReals4_type%faci(m,2)
             endif

             ! summing data*weight and weight for single grid point
             if (present(mask_in)) then
                call data_sum( data_in(is:ie,js:je), Interp%horizInterpReals4_type%area_src(is:ie,js:je), &
                     fis, fie, fjs,fje, dwtsum, wtsum, arsum, mask_in(is:ie,js:je)  )
             else if( allocated(Interp%horizInterpReals4_type%mask_in) ) then
                call data_sum( data_in(is:ie,js:je), Interp%horizInterpReals4_type%area_src(is:ie,js:je), &
                     fis, fie, fjs,fje, dwtsum, wtsum, arsum, Interp%horizInterpReals4_type%mask_in(is:ie,js:je)  )
             else
                call data_sum( data_in(is:ie,js:je), Interp%horizInterpReals4_type%area_src(is:ie,js:je), &
    ...
    if (iverbose > 0) then
       ...
       call stats(data_in, Interp%horizInterpReals4_type%area_src, asum, dsum, wsum, min_in, max_in, miss_in, mask_in)
       ! diagnostic messages
       ...
       call stats(data_out, Interp%horizInterpReals4_type%area_dst, asum, dsum, wsum, min_out, max_out, miss_out, mask_out)
       ! diagnostic messages
       if(pe == root_pe) then
          if (wsum > 0.0_kindl ) then
             avg_out=dsum/wsum
...
903    format ('          number of missing points = ',i6)
  ...
  subroutine horiz_interp_conserve_version2_r4 ( Interp, data_in, data_out, verbose )
    ...
    do i = 1, Interp%nxgrid
       ...
       data_out(i_dst, j_dst) = data_out(i_dst, j_dst) + data_in(i_src,j_src)*Interp%horizInterpReals4_type%area_frac_dst(i)
    end do
  ...
  subroutine data_sum_r4( grid_data, area, facis, facie, facjs, facje,  &
       dwtsum, wtsum, arsum, mask )

    !-----------------------------------------------------------------------
    real(r4_kind), intent(in), dimension(:,:) :: grid_data, area
...
# 52 "horiz_interp/include/horiz_interp_conserve_r4.fh" 2
# 203 "horiz_interp/horiz_interp_conserve.F90" 2

# 1 "horiz_interp/include/horiz_interp_conserve_r8.fh" 1
!***********************************************************************
!*                             Apache License 2.0
!*
!* This file is part of the GFDL Flexible Modeling System (FMS).
...
!* governing permissions and limitations under the License.
!***********************************************************************
subroutine horiz_interp_conserve_new_1dx1d_r8 ( Interp, lon_in, lat_in, lon_out, lat_out, verbose)
    type(horiz_interp_type), intent(inout) :: Interp
    ...
    !-----------------------------------------------------------------------
    real(r8_kind), dimension(size(lat_out(:))-1,2) :: sph
    real(r8_kind), dimension(size(lon_out(:))-1,2) :: theta
    real(r8_kind), dimension(size(lat_in(:)))      :: slat_in
    real(r8_kind), dimension(size(lon_in(:))-1)    :: dlon_in
    real(r8_kind), dimension(size(lat_in(:))-1)    :: dsph_in
    real(r8_kind), dimension(size(lon_out(:))-1)   :: dlon_out
    real(r8_kind), dimension(size(lat_out(:))-1)   :: dsph_out
    real(r8_kind)                                  :: blon, fac, hpi, tpi, eps
    integer, parameter :: num_iters = 4
    integer :: i, j, m, n, nlon_in, nlat_in, nlon_out, nlat_out,   &
         iverbose, m2, n2, iter
    logical :: s2n
    ...
    tpi = 4.0_kindl * real(hpi, r8_kind)
    Interp%version = 1
    nlon_in = size(lon_in(:))-1;  nlat_in = size(lat_in(:))-1
    nlon_out = size(lon_out(:))-1;  nlat_out = size(lat_out(:))-1

    allocate ( Interp % horizInterpReals8_type % facj (nlat_out,2), Interp % jlat (nlat_out,2),      &
               Interp % horizInterpReals8_type % faci (nlon_out,2), Interp % ilon (nlon_out,2),      &
               Interp % horizInterpReals8_type % area_src (nlon_in, nlat_in),   &
               Interp % horizInterpReals8_type % area_dst (nlon_out, nlat_out) )
          ...
          do iter=1,num_iters
             ...
             do j = 1, nlat_in
                if ( (s2n .and. (slat_in(j)-sph(n,n2)) <= eps .and.   &
                   ...
                   fac = (sph(n,n2)-slat_in(j))/(slat_in(j+1)-slat_in(j))
                   if (s2n) then
                      if (n2 == 1) Interp%horizInterpReals8_type%facj(n,n2) = 1.0_kindl - fac
                      if (n2 == 2) Interp%horizInterpReals8_type%facj(n,n2) = fac
                   else
                      if (n2 == 1) Interp%horizInterpReals8_type%facj(n,n2) = fac
                      if (n2 == 2) Interp%horizInterpReals8_type%facj(n,n2) = 1.0_kindl - fac
                   endif
                   exit
                endif
             enddo
...
710          format (': n,sph=',i3,f14.7,40x)
          ...
          do iter=1,num_iters
             ...
             do i = 1, nlon_in
                if ( (lon_in(i)-blon) <= eps .and. &
                   ...
                   Interp%ilon(m,m2) = i
                   fac = (blon-lon_in(i))/(lon_in(i+1)-lon_in(i))
                   if (m2 == 1) Interp%horizInterpReals8_type%faci(m,m2) = 1.0_kindl - fac
                   if (m2 == 2) Interp%horizInterpReals8_type%faci(m,m2) = fac
                   exit
                endif
             enddo
             if ( Interp%ilon(m,m2) /= 0 ) exit
    ...
    do j = 1,nlat_in
       do i = 1,nlon_in
          Interp%horizInterpReals8_type%area_src(i,j) = dlon_in(i) * dsph_in(j)
       enddo
    enddo
    ...
    do n = 1, nlat_out
       do m = 1, nlon_out
          Interp%horizInterpReals8_type%area_dst(m,n) = dlon_out(m) * dsph_out(n)
       enddo
    enddo
    ...
    if (iverbose > 2) then
       write (*,801) (i,Interp%ilon(i,1),Interp%ilon(i,2),  &
            Interp%horizInterpReals8_type%faci(i,1),Interp%horizInterpReals8_type%faci(i,2),i=1,nlon_out)
       write (*,802) (j,Interp%jlat(j,1),Interp%jlat(j,2),  &
            Interp%horizInterpReals8_type%facj(j,1),Interp%horizInterpReals8_type%facj(j,2),j=1,nlat_out)
801    format (/,2x,'i',4x,'is',5x,'ie',4x,'facis',4x,'facie',  &
            /,(i4,2i7,2f10.5))
802    format (/,2x,'j',4x,'js',5x,'je',4x,'facjs',4x,'facje',  &
            /,(i4,2i7,2f10.5))
  ...
  end subroutine horiz_interp_conserve_new_1dx1d_r8

  !#######################################################################

  subroutine horiz_interp_conserve_new_1dx2d_r8 ( Interp, lon_in, lat_in, lon_out, lat_out, &
                                               mask_in, mask_out, verbose)
    type(horiz_interp_type),        intent(inout) :: Interp
    real(r8_kind), intent(in),              dimension(:)   :: lon_in , lat_in
    real(r8_kind), intent(in),              dimension(:,:) :: lon_out, lat_out
    real(r8_kind), intent(in),    optional, dimension(:,:) :: mask_in
    real(r8_kind), intent(inout), optional, dimension(:,:) :: mask_out
    integer, intent(in), optional                 :: verbose
    ...
    allocate(Interp%i_src(nxgrid), Interp%j_src(nxgrid) )
    allocate(Interp%i_dst(nxgrid), Interp%j_dst(nxgrid) )
    allocate(Interp%horizInterpReals8_type%area_frac_dst(nxgrid) )
    Interp%version = 2
    Interp%nxgrid   = nxgrid
    Interp%i_src = i_src(1:nxgrid)+1 ! in C, the starting index is 0
    Interp%j_src = j_src(1:nxgrid)+1
    ...
    do i = 1, nxgrid
       Interp%horizInterpReals8_type%area_frac_dst(i) = real(xgrid_area(i)/dst_area(Interp%i_dst(i), Interp%j_dst(i) ), &
                                                    r8_kind)
    end do
    Interp%nlon_src = nlon_in;  Interp%nlat_src = nlat_in
    Interp%nlon_dst = nlon_out; Interp%nlat_dst = nlat_out
    if(present(mask_out)) then
       ...
       do i = 1, nxgrid
          mask_out(Interp%i_dst(i),Interp%j_dst(i)) = mask_out(Interp%i_dst(i), &
                  & Interp%j_dst(i)) + Interp%horizInterpReals8_type%area_frac_dst(i)
       end do
    end if
  ...
  end subroutine horiz_interp_conserve_new_1dx2d_r8

  !#######################################################################

  subroutine horiz_interp_conserve_new_2dx1d_r8 ( Interp, lon_in, lat_in, lon_out, lat_out, &
                                               mask_in, mask_out, verbose)
    type(horiz_interp_type),        intent(inout) :: Interp
    real(r8_kind), intent(in),              dimension(:,:) :: lon_in , lat_in
    real(r8_kind), intent(in),              dimension(:)   :: lon_out, lat_out
  ...
  end subroutine horiz_interp_conserve_new_2dx1d_r8

  !#######################################################################

  subroutine horiz_interp_conserve_new_2dx2d_r8 ( Interp, lon_in, lat_in, lon_out, lat_out, &
                                               mask_in, mask_out, verbose)
    type(horiz_interp_type),        intent(inout) :: Interp
    real(r8_kind), intent(in),              dimension(:,:) :: lon_in , lat_in
    real(r8_kind), intent(in),              dimension(:,:) :: lon_out, lat_out
  ...
  end subroutine horiz_interp_conserve_new_2dx2d_r8
  ...
  subroutine horiz_interp_conserve_version1_r8 ( Interp, data_in, data_out, verbose, &
    ...
    do n = 1, nlat_out
       ...
       if (Interp%jlat(n,1) <= Interp%jlat(n,2)) then
          js = Interp%jlat(n,1); je = Interp%jlat(n,2)
          fjs = Interp%horizInterpReals8_type%facj(n,1); fje = Interp%horizInterpReals8_type%facj(n,2)
       else
          js = Interp%jlat(n,2); je = Interp%jlat(n,1)
          fjs = Interp%horizInterpReals8_type%facj(n,2); fje = Interp%horizInterpReals8_type%facj(n,1)
       endif

       do m = 1, nlon_out
          ! longitude window
          is = Interp%ilon(m,1); ie = Interp%ilon(m,2)
          fis = Interp%horizInterpReals8_type%faci(m,1); fie = Interp%horizInterpReals8_type%faci(m,2)
          npass = 1
          dwtsum = 0.0_kindl
          wtsum =  0.0_kindl
          arsum =  0.0_kindl
          ...
          do np = 1, npass
             ...
             if ( np == 2 ) then
                ...
                fis = 1.0_kindl
                ie = Interp%ilon(m,2)
                fie = Interp%horizInterpReals8_type%faci(m,2)
             endif

             ! summing data*weight and weight for single grid point
             if (present(mask_in)) then
                call data_sum( data_in(is:ie,js:je), Interp%horizInterpReals8_type%area_src(is:ie,js:je), &
                     fis, fie, fjs,fje, dwtsum, wtsum, arsum, mask_in(is:ie,js:je)  )
             else if( allocated(Interp%horizInterpReals8_type%mask_in) ) then
                call data_sum( data_in(is:ie,js:je), Interp%horizInterpReals8_type%area_src(is:ie,js:je), &
                     fis, fie, fjs,fje, dwtsum, wtsum, arsum, Interp%horizInterpReals8_type%mask_in(is:ie,js:je)  )
             else
                call data_sum( data_in(is:ie,js:je), Interp%horizInterpReals8_type%area_src(is:ie,js:je), &
                     fis, fie, fjs,fje,  dwtsum, wtsum, arsum    )
             endif
    ...
    if (iverbose > 0) then
       ...
       call stats(data_in, Interp%horizInterpReals8_type%area_src, asum, dsum, wsum, min_in, max_in, miss_in, mask_in)
       ! diagnostic messages
       ! on the root_pe, we can calculate the global mean, minimum and maximum.
       if(pe == root_pe) then
          if (wsum > 0.0_kindl) then
       ...
       ! compute statistics of output data
       call stats(data_out, Interp%horizInterpReals8_type%area_dst, asum, dsum, wsum, min_out, max_out, miss_out, mask_out)
       ! diagnostic messages
       if(pe == root_pe) then
          if (wsum > 0.0_kindl ) then
             avg_out=dsum/wsum
...
903    format ('          number of missing points = ',i6)
  ...
  subroutine horiz_interp_conserve_version2_r8 ( Interp, data_in, data_out, verbose )
    ...
    do i = 1, Interp%nxgrid
       i_src = Interp%i_src(i); j_src = Interp%j_src(i)
       i_dst = Interp%i_dst(i); j_dst = Interp%j_dst(i)
       data_out(i_dst, j_dst) = data_out(i_dst, j_dst) + data_in(i_src,j_src)*Interp%horizInterpReals8_type%area_frac_dst(i)
    end do
  ...
  !> sums up the data and weights for a single output grid box
  subroutine data_sum_r8( grid_data, area, facis, facie, facjs, facje,  &
       dwtsum, wtsum, arsum, mask )

    !-----------------------------------------------------------------------
    real(r8_kind), intent(in), dimension(:,:) :: grid_data, area
...
# 52 "horiz_interp/include/horiz_interp_conserve_r8.fh" 2
# 204 "horiz_interp/horiz_interp_conserve.F90" 2
