...
program test_mpp_domains
  ...
  integer, parameter :: nx=20, ny=20, nz=1 !40
  ...
  !>
  !call test_global_field_r4_4d( 'Non-symmetry' )
...
contains
  ...
  subroutine test_global_field_r4_2d( type )
    ...
    call compare_checksums( global1(1:ni,js:je), gcheck(1:ni,js:je), &
                            type//' mpp_global_field xupdate only on r4 data domain' )
    ...
    call compare_checksums( global1(is:ie,1:nj), gcheck(is:ie,1:nj), &
                            type//' mpp_global_field yupdate only on r4 data domain' )
    !call mpp_clock_begin(id)
    call mpp_global_field( domain, x, gcheck, position=position )
    !call mpp_clock_end(id)
    !> compare checksums between global and x arrays
    call compare_checksums( global1(1:ni,1:nj), gcheck, &
                            type//' mpp_global_field on r4 data domain' )
  ...
  subroutine test_global_field_r8_3d( type )
    ...
    !> compare checksums between global and x arrays
    call compare_checksums( global1(1:ni,js:je,:), gcheck(1:ni,js:je,:), &
                            type//' mpp_global_field xupdate only on r8 data domain' )
    ...
    !> compare checksums between global and x arrays
    call compare_checksums( global1(is:ie,1:nj,:), gcheck(is:ie,1:nj,:), &
                            type//' mpp_global_field yupdate only on r8 data domain' )
    ...
    !> compare checksums between global and x arrays
    call compare_checksums( global1(1:ni,1:nj,:), gcheck, &
                            type//' mpp_global_field on r8 data domain' )
  ...
  !>
  !> test_global_field_r4_4d
  !>
  subroutine test_global_field_r4_4d( type )

    implicit none

    character(len=*), intent(in) :: type

    integer, parameter :: lsize=2

    type(domain2D)  :: domain
    integer         :: position, ishift, jshift, ni, nj, i, j, k, l
    integer         :: is, ie, js, je, isd, ied, jsd, jed
    !integer        :: id
    integer, allocatable :: pelist(:)
    real(kind=r4_kind), allocatable  :: global1(:,:,:,:), x(:,:,:,:), gcheck(:,:,:,:)
    ...
    ied = ied+ishift ; jed = jed+jshift
    ni  = nx+ishift  ; nj  = ny+jshift
    allocate( global1(1-whalo:ni+ehalo,1-shalo:nj+nhalo,nz,lsize) )
    global1 = real( 0.0, kind=r4_kind )
    do l=1, lsize
       do k=1, nz
          do j=1, nj
             do i=1, ni
                global1(i,j,k,l) = real( k+i*1e-3+j*1e-6, kind=r4_kind )
             end do
          end do
       enddo
    end do

    allocate( gcheck(ni,nj,nz,lsize) )
    allocate( x(isd:ied,jsd:jed,nz,lsize) )
  ...
  end subroutine test_global_field_r4_4d
